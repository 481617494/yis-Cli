...
//! grok-build implements it for `xai_yis_sampling_types::ConversationItem`.
//!
...
//! `<yis_user_queries>` blocks, drop tool content from assistant turns,
...
/// extraction (`<yis_file id=".." name=".." />` lines in the
/// `<yis_user_queries>` preamble).
...
pub trait CompactionItem {
    ...
    /// strips their `<yis_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<yis_file>` lines in the
    /// `<yis_user_queries>` preamble. Empty for items without attachments.
    ///
