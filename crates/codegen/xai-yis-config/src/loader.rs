...
use crate::paths::{system_config_dir, user_yis_home};
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_yis_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_yis_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.yis/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.yis` to the user tier).
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_yis_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `YIS_CAMPAIGNS_OVERRIDE` env layer.
...
/// `YIS_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("YIS_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`xai_yis_shell::util::config::load_effective_config`) when remote campaigns
/// or `YIS_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `YIS_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_yis_version::installed_semver() {
...
mod tests {
    ...
    /// `YIS_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `YIS_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("YIS_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads YIS_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("YIS_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("YIS_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("YIS_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("YIS_CAMPAIGNS") },
        }
    }
}
