//! Filesystem locations for yis config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static YIS_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user yis directory (`~/.yis`, canonicalized) used when
/// `YIS_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`yis_home()`] is the default without duplicating the computation.
///
...
/// `~/.yis/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_yis_home` (deliberately standalone crate).
pub fn default_yis_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".yis")
}

/// Per-user config directory: `$YIS_HOME` or `~/.yis`. Created if needed.
pub fn yis_home() -> PathBuf {
    YIS_HOME
        .get_or_init(|| {
            let yis_home = if let Ok(v) = std::env::var("YIS_HOME") {
                PathBuf::from(v)
            } else {
                default_yis_home()
            };
...
/// The user-global yis home, but only when one genuinely resolves: `Some` when
/// `$YIS_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`yis_home()`], this never falls back to a cwd-relative `.yis`, so callers
/// that *scan* user-global yis resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.yis` tree for the user-global one when no home resolves.
pub fn user_yis_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("YIS_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(yis_home)
}

/// Canonical yis application path: `$YIS_HOME/bin/yis` (Unix) or `yis.exe` (Windows).
pub fn yis_application() -> PathBuf {
    let name = if cfg!(windows) { "yis.exe" } else { "yis" };
    yis_home().join("bin").join(name)
}

/// System-wide config directory: `/etc/yis/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/yis"))
    } else {
        None
    }
}
...
/// `yis_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    yis_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = yis_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_yis_home_has_no_verbatim_prefix() {
        ...
        let home = default_yis_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".yis"));
    }
