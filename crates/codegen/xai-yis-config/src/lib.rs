...
//! 1. `/etc/yis/managed_config.toml`
//! 2. `$YIS_HOME/managed_config.toml`
//! 3. `$YIS_HOME/config.toml`
//! 4. `$YIS_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/yis/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.yis`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_yis_home, encode_cwd_dirname, ensure_sessions_cwd_dir, yis_application, yis_home,
    sessions_cwd_dir, system_config_dir, user_yis_home,
};
pub use validation::{
