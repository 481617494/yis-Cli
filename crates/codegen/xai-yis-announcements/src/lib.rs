//! Shared announcement types, persistence, and formatting for Yis Cli apps.
//!
//! This crate provides the common logic used by `xai-yis-shell` and
//! `xai-yis-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.yis/announcements.json`.
...
/// Write hidden announcement ids to `~/.yis/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    xai_yis_tools::util::yis_home::yis_home().join("announcements.json")
}
...
///
/// Precedence: `YIS_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("YIS_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid YIS_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("YIS_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("YIS_ANNOUNCEMENTS_OVERRIDE");
        }
    }
