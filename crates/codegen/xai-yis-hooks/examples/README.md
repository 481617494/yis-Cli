...
Sample hooks for Grok. Copy to `~/.yis/hooks/` to enable globally, or to `<project>/.yis/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.yis/hooks/bin
cp examples/hooks/safe-shell.json ~/.yis/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.yis/hooks/bin/
chmod +x ~/.yis/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.yis/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.yis/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.yis/hooks/bin/
chmod +x ~/.yis/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.yis/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.yis/hooks/bin
cp examples/hooks/session-log.json ~/.yis/hooks/
cp examples/hooks/bin/session-log.sh ~/.yis/hooks/bin/
chmod +x ~/.yis/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.yis/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.yis/hooks/bin
cp examples/hooks/tool-logger.json ~/.yis/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.yis/hooks/bin/
chmod +x ~/.yis/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.yis/hooks/`. The hook stops running on the next session.