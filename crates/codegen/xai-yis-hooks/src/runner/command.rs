...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("YIS_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = xai_yis_config::shell::shell_command_argv(&command_str);
    ...
    // `xai_yis_tools::util::detach_command`: Unix uses the same setsid /
    ...
    // plugin) can spoof `YIS_HOOK_EVENT`, `YIS_HOOK_NAME`, `YIS_SESSION_ID`,
    // `YIS_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("YIS_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("YIS_HOOK_NAME", &spec.name)
        .env("YIS_SESSION_ID", ctx.session_id)
        .env("YIS_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `YIS_WORKSPACE_ROOT`; native `.yis` hooks should use
        // `YIS_WORKSPACE_ROOT`.
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.yis/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
            Some(std::path::PathBuf::from(
                "/project/.yis/hooks/bin/check.sh"
            ))
        );
    }
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${YIS_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${YIS_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.yis-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".yis-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.yis-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.yis-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.yis-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
