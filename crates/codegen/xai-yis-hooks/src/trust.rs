...
// (`~/.yis/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_yis_home>/trusted-hook-projects`), or `None` when no user yis home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_yis_config::user_yis_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user yis home (set $YIS_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$YIS_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_yis_config::user_yis_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let yis_dir = dir.join(".yis");
        std::fs::create_dir_all(&yis_dir).unwrap();
        yis_dir.join("trusted-hook-projects")
    }
