use regex::Regex;
use xai_yis_tools::types::{claude_names_for, yis_names_for};
...
/// `xai-yis-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for yis_name in yis_names_for(term) {
            push(yis_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_yis_tool() {
