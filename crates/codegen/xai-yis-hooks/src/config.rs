...
pub struct RawHandler {
    ...
    /// keys (`YIS_HOOK_EVENT`, `YIS_HOOK_NAME`, `YIS_SESSION_ID`,
    /// `YIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`YIS_HOOK_EVENT`, `YIS_HOOK_NAME`, `YIS_SESSION_ID`,
    ///    `YIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`xai-yis-agent::plugins::hooks_adapter`). The adapter sets
    ///    `YIS_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `YIS_PLUGIN_DATA`,
    ...
    ///    (`YIS_HOOK_EVENT`, `YIS_HOOK_NAME`, `YIS_SESSION_ID`,
    ///    `YIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.yis/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.yis/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "YIS_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "YIS_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "YIS_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "YIS_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "YIS_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "YIS_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (YIS_HOOK_*,
    /// YIS_SESSION_ID, YIS_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "YIS_HOOK_EVENT": "spoofed",
                                    "YIS_HOOK_NAME": "spoofed",
                                    "YIS_SESSION_ID": "spoofed",
                                    "YIS_WORKSPACE_ROOT": "/etc",
