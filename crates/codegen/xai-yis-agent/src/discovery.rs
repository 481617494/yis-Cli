...
//!
//! Searches `.yis/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.yis/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.yis/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".yis/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.yis/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.yis/agents` / `.claude/agents`) under each
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, yis_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.yis/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.yis/agents/` walking from `cwd` up to repo root
/// 2. `~/.yis/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.yis/bundled/agents/` (bundled, lowest priority)
///
...
/// compat agents, then bundled. `.yis` dirs resolve from `yis_home`
/// (YIS_HOME-aware) plus the legacy literal `~/.yis` when YIS_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    yis_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.yis, included only when it differs from yis_home
    // (i.e. YIS_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_grok = home
        .map(|h| h.join(".yis"))
        .filter(|legacy| yis_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = yis_home {
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, yis_home) {
        if dir.is_dir() {
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, yis_home);
...
///
/// Project-level `.yis/agents/` has highest priority, then falls back
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, yis_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, yis_home) {
        return Some(def);
    }
...
/// Load project agent definitions from every `.yis/agents` / `.claude/agents`
...
mod tests {
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_grok_when_yis_home_differs() {
        ...
        assert!(paths.contains(&home.join(".yis").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&grok.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".yis").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_yis_home_is_dot_grok() {
        let home = Path::new("/home/u");
        let grok = home.join(".yis");
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.yis/agents when yis_home == ~/.yis"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_yis_build() {
        let def = by_name("yis-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "yis-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".yis").join("agents");
        let agents_dir_2 = inner_dir.join(".yis").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".yis").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".yis")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".yis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".yis").join("agents");
        let bundled_dir = home.join(".yis").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".yis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".yis")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".yis").join("agents");
        let bundled_dir = home.join(".yis").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "yis-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "grok-build.md",
            "yis-build",
        ...
        let def = by_name_in_cwd("yis-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "yis-build");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .yis/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        ...
        assert_eq!(variant, BuiltinAgentName::YisBuildOrchestrator);
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.yis/agents/) agent named "explore" should NOT shadow
