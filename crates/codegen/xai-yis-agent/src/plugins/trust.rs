...
//!
//! Plugins from project directories (`.yis/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.yis/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.yis/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.yis/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_yis_home() so a project's `.yis/trusted-plugins` is never
        // read as the user trust store when neither YIS_HOME nor a home dir resolves.
        let Some(grok) = xai_yis_config::user_yis_home() else {
            return Self {
                trusted: HashSet::new(),
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.yis/trusted-plugins`.
    ...
    /// rewrites `~/.yis/trusted-plugins` without the revoked entry.
