//! Agent definition types — parsed from `.yis/agents/*.md` files.
...
use xai_yis_tools::implementations::codex;
use xai_yis_tools::implementations::yis_build;
use xai_yis_tools::implementations::yis_build_concise;
use xai_yis_tools::implementations::memory;
use xai_yis_tools::implementations::opencode;
use xai_yis_tools::implementations::search_tool;
use xai_yis_tools::implementations::use_tool;
use xai_yis_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard YisBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&yis_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&yis_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&yis_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&yis_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&yis_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_yis_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_yis_build_toolset() -> ToolServerConfig {
    let mut tools = default_yis_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&yis_build::EnterPlanModeTool).into());
    tools.push((&yis_build::ExitPlanModeTool).into());
    tools.push((&yis_build::AskUserQuestionTool).into());
    tools.push((&yis_build::WebSearchTool).into());
    tools.push((&yis_build::ImageGenTool).into());
    tools.push((&yis_build::ImageToVideoTool).into());
    tools.push((&yis_build::ReferenceToVideoTool).into());
    tools.push((&yis_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&yis_build::LspTool).into());
    ToolServerConfig {
        tools,
...
fn yis_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&yis_build::ReadFileTool).into(),
        (&yis_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&yis_build::ListDirTool).into(),
        (&yis_build::GrepTool).into(),
        (&yis_build::KillTerminalCommandTool).into(),
        (&yis_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("yis-build", workspace_yis_build_toolset()),
        ("grok-build-concise", yis_build_concise_toolset()),
        ("grok-build-plan", yis_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("yis-computer", yis_computer_toolset()),
    ]
}
...
fn default_yis_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&yis_build::ReadFileTool).into(),
            ...
            (&yis_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&yis_build::SchedulerCreateTool).into(),
            (&yis_build::SchedulerDeleteTool).into(),
            (&yis_build::SchedulerListTool).into(),
            (&yis_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&yis_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn yis_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&yis_build_concise::BashConciseTool).into(),
            (&yis_build_concise::ReadFileConciseTool).into(),
            (&yis_build_concise::SearchReplaceConciseTool).into(),
            (&yis_build::ListDirTool).into(),
            (&yis_build::GrepTool).into(),
            kill_task_tool_config(),
            (&yis_build::TodoWriteTool).into(),
...
pub fn yis_build_hashline_toolset(
    hashline_tools: Vec<xai_yis_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_yis_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
fn yis_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&yis_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&yis_build::UpdateGoalTool).into(),
            (&yis_build::EnterPlanModeTool).into(),
            (&yis_build::ExitPlanModeTool).into(),
            (&yis_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&yis_build::MonitorTool).into(),
            (&yis_build::WebSearchTool).into(),
            (&yis_build::WebFetchTool).into(),
            (&yis_build::ImageGenTool).into(),
            (&yis_build::ImageToVideoTool).into(),
            (&yis_build::ReferenceToVideoTool).into(),
...
/// Yis Cli + plan mode toolset WITHOUT subagent tools.
///
/// Same as `yis_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn yis_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&yis_build::ReadFileTool).into(),
...
/// Default Yis Cli toolset + `ask_user_question`.
///
/// Same as `default_yis_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn yis_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&yis_build::ReadFileTool).into(),
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::YisBuild => AgentDefinition::default_yis_build(),
            Self::YisBuildConcise => AgentDefinition::yis_build_concise(),
            Self::YisBuildPlan => AgentDefinition::yis_build_plan(),
            Self::YisBuildPlanNoSubagents => AgentDefinition::yis_build_plan_no_subagents(),
            Self::YisBuildAskUser => AgentDefinition::yis_build_ask_user(),
            ...
            Self::YisBuildOrchestrator => AgentDefinition::yis_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .yis/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_yis_build_toolset")]
...
pub enum AgentScope {
    /// .yis/agents/ (project-level, highest priority)
    Project,
    /// ~/.yis/agents/ (user-level)
    User,
    /// ~/.yis/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_yis_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.yis/agent-memory/<name>/`
    User,
    /// `<project>/.yis/agent-memory/<name>/`
    Project,
    /// `<project>/.yis/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_yis_config::yis_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".yis/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".yis/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `xai_yis_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let grok = xai_yis_config::user_yis_home();
        ...
        if path_str.contains(".yis/agents/") || path_str.contains(".yis\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".yis/bundled/agents/")
            || path_str.contains(".yis\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_yis_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["YisBuild:read_file", "YisBuildHashline:hashline_read"],
            [
                "YisBuild:search_replace",
                "YisBuildHashline:hashline_edit",
            ],
            ["YisBuild:grep", "YisBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_yis_build_toolset(),
    ...
    pub fn default_yis_build() -> Self {
        Self::base(
            BuiltinAgentName::YisBuild,
            "Yis Cli agent for software engineering tasks.",
        )
    }
    /// Yis Cli Concise agent definition — concise output format for SFT/RL.
    pub fn yis_build_concise() -> Self {
        Self {
            tool_config: yis_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::YisBuildConcise,
                "Yis Cli agent with concise output format.",
            )
        }
    }
    /// Yis Cli agent with plan mode tools.
    pub fn yis_build_plan() -> Self {
        Self {
            tool_config: yis_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::YisBuildPlan,
                "Yis Cli agent with plan mode support.",
            )
        }
    }
    /// Yis Cli + plan mode WITHOUT subagent tools.
    pub fn yis_build_plan_no_subagents() -> Self {
        Self {
            tool_config: yis_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::YisBuildPlanNoSubagents,
                "Yis Cli agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Yis Cli agent with the `ask_user_question` tool.
    pub fn yis_build_ask_user() -> Self {
        Self {
            tool_config: yis_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::YisBuildAskUser,
                "Yis Cli agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// Yis Cli Orchestrator — GBL model with full YisBuild tools
    ...
    pub fn yis_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::YisBuildOrchestrator,
                "YisBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_yis_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("yis-build").unwrap();
    ...
    fn yis_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&yis_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&yis_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn yis_computer_preset_is_curated_yis_build_subset() {
        let gc = toolset_for_preset("yis-computer").unwrap();
        let gb = toolset_for_preset("yis-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = yis_computer_exclusive_ids();
    ...
    #[test]
    fn yis_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("yis-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&yis_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&yis_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&yis_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&yis_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&yis_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&yis_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&yis_build::KillTerminalCommandTool).id
            {
    ...
    #[test]
    fn yis_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("yis-computer").unwrap();
    ...
    #[test]
    fn yis_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("yis-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&yis_build::LspTool).id,
            ToolConfig::from(&yis_build::EnterPlanModeTool).id,
            ToolConfig::from(&yis_build::ExitPlanModeTool).id,
        ] {
            assert!(
        ...
        let full = workspace_yis_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_yis_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::YisBuildOrchestrator => true,
            BuiltinAgentName::YisBuild
            | BuiltinAgentName::YisBuildConcise
            | BuiltinAgentName::YisBuildPlan
            | BuiltinAgentName::YisBuildPlanNoSubagents
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.yis/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.yis/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_yis_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have yis_build tools"
        );
    }
    #[test]
    ...
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("yis-build", BuiltinAgentName::YisBuild),
            ("grok-build-concise", BuiltinAgentName::YisBuildConcise),
            ("grok-build-ask-user", BuiltinAgentName::YisBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_yis_build();
