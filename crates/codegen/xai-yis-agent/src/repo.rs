...
//! primitive consumed cross-crate by `xai-yis-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.yis/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.yis`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `xai-yis-workspace`, which depends on THIS crate) to keep the dep edge
