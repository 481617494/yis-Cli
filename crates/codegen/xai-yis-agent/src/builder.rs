...
use std::sync::Arc;
use xai_yis_tools::bridge::ToolBridge;
use xai_yis_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_yis_tools::notification::ToolNotificationHandle;
use xai_yis_tools::registry::types::SessionContext;
...
/// tool registry in `xai-yis-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    xai_yis_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.yis/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<xai_yis_tools::implementations::yis_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn xai_yis_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: xai_yis_tools::implementations::web_search::WebSearchConfig,
    ...
    lsp: Option<std::sync::Arc<dyn xai_yis_tools::implementations::lsp::LspBackend>>,
    ...
    app_builder_deployer_config:
        xai_yis_tools::implementations::yis_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: xai_yis_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<xai_yis_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in xai-yis-shell).
...
fn ensure_plan_mode_tools(tool_config: &mut xai_yis_tools::registry::types::ToolServerConfig) {
    use xai_yis_tools::implementations::yis_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("YisBuild:enter_plan_mode");
    let missing_exit = !existing.contains("YisBuild:exit_plan_mode");
    let missing_ask = !existing.contains("YisBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&yis_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&yis_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&yis_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut xai_yis_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(xai_yis_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: xai_yis_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<xai_yis_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn xai_yis_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: xai_yis_tools::implementations::yis_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: xai_yis_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `YIS_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: xai_yis_tools::implementations::yis_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn xai_yis_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: xai_yis_tools::implementations::yis_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: xai_yis_tools::implementations::yis_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: xai_yis_tools::implementations::yis_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: xai_yis_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the YisBuild Task description.
    ...
    ///
    /// When disabled, `YisBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.yis/skills, ~/.yis/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use xai_yis_tools::implementations::yis_build;
                tool_config.tools.push((&yis_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use xai_yis_tools::implementations::yis_build;
                tool_config.tools.push((&yis_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&xai_yis_tools::implementations::yis_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&xai_yis_tools::implementations::yis_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&xai_yis_tools::implementations::yis_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&xai_yis_tools::implementations::yis_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&xai_yis_tools::implementations::yis_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&xai_yis_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let yis_build_ns = xai_yis_tools::types::tool::ToolNamespace::YisBuild.to_string();
            let mem_search_id = format!(
                "{yis_build_ns}:{}",
                xai_yis_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{yis_build_ns}:{}",
                xai_yis_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                xai_yis_tools::types::tool::ToolNamespace::YisBuild,
            );
        ...
        if task_stripped {
            use xai_yis_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::YisBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::YisBuildConcise, "run_terminal_cmd", true)
        ...
        if let xai_yis_tools::implementations::yis_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["YisBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "YisBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                xai_yis_tools::types::resources::TruncationCfg(
                    xai_yis_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(xai_yis_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(xai_yis_sampling_types::HostedTool::XSearch);
            }
        }
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        xai_yis_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: xai_yis_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: xai_yis_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("yis", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use xai_yis_tools::types::template_renderer::TemplateRenderer;
        use xai_yis_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                ...
                profile: AgentDefinition::default_yis_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::yis_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::yis_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
            ...
            PagerFlagCase {
                ...
                profile: AgentDefinition::yis_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_yis_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::implementations::yis_build::ask_user_question::AskUserQuestionParams;
        use xai_yis_tools::notification::ToolNotificationHandle;
        use xai_yis_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_yis_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "YisBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_yis_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("YisBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::implementations::yis_build::bash::BashParams;
        use xai_yis_tools::notification::ToolNotificationHandle;
        use xai_yis_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_yis_build();
    ...
    #[tokio::test]
    async fn claude_tool_names_map_to_yis_equivalents() {
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use xai_yis_tools::computer::local::LocalTerminalBackend;
        use xai_yis_tools::implementations::yis_build::web_fetch::WebFetchConfig;
        use xai_yis_tools::implementations::web_search::WebSearchConfig;
        use xai_yis_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_yis_build();
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, xai_yis_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, xai_yis_sampling_types::HostedTool::XSearch)),
