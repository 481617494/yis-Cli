...
//! - `Default` -- the legacy Yis Cli prefix (built by the shell layer).
...
use xai_yis_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Yis Cli prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .yis/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_yis_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.yis/, ~/.claude/).
    ...
    /// (`~/.yis/projects/<encoded-cwd>/mcps`). Surfaced in
