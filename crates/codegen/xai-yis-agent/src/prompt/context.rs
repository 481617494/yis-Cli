...
//! `TemplateRenderer` in `xai-yis-tools`. This struct does NOT own a
...
/// Default identity in the primary system prompt (`You are Yis Cli…`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "Yis Cli 由中节能大数据开发的终端助手";
...
mod tests {
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.yis/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Yis Cli agent"),
