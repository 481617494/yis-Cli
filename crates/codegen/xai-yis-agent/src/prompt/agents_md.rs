...
//!
//! Searches from cwd to repo root, plus `~/.yis/`. Also discovers
//! `*.md` files in `.yis/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.yis/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.yis/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = xai_yis_tools::util::yis_home::yis_home();
    ...
    // Always include yis_home (~/.yis/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `xai-yis-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .yis/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.yis/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            xai_yis_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
