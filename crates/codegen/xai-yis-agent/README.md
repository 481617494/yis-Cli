...
This crate extracts a first-class `Agent` type from `xai-yis-shell`.
...
`xai-yis-shell`, another in-process host, or a headless batch runner.
...
in `.yis/agents/` (project-level) or `~/.yis/agents/` (user-level).
...
let def = AgentDefinition::from_file(".yis/agents/code-reviewer.md")?;
...
// Find all .md files in .yis/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xai-yis-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xai-yis-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xai-yis-agent` depends on it for tool setup.
- **`xai-yis-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xai-yis-agent` (AGENTS.md
...
|---|---|---|
| `yis-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p xai-yis-agent

# Test
cargo test -p xai-yis-agent

# Clippy
cargo clippy -p xai-yis-agent --fix --allow-dirty
