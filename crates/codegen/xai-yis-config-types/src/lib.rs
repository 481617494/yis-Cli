...
use xai_yis_announcements::RemoteAnnouncement;
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `yis_build_settings`
    ...
    #[serde(default)]
    pub yis_oauth_enabled: Option<bool>,
    #[serde(default)]
    ...
    /// default (on). Sits below env `YIS_FOLDER_TRUST`, user
    ...
    /// remote settings `yis_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `yis_build_settings` remote settings key) to enable it. See
    ...
    /// `xai_yis_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `YIS_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `yis_build_settings`).
    ...
    /// ghost text), from the `yis_build_settings` remote settings flag. Sits below
    /// env (`YIS_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `yis-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// Overridable locally via `YIS_VOICE_MODE`. Free-tier SuperGrok upsell
    ...
    /// User's subscription tier from remote settings `yis_build_access_gate`.
    ...
    /// Whether the user is allowed to use Yis Cli. Set by remote settings
    /// `yis_build_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `yis_build_usage_redirect_url`
    ...
    /// `yis_build_settings`. Per-model override on `ModelInfo`
    /// (`yis_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`YIS_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`YIS_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`YIS_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`YIS_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `yis workspace` CLI command (Computer Hub
    /// workspace exposure), from `yis_build_settings.workspace_command_enabled`.
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "yis-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("yis-build"));
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "yis-build-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("yis-build-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
