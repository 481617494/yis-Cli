//! Memory-system configuration value types, extracted from xai-yis-shell
...
//! `xai-yis-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.yis/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.yis/memory/` are scanned:
