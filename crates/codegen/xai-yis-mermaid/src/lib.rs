...
//!
//! ```
//! use xai_yis_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `YisDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `YisNight`, `TokyoNight`).
    Dark,
}
