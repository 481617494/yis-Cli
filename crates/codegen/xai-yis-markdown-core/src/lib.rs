//! Headless markdown analysis sharing Yis Cli's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-yis-markdown` so analysis matches what Grok
...
/// The exact `pulldown-cmark` option set Yis Cli uses to render markdown.
///
...
/// Parse `text` with Yis Cli's options; count elements and flag structural issues.
