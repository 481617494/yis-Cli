...
//!
//! Extracted from `xai-yis-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`yis-shell-` /
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("yis-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("yis-shell-turn"), "turn");
        assert_eq!(
            event_value("yis-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("yis-shell-workspace-x"), "workspace-x");
    }
    ...
    /// (e.g. a future `"yis-shell-ext-"` origin), an earlier `ALL` entry could
