...
//! file logger that writes to `~/.yis/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `xai_yis_hooks` or `xai_yis_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! YIS_HOOKS_LOG=1 grok              # enable, write to ~/.yis/logs/hooks.log
//! YIS_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! YIS_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.yis/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "YIS_HOOKS_LOG";
...
///
/// Writes to `~/.yis/logs/hooks.log` (or custom path via `YIS_HOOKS_LOG`).
/// Filters to hooks (`xai_yis_hooks`) and plugins (`xai_yis_agent::plugins`) targets.
/// Set `YIS_HOOKS_LOG=0` to disable, `YIS_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_yis_hooks=debug,xai_yis_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || yis_home().join("logs").join("hooks.log");
