...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "yis_code.session_start",
            Self::SessionEnd => "yis_code.session_end",
            Self::UserPrompt => "yis_code.user_prompt",
            Self::TurnCompleted => "yis_code.turn_completed",
            Self::ApiRequest => "yis_code.api_request",
            Self::ApiError => "yis_code.api_error",
            Self::ToolResult => "yis_code.tool_result",
            Self::ToolDecision => "yis_code.tool_decision",
            Self::McpServerConnection => "yis_code.mcp_server_connection",
            Self::PermissionModeChanged => "yis_code.permission_mode_changed",
            Self::SkillActivated => "yis_code.skill_activated",
            Self::PluginLoaded => "yis_code.plugin_loaded",
            Self::Compaction => "yis_code.compaction",
            Self::Subagent => "yis_code.subagent",
            Self::Auth => "yis_code.auth",
            Self::InternalError => "yis_code.internal_error",
            Self::ModelSwitched => "yis_code.model_switched",
            Self::ContextualTip => "yis_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `yis_code.session.count`.
    SessionCount,
    /// `yis_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "yis_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "yis_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "yis_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "yis_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "yis_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "yis_code.error.count";
...
/// Built-in tool names (closed enum surface in `xai-yis-tools`) that pass
...
/// `SessionHarness` → `yis_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `yis_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `yis_code.session_end`.
...
/// `PromptSubmitted` → `yis_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `yis_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `yis_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `yis_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `yis_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `yis_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `yis_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `yis_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `yis_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `yis_code.permission_mode_changed`.
...
/// `ContextualTip` → `yis_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `yis_code.permission_mode_changed`.
...
/// `SkillDispatched` → `yis_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `yis_code.plugin_loaded`.
...
/// `PluginUsed` → `yis_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `yis_code.compaction`.
...
/// `CompactionTriggered` → `yis_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `yis_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `yis_code.subagent` (`phase=completed`).
...
/// `InternalError` → `yis_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `yis_code.model_switched`.
