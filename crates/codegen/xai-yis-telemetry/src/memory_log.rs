...
//! file logger that writes to `~/.yis/logs/memory.log`.
//!
//! ## When to use
//!
...
//! YIS_MEMORY_LOG=0 grok                # disable even when enabled
//! tail -f ~/.yis/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "YIS_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.yis/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `YIS_MEMORY_LOG=0` to disable, `YIS_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || yis_home().join("logs").join("memory.log");
