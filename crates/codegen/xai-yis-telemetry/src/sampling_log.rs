//! Tracing layer for `target: "sampling_log"` → `~/.yis/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `YIS_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "YIS_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = yis_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
