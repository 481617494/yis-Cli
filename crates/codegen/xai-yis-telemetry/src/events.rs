...
//!
//! These structs were extracted from `xai-yis-shell` so they can be
...
// ---------------------------------------------------------------------------
// Pager events (called from xai-yis-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Yis Cli is launched from a non-project directory).
...
/// attributed to Yis Cli: the user saw the gate in Yis Cli, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Yis Cli's upsell surface.
...
/// `yis-shell-manual_auth` (the `manual_auth` binding gets the `yis-shell-`
