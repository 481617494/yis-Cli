...
//!
//! Extracted from `xai-yis-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-yis-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-yis-workspace` (remote sampler / workspace server).
    Workspace,
}
...
/// event-name prefix (`yis-shell-*` vs `grok-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "yis-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "grok-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("yis-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("yis-shell-{suffix}");
