...
//!
//! Extracted from `xai-yis-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$YIS_HOME` is resolved through `xai-yis-config::yis_home`.
...
/// This function caches the result in `$YIS_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = xai_yis_config::yis_home().join("agent_id");
...
///
/// Enabled only when `YIS_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("YIS_TELEMETRY_SPECIAL_USER").as_deref(),
