...
//! `yis-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use xai_yis_telemetry::client;
use xai_yis_telemetry::config::{TelemetryConfig, TelemetryMode};
...
async fn manual_auth_posts_to_events_endpoint_as_grok_shell_manual_auth() {
    ...
    xai_yis_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "yis-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no yis-shell-manual_auth POST received"
        );
