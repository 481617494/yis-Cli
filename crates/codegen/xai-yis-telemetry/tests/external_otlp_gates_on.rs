...
use xai_yis_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    xai_yis_telemetry::log_event(xai_yis_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "yis_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.yis_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("yis-cli"),
        "service.name=yis-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("yis_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "yis_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "yis_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "yis_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "yis_code.session.count");
