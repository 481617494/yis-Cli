...
fn external_stream_end_to_end() {
    ...
    let mut cfg = xai_yis_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "YIS_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = xai_yis_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    xai_yis_telemetry::external::init(Some(cfg));
    assert!(xai_yis_telemetry::external::is_active());
    ...
    assert!(!xai_yis_telemetry::is_enabled());
    xai_yis_telemetry::log_event(xai_yis_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: xai_yis_telemetry::enums::PermissionMode::Ask,
    });
    xai_yis_telemetry::log_event(xai_yis_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    xai_yis_telemetry::log_event(xai_yis_telemetry::events::PromptSubmitted {
    ...
    xai_yis_telemetry::log_event(xai_yis_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("yis-cli")),
        "service.name=yis-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "yis_code.session_start",
        "yis_code.user_prompt",
        "yis_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "yis_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "yis_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "yis_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "yis_code.token.usage"));
    assert_eq!(
    ...
    xai_yis_telemetry::external::shutdown();
    assert!(
