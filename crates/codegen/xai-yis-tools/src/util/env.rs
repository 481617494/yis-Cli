...
//! heavyweight `xai-yis-tools` dependency. This module re-exports the public
...
/// Note: the CLI also uses `YIS_AGENT` as an
/// optional agent-definition selector for launching `grok` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const YIS_AGENT_ENV: &str = "YIS_AGENT";

/// Sentinel value for [`YIS_AGENT_ENV`] on agent tool terminals.
pub const YIS_AGENT_ENV_VALUE: &str = "1";

/// Force `YIS_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_yis_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(YIS_AGENT_ENV, YIS_AGENT_ENV_VALUE);
}

/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${YIS_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${YIS_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
#[cfg(test)]
mod tests {
    use super::{YIS_AGENT_ENV, YIS_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${YIS_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${YIS_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${YIS_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(YIS_AGENT_ENV, "YIS_AGENT");
        assert_eq!(YIS_AGENT_ENV_VALUE, "1");
    }
}
