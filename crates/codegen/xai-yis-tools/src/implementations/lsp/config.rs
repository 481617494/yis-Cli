//! LSP server configuration from `.yis/lsp.json`.
...
/// This is the canonical merge function — both session startup and `yis inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::yis_home::yis_home().join("lsp.json");
    let project_path = cwd.join(".yis").join("lsp.json");
...
/// Load LSP server configs from `~/.yis/lsp.json` and `<cwd>/.yis/lsp.json`.
