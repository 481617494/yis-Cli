...
///
/// `skills` is the standard layout (`.yis/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Yis Cli sessions.
...
/// name (e.g. `~/.yis/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Yis Cli.
...
/// `cwd` (exclusive). At each directory, checks for `.yis/skills/`,
...
/// visits known config roots (`.yis`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.yis` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".yis", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.yis is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.yis/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.yis/ → kept (user content).
        let yis_shell = tmp.path().join(".yis").join("skills").join("shell");
        std::fs::create_dir_all(&yis_shell).unwrap();
        std::fs::write(
            yis_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (yis_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.yis/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .yis skill in an intermediate dir.
        ...
        .unwrap();
        let yis_skill = sub.join(".yis").join("skills").join("grok-dyn");
        std::fs::create_dir_all(&yis_skill).unwrap();
        std::fs::write(yis_skill.join("SKILL.md"), "---\nname: grok-dyn\n---\n").unwrap();
