...
//! (in `xai-yis-tools`) and the subagent coordinator (in `xai-yis-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `xai-yis-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "YisBuild:run_terminal_cmd" | "YisBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `xai-yis-shell` relies on
...
register_resource!("yis_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("yis_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// xai-yis-shell produces it, the channel protocol carries it, and the
...
register_resource!("yis_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("yis_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("yis_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by xai-yis-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-yis-shell at turn start.
...
register_resource!("yis_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("YisBuild:run_terminal_cmd", ToolKind::Execute),
                tc("YisBuild:read_file", ToolKind::Read),
                tc("YisBuild:list_dir", ToolKind::List),
                tc("YisBuild:grep", ToolKind::Search),
                tc("YisBuild:kill_task", ToolKind::KillTaskAction),
                tc("YisBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("YisBuild:kill_task", ToolKind::KillTaskAction),
                tc("YisBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("YisBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("YisBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
