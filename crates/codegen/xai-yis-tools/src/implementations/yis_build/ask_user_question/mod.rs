...
/// params are injected, `YIS_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `YIS_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "YIS_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("yis_build", "AskUserQuestion", AskUserQuestionParams);
...
/// session-owned coordinator (in xai-yis-shell), which performs an ACP
