...
//!
//! - **`xai-yis-tools`** — tool blocks on a oneshot, formats the result.
//! - **`xai-yis-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`xai-yis-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `xai-yis-tools`.
...
register_resource!("yis_build", "UserQuestionSender", UserQuestionSender);
