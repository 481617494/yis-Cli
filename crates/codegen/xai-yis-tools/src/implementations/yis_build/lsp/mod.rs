...
//! `LspTool` (Tool trait impl) under the `YisBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    xai_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.yis/lsp.json or <cwd>/.yis/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
