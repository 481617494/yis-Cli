...
//!
//! `MemoryBackend` is defined in `xai-yis-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-yis-tools`.
/// Mirrors `xai_yis_shell::session::memory_log::TARGET`.
