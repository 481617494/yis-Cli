...
//!
//! Two consumers read it independently. The hook matcher (`xai-yis-hooks`) needs the
//! Grok tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-yis-agent`) needs the [`ToolKind`] a `tools:`
...
/// One Claude tool's correspondence to Yis, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Yis [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Yis tool names this Claude tool maps to (empty when there is no direct
    /// Yis tool — the entry then only contributes a `kind`).
    yis: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, yis: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        yis,
...
const fn match_only(claude: &'static str, yis: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        yis,
...
/// The Yis tool names a Claude matcher term fires on.
pub fn yis_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.yis)
...
/// The Claude names that map to `yis_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(yis_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.yis.contains(&yis_name))
        .map(|t| t.claude)
}

/// Every distinct Yis name the table references, for the `xai-yis-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn yis_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.yis.iter().copied())
