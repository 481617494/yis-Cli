...
//!
//! `ToolSearchIndex` is defined in `xai-yis-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-yis-shell`
...
/// in `xai-yis-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-yis-shell` after MCP initialization.
#[derive(Clone)]
