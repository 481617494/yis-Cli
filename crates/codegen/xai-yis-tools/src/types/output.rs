...
use crate::implementations::yis_build::todo::{TodoItem, TodoState};
...
/// protocol boundary in `xai-yis-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the yis_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the yis_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when YisBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::yis_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::yis_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::yis_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::yis_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::yis_build::update_goal::UpdateGoalOutput),
...
mod tests {
    use super::*;
    use crate::implementations::yis_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
