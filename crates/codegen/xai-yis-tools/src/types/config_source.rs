...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.yis/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.yis/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.yis/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.yis/ or ~/.claude/.
    User { path: PathBuf },
