...
//! after every tool call to check for SKILL.md files in `.yis/skills/`,
...
/// Directories that contain skill definitions (`.yis/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".yis", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.yis/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.yis/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.yis/) are outside the git root so the walk breaks early.
