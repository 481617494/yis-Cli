...
//! config helper `xai-yis-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `YIS_TOOLS_FIND_BFS` / `YIS_TOOLS_GREP_UGREP` (+
//! `YIS_FIND_BFS` / `YIS_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.yis/vendor/<name>-<ver>-<target>`) →
//! `~/.yis/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__yis_shadow_{name}` marker) or a
...
// Binaries embedded by build.rs when `YIS_TOOLS_BUNDLE_{BFS,UGREP}_PATH` is set
// (release pipeline). Self-extracted to `~/.yis/vendor` on first use, mirroring
// the ripgrep bundling in `yis_build::grep::ripgrep`.
#[cfg(bundle_bfs)]
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("YIS_TOOLS_BFS_VER"),
    "-",
    env!("YIS_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("YIS_TOOLS_UGREP_VER"),
    "-",
    env!("YIS_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
/// OS binary again. Gated on the `__yis_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__yis_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __yis_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "YIS_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "YIS_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.yis/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::yis_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::yis_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.yis/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__yis_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        assert!(fn_body.contains("__yis_shadow_find=1"));
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__yis_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __yis_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__yis_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__yis_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__yis_shadow_find=1"));
    }
    ...
    /// `YIS_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.yis/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::yis_home().join("vendor");
    ...
    /// mode-0644 `YIS_TOOLS_*_PATH` / vendor copy) must fall through to the OS
