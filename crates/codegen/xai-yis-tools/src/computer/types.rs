...
/// (`xai-yis-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-yis-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-yis-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-yis-shell, calls ACP protocol)
#[async_trait::async_trait]
