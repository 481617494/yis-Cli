...
//!
//! These tests touch global process state — `YIS_HOME` (a `OnceLock` in
//! `xai-yis-config`), `YIS_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `YIS_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the YIS_HOME isolation used in other
//! integration tests.
...
use xai_yis_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_yis_home() {
    let _ = test_home();
    reset();
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under YIS_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every yis launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_yis_version env override
//
// The function honors `YIS_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_yis_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("YIS_TEST_VERSION", "9.9.9");
    }
    let v = xai_yis_update::version::get_installed_yis_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("YIS_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        xai_yis_update::version::get_installed_yis_version()
    };

    unsafe {
        std::env::set_var("YIS_TEST_VERSION", "0.0.0-test");
    }
    let overridden = xai_yis_update::version::get_installed_yis_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("YIS_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = xai_yis_update::version::get_installed_yis_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("YIS_TEST_VERSION", "not-a-version");
    }
    let v = xai_yis_update::version::get_installed_yis_version();
