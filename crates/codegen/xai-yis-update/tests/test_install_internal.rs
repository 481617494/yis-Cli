...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `YIS_HOME`
...
//!
//! The function reads `yis_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `YIS_HOME` and run serially via
//! `#[serial]`.
...
use xai_yis_update::UpdateConfig;
use xai_yis_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let symlink = home.join("bin").join("yis");
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("yis")).unwrap();
...
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_yis_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let yis_target = std::fs::read_link(bin_dir.join("yis")).unwrap();
    assert_eq!(
        yis_target.file_name().unwrap(),
...
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_yis_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("yis").exists() && !bin_dir.join("yis").is_symlink(),
    ...
    let yis_path = bin_dir.join("yis");
    assert!(
        !yis_path.is_symlink() && !yis_path.exists(),
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate yis-pager binary. Verify the
    // update removes the stale symlink from ~/.yis/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("yis-pager");
    ...
    assert!(
        ...
        "stale yis-pager symlink should be removed"
    );
}
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_yis_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    let target = std::fs::read_link(home.join("bin").join("yis")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("yis")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_yis_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
