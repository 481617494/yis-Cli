...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `yis update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `yis update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
use xai_yis_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use xai_yis_update::version::installed_on_disk_version;

/// Assert the active `~/.yis/bin/grok` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("yis");
    assert!(link.is_symlink(), "yis must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active yis symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("yis-{version}-{platform}"),
        "active yis must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active yis must pass the smoke-test");
}

/// Lay down a managed-install layout in the test YIS_HOME:
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `yis update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.yis/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.yis/downloads cleanup), the probe must report None — otherwise
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("yis-{version}-{platform}"));
        assert_eq!(
