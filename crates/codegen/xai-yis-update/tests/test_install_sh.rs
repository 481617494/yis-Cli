...
//!
//! The installer lives in the sibling `xai-yis-pager` crate; it is resolved by
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../xai-yis-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> yis installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".yis").join("downloads");
    let bin = home.join(".yis").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("yis-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/yis-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/grok` from disk and re-run it: the active yis must
/// always execute, and never be a `.tmp`/partial file.
fn assert_active_yis_runs(home: &Path) {
    let link = home.join(".yis").join("bin").join("yis");
    ...
    assert!(
        !name.contains(".tmp"),
        "active yis must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active yis must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("YIS_BIN_DIR", home.join(".yis").join("bin"))
        .env("YIS_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one yis installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_yis_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_yis_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // rejection).
        assert_active_yis_runs(home.path());
    }
}
