...
//!
//! > `~/.yis/bin/grok` resolves to a binary that passes the smoke-test, OR it
...
use xai_yis_update::auto_update::install_internal_from_base;
...
/// `cargo nextest run -p xai-yis-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("YIS_BLITZ_ITERS")
        .ok()
