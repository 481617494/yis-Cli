...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_yis_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use xai_yis_shell::util::config;
use xai_yis_shell::util::yis_home::{yis_application, yis_home};
...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `yis update` to get the latest version.";
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Yis Cli is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Yis Cli is available.");
        }
        return Ok(());
    }
    ...
    println!("Yis Cli - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_yis_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_yis_version();
...
/// background download, explicit `yis update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_yis_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_yis_version();
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("YIS_INSTALLER") {
    ...
    if std::env::var_os("YIS_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("YIS_MANAGED_BY_INTERNAL").is_some() {
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `yis update` child, `Some` only when a
...
/// (spawns `yis update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `yis update` child resolves
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        ...
        UpdateRunMode::NonBlocking => {
            ...
            xai_yis_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Prefer `~/.yis/bin/grok` which always points to the latest version.
fn resolve_restart_exe() -> Result<std::path::PathBuf> {
    let canonical = yis_application();
    if canonical.exists() {
        return Ok(canonical);
    }
...
pub fn restart_grok() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "YIS_AUTO_UPDATE"));
...
/// Delete `~/.yis/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = yis_home().join("models_cache.json");
...
/// Remove the stale `yis-pager` symlink/binary from `~/.yis/bin/` left by
...
/// config and writes to `~/.yis/bin/`, so callers must isolate
/// `YIS_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.yis/downloads/`, not yet
...
async fn download_verified_from_base(
    ...
    let yis_home = yis_home();
    let download_dir = yis_home.join("downloads");
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let yis_home = yis_home();
    let download_dir = yis_home.join("downloads");
    let bin_dir = yis_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.yis/bin/{grok,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "yis", &download.version).await;
    cleanup_old_downloads(&download_dir, "yis-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &yis_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, yis_home: &std::path::Path) {
    // Derive $HOME independently — yis_home may be overridden via YIS_HOME
    // env var, so yis_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", yis_home.join("completions/bash/grok.bash")),
        ("zsh", yis_home.join("completions/zsh/_grok")),
...
///
/// When both paths share a grandparent (e.g. `~/.yis/bin/grok` and
/// `~/.yis/downloads/grok-0.1.203-linux-x86_64`), returns a relative path
...
///
/// Relative symlinks survive Docker bind-mounts where `~/.yis/` is mapped
...
/// Swap `~/.yis/bin/{grok,agent}` to point at `binary_path`. Returns the
...
/// maintain in lockstep, and so must the updater — otherwise `yis update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.yis/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let yis_name = if cfg!(windows) { "yis.exe" } else { "yis" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let yis_link = bin_dir.join(yis_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [yis_link.clone(), agent_link];
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"yis"` or `"yis-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `grok-*` matching `yis-pager-*` or `grok-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // binary (avoids `grok-latest`, `yis-pager-*` when prefix is `grok`).
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    xai_yis_tools::util::detach_command(&mut cmd);
    cmd.envs(xai_yis_tools::util::pager_env());
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // through it (e.g. /usr/local/bin/grok -> ~/.yis/downloads/grok-latest)
    ...
    // ~/.yis/downloads/ (legacy layout — skips the grok-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["yis", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".yis/downloads/") && !target_str.ends_with("grok-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "yis", &version).await;
    cleanup_old_downloads(&download_dir, "yis-pager", &version).await;
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.yis/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_yis_processes_running() {
    ...
    cmd.args(["-f", "yis"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    xai_yis_tools::util::detach_std_command(&mut cmd);
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_yis_processes_running();
...
/// Run the `yis update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if !force && std::env::var_os("YIS_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Grok.");
    }
...
async fn refresh_deployment_config() {
    if !xai_yis_shell::managed_config::has_principal() {
        return;
    }
    if !xai_yis_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    xai_yis_shell::managed_config::clear_orphan();
    if !xai_yis_shell::config::is_managed_config_stale_for(
        &xai_yis_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match xai_yis_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `yis setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `yis setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        let dest_181 = std::path::Path::new("/home/u/.yis/downloads/grok-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.yis/downloads/grok-0.1.182-linux-x86_64");
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.yis/downloads").into(),
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        ...
        let target = std::path::Path::new("/home/alice/.yis/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.yis/bin/grok");
    ...
    fn test_relative_symlink_target_same_dir() {
        ...
        let target = std::path::Path::new("/home/alice/.yis/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.yis/downloads/grok-latest");
        let result = relative_symlink_target(target, link);
    ...
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/grok -> /home/alice/.yis/downloads/grok-0.1.203
        ...
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.yis/downloads/grok-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.yis/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".yis");
        ...
        let bob_link = bob.join("bin").join("yis");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        cleanup_old_downloads(d, "yis", "0.1.145").await;
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // grok and yis-pager should not interfere with each other.
        ...
        std::fs::write(d.join("yis-pager-0.1.140-macos-aarch64"), "old-pager").unwrap();
        std::fs::write(d.join("yis-pager-0.1.141-macos-aarch64"), "current-pager").unwrap();

        // Cleanup only grok — pager files must be untouched.
        make_all_stale(d);

        cleanup_old_downloads(d, "yis", "0.1.141").await;

        assert!(d.join("grok-0.1.141-macos-aarch64").exists());
        assert!(d.join("grok-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
        assert!(
            d.join("yis-pager-0.1.140-macos-aarch64").exists(),
            "pager untouched"
        );
        assert!(
            d.join("yis-pager-0.1.141-macos-aarch64").exists(),
            "pager untouched"
        );
    }
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "yis", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        cleanup_old_downloads(d, "yis", "0.1.14").await;
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for yis-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.148", "0.1.149", "0.1.150"] {
            std::fs::write(d.join(format!("yis-pager-{}-linux-x64", v)), v).unwrap();
        }
        std::fs::write(d.join("yis-pager-0.1.151-linux-x64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "yis-pager", "0.1.151").await;

        assert!(d.join("yis-pager-0.1.151-linux-x64").exists(), "current");
        assert!(d.join("yis-pager-0.1.150-linux-x64").exists(), "N-1 kept");
        assert!(
            !d.join("yis-pager-0.1.149-linux-x64").exists(),
            "0.1.149 deleted"
        );
        assert!(
            !d.join("yis-pager-0.1.148-linux-x64").exists(),
            "0.1.148 deleted"
        );
    }
    ...
    async fn test_cleanup_old_downloads_alpha_versions() {
        ...
        cleanup_old_downloads(d, "yis", "0.1.150-alpha.1").await;
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        cleanup_old_downloads(d, "yis", "0.1.150").await;
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        ...
        cleanup_old_downloads(d, "yis", "not-a-version").await;
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "yis", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. grok-latest, yis-pager-* when prefix is grok).
        std::fs::write(d.join("grok-latest"), "alias").unwrap();
        std::fs::write(d.join("yis-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("grok-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("grok-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        // grok-latest and yis-pager-* must be untouched.
        assert!(d.join("grok-latest").exists());
        assert!(d.join("yis-pager-0.1.141-macos-aarch64").exists());
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert_eq!(
            ...
            "Run `yis update` to get the latest version."
        );
    }
    ...
    //   1. YIS_INSTALLER (npm | internal | gh-release | gh)
    //   2. YIS_MANAGED_BY_NPM       → npm
    //   3. YIS_MANAGED_BY_INTERNAL  → internal
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `YIS_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("YIS_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit YIS_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("YIS_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("YIS_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // YIS_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("YIS_INSTALLER", "internal");
            std::env::set_var("YIS_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
