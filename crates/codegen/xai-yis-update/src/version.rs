...
use xai_yis_shell::env::YisBuildEnvironment;
...
///
/// Constructed once from `YisBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `YisBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.grok.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.yis/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (YIS_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &YisBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: xai_yis_shell::auth::YisComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = yis_home().join("version.json");
...
pub use xai_yis_version::installed as get_installed_yis_version;

/// Version of the managed grok binary currently on disk, read from the
/// `~/.yis/bin/grok` symlink target (`../downloads/grok-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `yis update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.yis/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = xai_yis_shell::util::yis_home::yis_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "yis")
    }
    #[cfg(not(unix))]
    {
        None
...
/// so unknown layouts (`grok-latest`, `yis-pager-*` when `bin_prefix` is
...
/// Read the cached stable version from `~/.yis/version.json` (sync, for display).
///
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(xai_yis_version::VERSION, &stable)
    })
}
...
/// `~/.yis/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(xai_yis_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ...
            ("yis-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
        ...
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "yis").as_deref(),
                *expected,
        ...
        // prefix but not under "yis".
        assert_eq!(
            version_from_versioned_binary_name("yis-pager-0.1.5-darwin-arm64", "yis-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    fn test_update_config_default_channel_is_stable() {
        use xai_yis_shell::env::YisBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&YisBuildEnvironment::Production);
