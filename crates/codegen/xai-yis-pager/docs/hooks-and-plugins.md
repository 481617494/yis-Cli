...
Yis Cli supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.yis/hooks/`
- **Project hooks** — from `.yis/hooks/` in your repo
...
Plugin hooks automatically receive `YIS_PLUGIN_ROOT` and `YIS_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.yis/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `yis plugin marketplace add <source>`)
...
Sources land in `~/.yis/config.toml`:
...
Or in `~/.yis/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-yis-hooks/examples/README.md) — ready-to-use sample hooks
