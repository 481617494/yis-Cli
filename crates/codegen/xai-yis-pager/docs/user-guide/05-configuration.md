...
2. **Environment variables** (e.g., `XAI_API_KEY`, `YIS_MEMORY`)
3. **config.toml** (`~/.yis/config.toml`)
...
The setting can also be overridden with the `YIS_DEFAULT_SELECTED_PERMISSION`
...
`[ui] vim_mode` in `~/.yis/config.toml` immediately and applies it to every
...
preference**: whichever mode you last chose explicitly is what a plain `yis`
...
A plain `yis` launch only reads it. A CLI flag always wins over the config
...
`YIS_SCROLL_SPEED`, `YIS_SCROLL_MODE`, `YIS_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `YIS_SCROLL_LINES`. Precedence: env var →
...
(`YIS_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`YIS_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
MCP servers can also be configured per-project in `.yis/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.yis/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.yis/config.toml` (current dir) > `<repo-root>/.yis/config.toml` > `~/.yis/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `YIS_MEMORY=1`).
...
[subagents.models]
explore = "yis-build"               # route to different models
```
...
`yis inspect` reports cells that still need session-start resolution as
...
`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.yis/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "yis"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `yis`. |
...
| Yis Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |
...
`$YIS_EVENT`, `$YIS_MESSAGE`, and `$YIS_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Yis Cli' -message '$YIS_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$YIS_MESSAGE' ntfy.sh/my-grok-alerts"
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= YIS_EXTERNAL_OTEL)
...
| `YIS_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `YIS_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `YIS_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `YIS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `YIS_OIDC_ISSUER` | OIDC issuer URL |
| `YIS_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `YIS_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `YIS_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `YIS_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `YIS_AGENT` | Custom agent definition path or name |
| `YIS_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `YIS_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `YIS_LOG_FILE` log and headless stderr output |
...
| `YIS_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `YIS_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `YIS_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `YIS_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.yis/config.toml` | Main configuration file |
| `~/.yis/pager.toml` | TUI appearance configuration |
...
| `~/.yis/lsp.json` | LSP server configuration (user-scoped) |
| `~/.yis/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.yis/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.yis/` within your repository:

| File | What it configures |
|------|--------------------|
| `.yis/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.yis/config.toml`) |
| `.yis/skills/` | Project-specific skills |
| `.yis/hooks/` | Project-specific lifecycle hooks |
| `.yis/agents/` | Project-specific agent definitions |
| `.yis/lsp.json` | LSP server configuration |
| `.yis/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.yis/lsp.json` | All projects |
| Project | `.yis/lsp.json` | Current repository |
