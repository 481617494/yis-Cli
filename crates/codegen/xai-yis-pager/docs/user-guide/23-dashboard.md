...
- **`yis dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.yis/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Yis Cli · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Yis Cli's sibling views (
`tasks_pane`):
...
Set `YIS_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `yis --bg`") would list
