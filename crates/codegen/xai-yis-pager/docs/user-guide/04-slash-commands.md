...
- **Shell builtins** -- handled by the agent backend (xai-yis-shell)
- **Pager builtins** -- handled by the TUI frontend (xai-yis-pager)
...
切换当前会话模型。不带参数时打开模型选择器（与 Ctrl+M / 命令面板「切换模型」相同）。

支持模型 ID 或显示名（不区分大小写）。对支持推理的模型，可再跟 effort：
...
choice persists to `[ui].screen_mode` in `config.toml`, so future plain `yis`
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `YIS_MEMORY=1`. `/remember` is always available.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.yis/skills/commit/SKILL.md`, you can invoke it with:
