...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.yis/sessions/`. Set `YIS_HOME` to override the base directory; when it is unset, Grok uses `~/.yis`.
...
Run `yis --resume` without an ID to resume the most recent session for the current directory.

### From the Welcome Screen

When you launch `yis`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
yis -p "Continue where we left off" -r <session-id>
...
## The yis sessions Subcommand

List or search sessions from the command line. `yis sessions` requires a subcommand:
...
yis sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
yis sessions search "rate limit"
```

`yis sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `yis sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `yis -w -r <session-id>`.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `yis sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
