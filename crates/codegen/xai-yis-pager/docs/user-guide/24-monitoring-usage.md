...
> **Status: alpha.** The schema below is versioned (`yis_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Yis Cli can export usage **metrics** and **events** to your organization's
...
```bash
export YIS_EXTERNAL_OTEL=1                  # master switch
...
`YIS_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `YIS_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `YIS_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `yis_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `YIS_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`YIS_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `yis setup` managed config /
...
## Metrics (meter scope `ai.xai.yis_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `yis_code.session.count` | `{session}` | base attrs only |
| `yis_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `yis_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `yis_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `yis_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `yis_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `yis_code.token.usage` with your own
...
|---|---|
| `yis_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `yis_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `yis_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `yis_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `yis_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `yis_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `yis_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `yis_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `yis_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `yis_code.permission_mode_changed` | `to_mode`, `trigger` |
| `yis_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `yis_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `yis_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `yis_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `yis_code.auth` | `auth_method` |
| `yis_code.internal_error` | `error_type` (class only — no message, no location) |
| `yis_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(yis_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(yis_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(yis_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(yis_code_tool_decision_total[1h]))
```
