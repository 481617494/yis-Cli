...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.yis/agents/` or `~/.yis/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.yis/personas/`; applied during subagent resolution |
...
| **Examples** | `yis-build`, `explore`, `plan` | `researcher`, `concise` |
...
Yis Cli discovers file-based personas from these locations, in priority order:

- `.yis/personas/*.toml` (project)
- `~/.yis/personas/*.toml` (user)
...
> **Note:** Yis Cli applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Yis Cli resolves the effective model and reasoning effort in this order, highest priority first:
...
Yis Cli manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "yis-build"               # route explore to a specific model
```
...
# instructions_file = ".yis/personas/concise.md"  # or load from a file
```

Yis Cli also discovers roles from `.yis/roles/*.toml` and personas from `.yis/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Yis Cli shows running and finished work in side panes on the agent screen:
