...
MCP servers are configured in `~/.yis/config.toml` under `[mcp_servers.<name>]` sections.
...
> `YIS_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `YIS_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Grok-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.yis/config.toml`) **or repo-level**
>   (`.yis/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.yis/config.toml` >
...
yis mcp list
yis mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by yis.
yis mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
yis mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
yis mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
yis mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
yis mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
yis mcp remove github

# Diagnose a server's configuration and connectivity
yis mcp doctor               # Check every configured server
yis mcp doctor github        # Check one server
yis mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `yis mcp add` writes to `~/.yis/config.toml` (`--scope user`). Use `--scope project` to write to `.yis/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `yis mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`yis mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.yis/config.toml` in your repository:
...
Grok walks from the current directory up to the git repo root, loading `.yis/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.yis/config.toml` | All projects | Lowest |
| `<repo-root>/.yis/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.yis/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.yis/config.toml`, `.yis/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.yis/config.toml` or the corresponding environment variable (`YIS_CURSOR_MCPS_ENABLED`, `YIS_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `yis inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.yis/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.yis/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.yis/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `yis inspect` to see all loaded MCP servers and their sources:

```bash
yis inspect          # Human-readable
yis inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug YIS_LOG_FILE=/tmp/yis.log yis
tail -f /tmp/yis.log
```
