...
1. **Global rules**: `~/.yis/` (applies to all projects)
...
As top-level instruction files, Grok discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.yis/rules/`, every `*.md` file is loaded regardless of name.)

---

## The .yis/ Project Directory

Beyond AGENTS.md files, the `.yis/` directory in your project root can contain additional project-level configuration:

| Path | Purpose |
|------|---------|
| `.yis/config.toml` | Project-scoped MCP servers, plugins, and permission rules (other settings load only from `~/.yis/config.toml`) |
| `.yis/skills/` | Project-scoped skill definitions |
| `.yis/plugins/` | Project-scoped plugins |
| `.yis/agents/` | Project-scoped agent definitions |
| `.yis/hooks/` | Project-scoped lifecycle hooks |
...
Use `yis inspect` to see all loaded project instructions:
...
5. **Version control your rules.** Commit AGENTS.md to the repository so the whole team benefits. User-specific overrides belong in `~/.yis/` (global rules).
