...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.yis/)
yis --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.yis/ + temp dirs)
yis --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.yis/, no child network)
yis --sandbox strict
```
...
| `workspace`           | Everywhere         | CWD + `~/.yis/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.yis/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.yis/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.yis/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.yis/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.yis/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

---

## Custom Profiles

Create custom sandbox profiles in `~/.yis/sandbox.toml` (global) or `.yis/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire yis process** at startup using kernel primitives -- not per-command wrapping. This means all tool operations are covered:
...
for the life of the session**. When you resume it (`yis --resume <id>`,
`yis --continue`, or `yis -r`), Grok restores that same profile automatically —
...
1. An explicit `--sandbox <profile>` flag or `YIS_SANDBOX` environment variable
...
Sandbox events are logged to `~/.yis/sandbox-events.jsonl` for debugging. Events include:
