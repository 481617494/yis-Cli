...
Yis Cli is a terminal-based AI coding assistant from SpaceXAI. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
```powershell
$env:YIS_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.yis\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
On first launch, Grok opens your browser to authenticate with grok.com. After you sign in, Grok stores your credentials in `~/.yis/auth.json`, where they persist across sessions. Grok refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
- Use the `--yolo` flag at launch: `yis --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.yis/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `yis -c`
...
yis --resume <session-id>

# Continue the most recent session
yis -c

# Experimental scrollback-native render mode. Sticky: plain `yis` reopens in
...
```bash
yis -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
```
~/.yis/AGENTS.md           # Global rules (apply to all projects)
