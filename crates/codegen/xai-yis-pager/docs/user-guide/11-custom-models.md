...
Yis Cli 本地安全模式 **不使用** xAI 云端默认模型。请用厂商 API（DeepSeek、百炼、智谱等）或自建 OpenAI 兼容端点。
...
```bash
yis models setup                                    # 交互向导
yis models add --preset deepseek --api-key sk-... # 非交互
...
- `/model-add` — 模型管理（选厂商 / Key / 拉列表 / 编辑删除）
- `/model` 或 `Ctrl+M` — 切换当前会话模型
- 首次会话且无已配置模型时，自动打开管理弹窗
...
Add custom model endpoints in `~/.yis/config.toml` under `[model.<name>]` sections:
...
3. Your signed-in session token (from `yis login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; Grok also accepts `YIS_CODE_XAI_API_KEY` for backward compatibility)
...
|----------|----------|-------------|
| `YIS_MODELS_BASE_URL` | Yes | Base URL for inference. Grok fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Grok also accepts `YIS_CODE_XAI_API_KEY`. |
| `YIS_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you set `models_base_url`, Grok uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `yis login` -- the API key is enough.
...
```bash
export YIS_WEB_SEARCH_MODEL="grok-4.20-multi-agent"
```
