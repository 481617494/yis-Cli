...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `YIS_PLUGIN_ROOT` and `YIS_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
...
|----------------------|-------------|
| `YIS_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `YIS_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |
...
| `--plugin-dir` (CLI flag, `yis agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.yis/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.yis/plugins/` | User -- personal plugins for every project | Trusted automatically |
...
```bash
yis plugin list [--json] [--available]   # List installed plugins (--available requires --json)
yis plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
yis plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
yis plugin update [<name>]               # Omit the name to update all plugins
yis plugin enable <name>
yis plugin disable <name>
yis plugin details <name>                # Show the plugin's component inventory
yis plugin validate [<path>]             # Validate plugin.json (default: current directory)
yis plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `yis plugin install <source>` without `--trust` and Grok prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
```bash
yis plugin marketplace list [--json]
yis plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
yis plugin marketplace remove <url>      # Git URL or local path of a configured source
yis plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
yis plugin marketplace add my-org/team-plugins
yis plugin marketplace list
yis plugin install my-org/team-plugins --trust
yis plugin list
yis plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `yis plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.yis/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `yis plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.yis/pager.toml`:
...
Place this file at `~/.yis/settings.json` or `~/.claude/settings.json`.
...
Grok trusts plugins from `~/.yis/plugins/` automatically. Project plugins in `.yis/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
yis plugin install <source> --trust
```
...
Run `yis inspect` to see every discovered plugin and what it provides:

```bash
yis inspect          # Show plugins with their skills, agents, hooks, and MCP servers
yis inspect --json   # Emit machine-readable JSON
```
