...
By default, each `yis -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
yis -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
yis -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
yis -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
yis -p "What's the secret number?" --resume abc123
```
...
yis -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
yis --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  yis -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
RESULT=$(yis -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
yis -p "Format all files" --yolo

# Run tests and fix failures
yis -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| `YIS_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
...
- **`yis login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`yis login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `yis -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug yis -p "..." 2> debug.log`.
...
Grok stores data in `~/.yis` (override with `YIS_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.yis` read-only:
...
```bash
export XAI_API_KEY="xai-..."
export YIS_DISABLE_AUTOUPDATER=1
yis -p "..." --no-auto-update
```
...
| `YIS_DISABLE_AUTOUPDATER=1`    | Process   |
...
- Resume: `yis -p "continue" --resume "<id>"` or `yis -p "continue" --continue`
