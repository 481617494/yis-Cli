...
//!
//! Business logic lives in `xai_yis_shell::plugin` (shared orchestration)
//! and lower crates (`xai-yis-agent`, `xai-yis-plugin-marketplace`). This
...
use xai_yis_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use xai_yis_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use xai_yis_plugin_marketplace::SourceKind;
use xai_yis_plugin_marketplace::git::SourceCacheLease;
use xai_yis_shell::plugin::{self, RepoUpdateOutcome, UninstallError};
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `yis plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &xai_yis_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = xai_yis_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = xai_yis_plugin_marketplace::load_sources(&config);
    sources.extend(xai_yis_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in xai_yis_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                xai_yis_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &xai_yis_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = xai_yis_plugin_marketplace::git::default_cache_root();
            xai_yis_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> xai_yis_telemetry::events::InstallKind {
    if is_git {
        xai_yis_telemetry::events::InstallKind::Git
    } else {
        xai_yis_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: xai_yis_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    xai_yis_telemetry::session_ctx::log_event(xai_yis_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = xai_yis_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use xai_yis_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &xai_yis_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `yis plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            xai_yis_telemetry::session_ctx::log_event(
                xai_yis_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `yis plugin list` to see installed plugins."
        );
    }
    if let Err(e) = xai_yis_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    xai_yis_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = xai_yis_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    xai_yis_shell::config::add_disabled_plugin(name)
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `yis plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use xai_yis_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        xai_yis_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &xai_yis_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match xai_yis_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use xai_yis_plugin_marketplace::MarketplaceSource;
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = xai_yis_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = xai_yis_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
