//! xai-yis-pager — Yis Cli TUI（基于 Yis Cli 引擎的二次发行）。
//!
...
// minimal rendering lives in the sibling `xai-yis-pager-minimal` crate; these
...
pub mod models;
/// Interactive / non-interactive model setup (qoder-switch style BYOK).
pub mod model_setup_cmd;
...
// `xai-yis-pager-render`. Re-exported at the crate root so existing
// `crate::<module>::...` references throughout the pager keep resolving.
pub use xai_yis_pager_render::{
