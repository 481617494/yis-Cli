...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("YIS_EVENT", event_str)
        .env("YIS_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("YIS_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn test_event() -> NotificationEvent {
        NotificationEvent {
            ...
            title: crate::i18n::PRODUCT_NAME.into(),
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'YIS_EVENT=%s\\nYIS_MESSAGE=%s\\nYIS_SESSION_ID=%s\\n' \
             \"$YIS_EVENT\" \"$YIS_MESSAGE\" \"$YIS_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("YIS_EVENT=Turn complete"),
            "missing YIS_EVENT: {content}"
        );
        assert!(
            content.contains("YIS_MESSAGE=hello world"),
            "missing YIS_MESSAGE: {content}"
        );
        assert!(
            content.contains("YIS_SESSION_ID=sess-42"),
            "missing YIS_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("YIS_SESSION_ID"),
            "YIS_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("YIS_EVENT=Turn complete"));
        assert!(content.contains("YIS_MESSAGE=test body payload"));
        assert!(content.contains("YIS_SESSION_ID=test-session-123"));
    }
}
