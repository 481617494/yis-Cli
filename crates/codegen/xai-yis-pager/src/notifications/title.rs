...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("yis");
        self.last_title.clear();
        self.last_title.push_str("yis");
...
mod tests {
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("yis"), "title missing 'yis': {t}");
