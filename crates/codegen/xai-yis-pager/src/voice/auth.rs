...
use xai_yis_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<xai_yis_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        xai_yis_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
