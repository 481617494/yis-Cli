...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: xai_yis_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`yis --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: xai_yis_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    /// Open Yis model manager (qoder-switch style add/edit/delete/apply).
    OpenModelManager,
    ...
    AnnouncementsOpenCta(xai_yis_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"yisnight"`, `"auto"`).
    SetTheme(String),
    ...
    OpenCommandPalette,
    /// Open the model picker (`/model` with no args, palette "切换模型",
    /// `ActionId::ModelPicker` / Ctrl+M off-prompt). Toggles closed if already open.
    OpenModelPicker,
...
///
/// Both variants write to `~/.yis/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: xai_yis_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.yis/config.toml`.
    ...
    /// Persist a typed setting to `~/.yis/config.toml`. On failure,
    ...
    /// registry (`~/.yis/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`yis-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<xai_yis_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<xai_yis_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<xai_yis_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<xai_yis_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<xai_yis_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<xai_yis_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<xai_yis_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    ...
    GateRefreshed {
        settings: Option<xai_yis_shell::util::config::RemoteSettings>,
    },
