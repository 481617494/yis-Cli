...
/// [`xai_yis_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<xai_yis_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: xai_yis_shell::util::config::WorktreeHintMode) -> Self {
        use xai_yis_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        xai_yis_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            xai_yis_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`xai_yis_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `YIS_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `YIS_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("YIS_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`xai_yis_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => xai_yis_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: xai_yis_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`xai_yis_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.yis/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`YIS_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `YIS_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<xai_yis_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<xai_yis_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `YIS_PLUGIN_CTA`
    ...
    /// non-selectable headers. Gated by `YIS_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `YIS_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `xai-yis-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<xai_yis_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        xai_yis_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `xai_yis_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `YIS_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: xai_yis_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<xai_yis_shell::util::config::ContextualHintsRemote>,
    ...
    /// `yis_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, xai_yis_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `yis_build_access_gate`. `Some` = blocked.
    pub gate: Option<xai_yis_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<xai_yis_shell::auth::GateInfo>,
    ...
    /// `~/.yis/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `YIS_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &xai_yis_shell::util::config::RemoteSettings,
    ) -> Option<xai_yis_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(xai_yis_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &xai_yis_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            xai_yis_telemetry::session_ctx::log_event(
                xai_yis_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: xai_yis_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: xai_yis_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: xai_yis_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<xai_yis_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && xai_yis_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: xai_yis_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(xai_yis_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(xai_yis_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `YIS_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<xai_yis_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_yis_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        xai_yis_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                xai_yis_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            xai_yis_telemetry::session_ctx::log_event(
                                xai_yis_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        xai_yis_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`xai_yis_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use xai_yis_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = xai_yis_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                xai_yis_telemetry::session_ctx::log_event(
                    xai_yis_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: xai_yis_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            xai_yis_telemetry::session_ctx::log_event(xai_yis_telemetry::events::ContextualTip {
                tip: xai_yis_telemetry::events::ContextualTipKind::ImageInput,
                action: xai_yis_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![xai_yis_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![xai_yis_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: xai_yis_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = xai_yis_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = xai_yis_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&xai_yis_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&xai_yis_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(xai_yis_shell::auth::GateInfo {
            message: "Subscribe to use Yis Cli".into(),
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = xai_yis_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(xai_yis_workspace::foreign_sessions::RecentForeignSession {
                tool: xai_yis_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![xai_yis_workspace::file_system::FuzzyMatchResult {
