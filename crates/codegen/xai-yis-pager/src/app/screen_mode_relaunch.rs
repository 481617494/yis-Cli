...
pub(crate) const YIS_SCREEN_MODE_ENV: &str = "YIS_SCREEN_MODE";
...
///
/// Includes [`YIS_SCREEN_MODE_ENV`] so the resume hits the same override path
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{YIS_SCREEN_MODE_ENV}={mode} grok {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(YIS_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`YIS_SCREEN_MODE_ENV`] value (pure; unit-tested directly).
///
/// | Value | Mode |
/// |---|---|
...
/// Consume the one-shot screen-mode override env (see [`YIS_SCREEN_MODE_ENV`]).
///
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(YIS_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(YIS_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["yis", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["yis", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn strips_positional_prompt() {
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "--no-leader", "fix the bug"]),
            "sid",
            true,
        );
        assert_eq!(
    ...
    fn keeps_equals_form_and_short_model_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["yis", "-m", "grok-4", "--cwd=/tmp/proj", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry YIS_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "YIS_SCREEN_MODE=fullscreen grok --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "YIS_SCREEN_MODE=minimal grok --minimal --resume abc-sid"
        );
    }
    ...
    fn preference_config_screen_mode_beats_legacy_pager_toml() {
        ...
        // "full" alias accepted (same vocabulary as YIS_SCREEN_MODE).
        assert_eq!(
