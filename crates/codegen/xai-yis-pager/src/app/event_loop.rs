...
/// [`decide`](xai_yis_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<xai_yis_update::auto_update::UpdateAvailable>>,
    >,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("YIS_LOG_SAMPLING", "1") };
    }
    ...
    let launch_auto = xai_yis_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = xai_yis_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(xai_yis_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = xai_yis_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match xai_yis_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = xai_yis_config::env_bool("YIS_PLUGIN_CTA")
    ...
    // switch; `YIS_VOICE_MODE` overrides for local dev (env > remote > default on).
    ...
    let voice_mode_enabled = crate::app::resolve_voice_mode_enabled(
        xai_yis_config::env_bool("YIS_VOICE_MODE"),
        remote_settings.as_ref().and_then(|s| s.voice_mode_enabled),
    );
    app.apply_voice_mode_enabled(voice_mode_enabled);
    app.session_picker_grouped = std::env::var("YIS_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            xai_yis_shell::config::load_effective_config()
                .ok()
    ...
    //
    // Yis local mode (default): never auto-start browser OIDC
    // ("A browser window will open for authentication…"). Users configure
    // their own API keys via the model manager; xAI account login is optional
    // and only available via explicit `/login` or `--force-login`.
    let force_login = args.force_login && !connection.auth_methods.is_empty();
    let yis_skip_browser_login = crate::i18n::local_mode() && !args.force_login;
    let needs_interactive_login = if yis_skip_browser_login {
        false
    } else {
        connection.needs_login || force_login
    };

    if yis_skip_browser_login {
        tracing::info!("Yis local mode: skipping interactive browser authentication");
        app.auth_state = super::app_view::AuthState::Done;
        app.welcome_prompt_focused = true;
        // Treat as key-style auth so usage gate / voice behave offline-friendly.
        if connection.auth_meta.is_none() {
        ...
        if connection.needs_login {
            ...
            let yis_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "grok.com");
            if let Some(method) = yis_com {
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    xai_yis_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
        ...
        // Local mode / already authenticated: keep AuthState::Done, no browser.
        vec![]
    };

    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<xai_yis_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == xai_yis_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = xai_yis_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = xai_yis_shell::config::load_merged_requirements();
    let user_config = xai_yis_shell::config::load_from_disk().ok();
    let managed_config = xai_yis_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match xai_yis_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = xai_yis_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = xai_yis_voice::VoiceConfig::from_config_table(table);
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to yis-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = xai_yis_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        xai_yis_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        xai_yis_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        xai_yis_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use xai_yis_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = xai_yis_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let yis_home = xai_yis_tools::util::yis_home::yis_home();
            app.tip = xai_yis_shell::util::tips::pick_and_advance(&app.tips, &yis_home);
        }
    }

    let hints = xai_yis_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.yis/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        xai_yis_shell::util::config::clamped_display_permission_mode(
            xai_yis_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        xai_yis_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = xai_yis_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = xai_yis_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // YIS_SCROLL_SPEED/_MODE/_LINES + YIS_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<xai_yis_voice::VoiceEvent>>;
    ...
    // CLI subcommand wrote a `YIS_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("YIS_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(xai_yis_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.yis/config.toml [ui]`
                // rather than `~/.yis/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = xai_yis_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> xai_yis_shell::agent::config::UiConfig {
    use xai_yis_shell::agent::config::UiConfig;
    let Ok(root) = xai_yis_shell::config::load_effective_config() else {
