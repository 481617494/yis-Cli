...
use super::*;
use xai_yis_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use xai_yis_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use xai_yis_workspace::session::git::RestoreDegree;
...
/// Redirect `YIS_HOME` to a tempdir for test isolation.
fn setup_yis_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("YIS_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xai_yis_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        xai_yis_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        xai_yis_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_yis_home_in_tempdir();
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xai_yis_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            yis_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// Neutralize `YIS_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(YIS_AGENT)]` (process-global env).
fn without_grok_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("YIS_AGENT", "")
}
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> xai_yis_shell::session::SessionInfoResponse {
    use xai_yis_shell::session::acp_types::{ContextInfo, SessionInfoData};
    xai_yis_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("yis-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
