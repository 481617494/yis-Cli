...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + yis_home OnceLock in the shared lib test binary); run: cargo test -p xai-yis-pager --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<yis_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_yis_home().join("sessions");
