...
use xai_yis_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, yis_home: &Path) -> [PathBuf; 2] {
        ...
        [
            yis_home
                ...
                .join(skill)
                .join("SKILL.md"),
            yis_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(yis_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, yis_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, yis_home, metadata_exists).await;
...
mod tests {
    ...
    use xai_yis_workspace::foreign_sessions::ForeignSessionSource;
