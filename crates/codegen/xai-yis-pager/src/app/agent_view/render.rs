...
impl AgentView {
    ...
    pub fn draw(
        ...
        if let Some(profile) = xai_yis_sandbox::profile_name() {
        ...
        if let Some(ref mut modal_state) = self.model_manager {
            let overlay_area = Rect {
                x: area.x,
                y: area.y,
                width: area.width,
            ...
            let theme = Theme::current();
            crate::views::model_manager::render_model_manager(buf, overlay_area, modal_state, &theme);
            self.pane_areas = layout.pane_areas();
            return (None, crate::terminal::overlay::clear().map(Into::into));
        }
