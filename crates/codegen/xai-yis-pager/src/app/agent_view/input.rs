    ...
    fn handle_input_inner(
        ...
        if self.model_manager.is_some() {
            return match ev {
                Event::Key(key) if key.kind != KeyEventKind::Release => {
                    ...
                    self.handle_model_manager_key(key)
                }
                Event::Paste(text) => self.handle_model_manager_paste(text),
                _ => InputOutcome::Changed,
            };
        }
            ...
            && registry.lookup(key, When::AgentScreen) == Some(ActionId::OpenExtensions)
        {
            return InputOutcome::Action(Action::OpenExtensionsModal {
                ...
                trigger: xai_yis_telemetry::events::ExtensionsModalTrigger::KeyboardShortcut,
            });
        }
        if let Event::Key(key) = ev
            && key.kind != KeyEventKind::Release
    ...
    pub(super) fn handle_agent_action(&mut self, action_id: ActionId) -> InputOutcome {
        match action_id {
            ...
            ActionId::ModelPicker => {
                // 与 `/model`、命令面板共用 `Action::OpenModelPicker`。
