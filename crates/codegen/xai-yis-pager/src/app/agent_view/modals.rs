...
impl AgentView {
    // -- Model manager (qoder-switch style) --

    pub(super) fn handle_model_manager_key(
        &mut self,
        key: &crossterm::event::KeyEvent,
    ) -> InputOutcome {
        let outcome = {
            let Some(ref mut state) = self.model_manager else {
                return InputOutcome::Unchanged;
            };
            crate::views::model_manager::handle_key(state, key)
        };
        self.apply_model_manager_outcome(outcome)
    }

    pub(super) fn handle_model_manager_paste(&mut self, text: &str) -> InputOutcome {
        let outcome = {
            let Some(ref mut state) = self.model_manager else {
                return InputOutcome::Unchanged;
            };
            crate::views::model_manager::handle_paste(state, text)
        };
        self.apply_model_manager_outcome(outcome)
    }

    fn apply_model_manager_outcome(
        &mut self,
        outcome: crate::views::model_manager::ModelManagerOutcome,
    ) -> InputOutcome {
        match outcome {
            crate::views::model_manager::ModelManagerOutcome::Close => {
                self.model_manager = None;
                InputOutcome::Changed
            }
            crate::views::model_manager::ModelManagerOutcome::Changed => InputOutcome::Changed,
            crate::views::model_manager::ModelManagerOutcome::Unchanged => InputOutcome::Unchanged,
            crate::views::model_manager::ModelManagerOutcome::Applied { name, .. } => {
                // config.toml written; watcher / next /model should pick it up.
                // Prompt a soft restart only if catalog is still empty.
                self.show_toast(&if crate::i18n::is_zh() {
                    format!("已启用 {name} · 已写入 ~/.yis/config.toml（可用 /model 切换）")
                } else {
                    format!("Enabled {name} · wrote ~/.yis/config.toml (use /model to switch)")
                });
                InputOutcome::Changed
            }
        }
    ...
    fn log_extensions_modal_action(
        &self,
        action: &str,
        input_method: xai_yis_telemetry::events::ExtensionsInputMethod,
    ) {
    ...
    fn log_extensions_modal_action_with(
        ...
        if let Some(ref state) = self.extensions_modal {
            xai_yis_telemetry::session_ctx::log_event(
                xai_yis_telemetry::events::ExtensionsModalAction {
    ...
    pub(super) fn handle_extensions_modal_key(
            ...
            })
        {
            self.log_extensions_modal_action(
                "open_connectors",
                xai_yis_telemetry::events::ExtensionsInputMethod::Keyboard,
            );
            return self.execute_modal_button_action(
        ...
        if let Some(ch) = chrome_shortcut_ch {
                ...
                .and_then(|s| crate::views::extensions_modal::resolve_key(s.active_tab, ch))
            {
                self.log_extensions_modal_resolved_action(
                    ch,
                    &action,
                    xai_yis_telemetry::events::ExtensionsInputMethod::Mouse,
                );
...
mod extensions_action_target_tests {
    ...
    fn skills_toggle_resolves_name_and_resulting_state() {
        let skill = xai_yis_tools::implementations::skills::types::SkillInfo {
            name: "my-skill".into(),
            enabled: false,
            ..Default::default()
        };
