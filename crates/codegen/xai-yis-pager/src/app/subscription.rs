...
/// `yis_build_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `YIS_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    /// Yis 本地模式下视为非 consumer，避免向 xAI 轮询订阅/门闸。
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("YIS_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: xai_yis_shell::auth::GateInfo) -> Vec<Effect> {
        // 本地模式：忽略云端订阅门闸（无需访问 cli-chat-proxy / auth.x.ai）。
        if crate::i18n::local_mode() {
            crate::unified_log::info(
                "subscription.gate.skipped_local_mode",
                None,
                Some(serde_json::json!({ "local_mode": true })),
            );
            return vec![];
        }
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: xai_yis_shell::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> xai_yis_shell::auth::GateInfo {
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = xai_yis_shell::auth::GateInfo {
