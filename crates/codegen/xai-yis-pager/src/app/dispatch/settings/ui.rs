...
/// Open the model picker (ArgPicker for `/model`). Used by bare `/model`,
/// the command palette, and Ctrl+M (via `Action::OpenModelPicker`).
/// Toggles closed when the model ArgPicker is already open.
pub(in crate::app::dispatch) fn dispatch_open_model_picker(app: &mut AppView) -> Vec<Effect> {
    ...
    // Toggle: second open while model picker is active closes it.
    ...
    match (key, rollback_value) {
        ...
        ("fork_secondary_model", SettingValue::String(s)) => {
            let restored = if s.is_empty() {
                xai_yis_shell::models::default_model().to_string()
            } else {
                s.clone()
            };
