    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    yis_home: xai_yis_tools::util::yis_home::yis_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                xai_yis_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
        ...
        TaskResult::LogoutComplete => {
            ...
            // Yis local mode: do not drop into the browser login splash after
            // logout — stay on the welcome prompt (BYOK / model manager).
