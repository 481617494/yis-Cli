...
use super::transcript::{
    ...
    dispatch_open_model_manager, dispatch_open_transcript_pager,
};
use super::turn::{
...
pub(crate) fn dispatch(action: Action, app: &mut AppView) -> Vec<Effect> {
    ...
    let effects = match action {
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, xai_yis_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(xai_yis_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::OpenModelManager => dispatch_open_model_manager(app),
        ...
        Action::AnnouncementsOpenCta(surface) => {
            ...
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(xai_yis_telemetry::events::AnnouncementCtaClicked {
                    id: promo.id.clone(),
                    source: surface,
                });
