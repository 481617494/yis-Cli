...
use xai_yis_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(xai_yis_shell::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Yis Cli usage limit for now. Get SuperGrok for much higher limits, or try again later: https://grok.com/supergrok?referrer=grok-build";
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_yis_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Yis Cli",
            CreditLimitCardAction::PurchaseCredits,
            xai_yis_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_yis_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_yis_telemetry::events::CreditLimitUpsellShown {
            surface: xai_yis_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_yis_telemetry::events::CreditLimitUpsellShown {
        surface: xai_yis_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_yis_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
fn open_supergrok_upsell(
    ...
    log_event(xai_yis_telemetry::events::SuperGrokUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of Yis Cli. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_yis_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_yis_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_supergrok_url(app: &mut AppView) -> Vec<Effect> {
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "yis-build");
