...
pub(super) fn dispatch_dashboard_dispatch_slash(app: &mut AppView, text: String) -> Vec<Effect> {
    ...
    let result = {
        ...
        {
            use xai_yis_telemetry::events::{PagerCommandSource, PagerSlashCommand};
    ...
    match result {
        ...
        // `/model` with no args opens the interactive picker in a session;
        // on the session-less dashboard, prompt the user to name a model.
        CommandResult::Action(Action::OpenModelPicker) => {
            if let Some(d) = app.dashboard.as_mut() {
                d.dispatch.set_text("");
                d.set_error_toast("请使用 /model <名称> 为下一次会话指定模型");
            }
            vec![]
        }
...
pub(super) fn dispatch_dashboard_permission_select(
        ...
        .filter(|_| option_id.0.as_ref() == "allow-always-mcp")
    {
        let selection = match scope.selected {
            crate::views::permission_view::McpScope::Tool => {
                xai_yis_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                }
            }
            crate::views::permission_view::McpScope::Server => match &scope.server_prefix {
                Some(prefix) => xai_yis_workspace::permission::McpScopeSelection::Server {
                    server: prefix.clone(),
                },
                None => xai_yis_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                },
            },
        };
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(xai_yis_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
