...
pub(super) fn dispatch_switch_account(app: &mut AppView) -> Vec<Effect> {
    // Yis local mode: no browser OIDC account switch.
    if crate::i18n::local_mode() {
        tracing::info!("Yis local mode: ignoring switch-account browser login");
...
///
/// Yis local mode: never open the browser OIDC page automatically. Startup
/// already skips this path; this guard also blocks residual `/login` or
/// auto-Login effects from flipping into `AuthState::Authenticating`.
/// Set `YIS_LOCAL_MODE=0` or use cloud mode if xAI browser login is needed.
pub(super) fn dispatch_login(app: &mut AppView) -> Vec<Effect> {
    if crate::i18n::local_mode() {
        tracing::info!("Yis local mode: ignoring interactive browser login request");
        app.auth_state = AuthState::Done;
        app.welcome_prompt_focused = true;
        app.auth_show_raw_url = false;
        app.auth_code_input.clear();
        if !app.is_api_key_auth && app.auth_methods.iter().any(|m| {
        ...
        } else if !app.is_api_key_auth {
            // No xAI session and no xAI API key method — still allow local BYOK
            // models; keep offline-friendly defaults.
            app.is_api_key_auth = true;
            app.usage_visible = false;
            app.ensure_voice_for_api_key();
        }
...
pub(super) fn handle_auth_complete(
        ...
        && *current_seq == request_seq
    {
        if let Some(meta_val) = meta.as_ref()
            && let Ok(auth_meta) =
                serde_json::from_value::<xai_yis_shell::auth::AuthMeta>(meta_val.clone())
        {
