...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_yis_telemetry::events::ContextualTip {
            tip: xai_yis_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_yis_telemetry::events::ContextualTip {
            tip: xai_yis_telemetry::events::ContextualTipKind::SmallScreen,
            action: xai_yis_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_yis_telemetry::events::ContextualTip {
            tip: xai_yis_telemetry::events::ContextualTipKind::WordSelect,
            action: xai_yis_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(xai_yis_telemetry::events::ContextualTip {
            tip: xai_yis_telemetry::events::ContextualTipKind::SendNow,
            action: xai_yis_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
        ...
        if let Some((kind, body)) = notification {
            ...
            if kind != NotificationEventKind::TurnComplete || queue_empty {
                ...
                let notif_title = session_name
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| crate::i18n::PRODUCT_NAME.into());
