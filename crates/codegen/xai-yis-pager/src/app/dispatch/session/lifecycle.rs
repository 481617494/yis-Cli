...
pub(in crate::app::dispatch) fn open_agent_type_mismatch_question(
    ...
    effort: Option<xai_yis_shell::sampling::types::ReasoningEffort>,
    model_name: &str,
) -> Vec<Effect> {
    use crate::views::question_view::{LocalQuestionKind, QuestionViewState};
    use xai_yis_tools::implementations::yis_build::ask_user_question::{
...
pub(in crate::app::dispatch) fn dispatch_new_session_inner_with_id(
    ...
    // Local privacy: no cloud login — guide users to configure a BYOK model
    // (qoder-switch style manager) when none exist yet.
    if crate::i18n::local_mode()
        && !xai_yis_shell::model_presets::has_configured_byok_model()
        && let Some(agent) = app.agents.get_mut(&agent_id)
        && agent.model_manager.is_none()
    {
        agent.model_manager = Some(crate::views::model_manager::ModelManagerState::open());
        agent.show_toast(if crate::i18n::is_zh() {
            "本地模式：请配置模型（选厂商 + API Key）。也可运行 yis models setup"
        } else {
            "Local mode: configure a model (vendor + API key). Or: yis models setup"
        });
    }
    (agent_id, effects)
}
...
/// the workspace (writes `~/.yis/trusted_folders.toml`), mark trust resolved,
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    if let TrustState::Pending { workspace } = &app.trust_state {
        xai_yis_workspace::folder_trust::grant_folder_trust(workspace);
    }
    finish_trust(app)
}
