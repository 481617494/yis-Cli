...
pub enum ActiveModal {
    ...
    SessionPicker {
        ...
        content_results: Option<Vec<xai_yis_shell::extensions::session_search::SearchSessionHit>>,
...
pub enum PaletteCommand {
    ...
    OpenSettings,
    /// Open the model picker (same as `/model` / Ctrl+M).
    OpenModelPicker,
...
pub fn default_palette_entries(sharing_enabled: bool) -> Vec<PaletteEntry> {
    let mut entries = vec![
        ...
        PaletteEntry {
            label: "切换模型".into(),
            shortcut: "/model · Ctrl+M".into(),
            command: PaletteCommand::OpenModelPicker,
        },
        PaletteEntry {
            label: "始终批准模式".into(),
...
impl ActiveModal {
    ...
    pub fn message(&self, drain_blocked: bool) -> &str {
        match self {
            ...
            ActiveModal::ResetSettingsConfirm { .. } => "重置该设置？",
...
pub fn render_doc_picker_overlay(
    ...
    if let Some(tip_rect) = tip_area {
        let docs_path = crate::util::display_user_yis_path(DOCS_USER_GUIDE_REL);
...
mod doc_picker_tip_tests {
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        let path = crate::util::display_user_yis_path(DOCS_USER_GUIDE_REL);
        let long =
