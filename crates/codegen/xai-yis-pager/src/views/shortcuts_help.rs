...
/// Category display order for the cheatsheet (labels via [`crate::i18n_shortcuts`]).
...
pub fn build_entries(
    ...
    for (cat_idx, &cat) in CATEGORY_ORDER.iter().enumerate() {
        ...
        entries.push(ShortcutsHelpEntry::SectionHeader {
            label: crate::i18n_shortcuts::category_label(cat),
        ...
        for def in defs {
            ...
            // off (remote kill switch / `YIS_VOICE_MODE=0`). Unlike the old
            ...
            // Localize short label + description + long_help for the cheatsheet UI.
            // `def.label` / `def.description` / `def.long_help` are `'static`, so
            // localized borrows stay valid for the modal lifetime.
            item.label = Cow::Borrowed(crate::i18n::hint_label(def.label));
            item.description =
                Some(Cow::Borrowed(crate::i18n_shortcuts::description(def.description)));
            // Identical row for both arms; `item` moves in, `dimmed`/`def.id` are Copy.
            let hint = ShortcutsHelpEntry::Hint {
                item,
                dimmed,
                action_id: Some(def.id),
                long_help: def.long_help.map(crate::i18n_shortcuts::long_help),
            };
        ...
        if vim_mode && cat == Category::ConversationNav {
            let mut item = HintItem::new(
                crate::key!('/'),
                crate::i18n::hint_label("search"),
            );
            item.description = Some(Cow::Borrowed(crate::i18n_shortcuts::search_scrollback_desc()));
        ...
        if cat == Category::Input {
            let mut item = HintItem::new(
                crate::key!('v', CONTROL),
                crate::i18n::hint_label("paste"),
            );
            item.description = Some(Cow::Borrowed(crate::i18n_shortcuts::paste_desc()));
            ...
            entries.push(ShortcutsHelpEntry::Hint {
                ...
                dimmed,
                action_id: None,
                long_help: Some(crate::i18n_shortcuts::paste_long_help()),
            });
        }
...
fn hint_key_display(h: &HintItem) -> String {
    if let Some(d) = h.custom_display {
        crate::input::key::platformize_chord_display(d)
    } else {
        h.keys
            .iter()
            .map(|k| k.display())
...
/// the actual keys with pretty formatting (e.g. "Ctrl+Q" / "⌃+Q" on macOS).
fn hint_key_pretty(h: &HintItem) -> String {
    if let Some(d) = h.custom_display {
        return crate::input::key::platformize_chord_display(d);
    }
    h.keys
        .iter()
...
pub fn detail_from_entry(entry: &ShortcutsHelpEntry) -> Option<ShortcutsHelpMode> {
    ...
    let keys_line = item
        .custom_display
        .map(crate::input::key::platformize_chord_display)
        .unwrap_or_else(|| {
            item.keys
                .iter()
                .map(|k| k.display())
...
pub fn modal_footer_detail() -> Vec<crate::views::modal_window::Shortcut<'static>> {
    ...
    vec![
        Shortcut {
            label: crate::i18n_shortcuts::footer_back(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_scroll(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_close_chord(),
            clickable: false,
            id: 0,
        },
    ]
...
pub fn render_detail_body<'a>(
    ...
    if dimmed_note {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            crate::i18n_shortcuts::dimmed_context_note(),
...
pub fn render_detail(
    ...
    let modal_config = mw::ModalWindowConfig {
        title: crate::i18n_shortcuts::shortcuts_title(),
        tabs: None,
        shortcuts: &footer,
...
pub fn modal_footer(filter_active: bool) -> Vec<crate::views::modal_window::Shortcut<'static>> {
    ...
    let mut shortcuts = vec![
        Shortcut {
            label: crate::i18n_shortcuts::footer_nav(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_filter(filter_active),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_expand(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_collapse(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_details(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_search(),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: crate::i18n_shortcuts::footer_close(),
            clickable: false,
            id: 0,
        },
    ];
...
mod tests {
                ...
                ShortcutsHelpEntry::Hint { item, .. } => {
                    if item.label == "mouse reporting" {
                        ...
                        let ok = if cfg!(target_os = "macos") {
                            key_text.contains("⌃+r") || key_text.contains("⌃+R")
                        } else {
                            key_text.contains("Ctrl+r") || key_text.contains("Ctrl+R")
                        };
                        assert!(ok, "expected Ctrl/⌃+r in key display, got {key_text:?}");
                        seen = true;
                    }
                }
            }
    ...
    fn build_entries_surfaces_interject_ctrl_i_fallback() {
        ...
        assert_cheatsheet_row_has_key(&entries, "send now", expected);
    }
    ...
    fn build_entries_surfaces_queue_ctrl_apostrophe_fallback() {
        ...
        assert_cheatsheet_row_has_key(&entries, "queue", expected);
    }
