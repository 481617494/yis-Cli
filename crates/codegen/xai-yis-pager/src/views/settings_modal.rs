...
fn render_docs_footer(buf: &mut Buffer, area: Rect, theme: &Theme) {
    const LONG: &str =
        "Tip · Ask Grok: \"change theme to yisday\" or \"what does compact mode do?\"";
...
mod tests {
    ...
    fn picking_enum_esc_returns_to_browse() {
        ...
        match outcome {
            ...
            other => panic!("expected Action::PreviewTheme(\"yisnight\") on Esc, got {other:?}"),
        }
        assert!(matches!(s.mode, SettingsModalMode::Browse));
    }
    ...
    fn docs_footer_tip_is_centered() {
        ...
        assert!(
            !row_short.contains("yisday"),
            "width=40 must NOT render LONG path (no `yisday`): {row_short:?}",
        );
        assert!(
    ...
    fn click_settings_breadcrumb_collapses_picker_to_browse() {
        ...
        // `"yisnight"`. Tightened from the previous `Action(_) |
        ...
        match outcome {
            ...
            other => panic!(
                "expected Action(PreviewTheme(\"yisnight\")) — the keyboard \
    ...
    fn click_settings_breadcrumb_after_nav_reverts_to_original() {
        ...
        // Pick a different index. The default theme is `yisnight`
    ...
    fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
        ...
        match outcome {
            SettingsKeyOutcome::ActionPair(
                ...
                // Default theme is `yisnight`; entering the picker
                // captures `original_value = current value = yisnight`,
    ...
    /// misleading: on dark themes (YisNight, TokyoNight, RosePine
    /// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
    /// only on the YisDay light theme is title darker. The
    ...
    fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
        ...
        let raw_theme = match crate::theme::Theme::current_kind() {
            crate::theme::ThemeKind::YisNight => crate::theme::Theme::yisnight(),
            crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
            crate::theme::ThemeKind::YisDay => crate::theme::Theme::yisday(),
            ...
            crate::theme::ThemeKind::Auto => crate::theme::Theme::yisnight(),
        };
        assert_ne!(
