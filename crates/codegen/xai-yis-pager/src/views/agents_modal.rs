...
use xai_yis_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use xai_yis_shell::agent::config::AgentSelectionConfig;
use xai_yis_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.yis` vs `{cwd}/.yis`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `YIS_AGENT`,
...
/// Skips internal variants (YisBuildConcise, YisBuildPlan,
/// YisBuildPlanNoSubagents, YisBuildAskUser, Codex, Opencode,
/// CursorExtended, YisBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = xai_yis_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.yis/personas` and `{cwd}/.yis/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = yis_home.join("bundled").join("personas");
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".yis").join("personas")),
        (ConfigFileScope::User, yis_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match xai_yis_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => xai_yis_config::yis_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".yis").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.yis/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.yis/{subdir}` or `{cwd}/.yis/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let yis_home = xai_yis_config::yis_home();
    let in_user = dunce::canonicalize(yis_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".yis").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    xai_yis_shell::config::load_effective_config()
        .ok()
        .and_then(|root| xai_yis_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in xai-yis-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    xai_yis_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        xai_yis_agent::config::PromptMode::Extend => "extend",
        xai_yis_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use xai_yis_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".yis").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".yis").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.yis/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".yis").join("personas").join("gone.toml");
