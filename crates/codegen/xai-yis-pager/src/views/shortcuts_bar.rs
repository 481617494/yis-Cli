...
impl HintItem {
    ...
    /// Hardcoded `custom_display` chords are platformized (macOS: Ctrl→⌃, Alt→⌥, …).
    fn key_display(&self) -> String {
        if let Some(display) = self.custom_display {
            crate::input::key::platformize_chord_display(display)
        } else {
            self.keys
                .iter()
                .map(|k| k.display())
...
impl Widget for ShortcutsBar<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (i, hint) in effective.iter().enumerate() {
            ...
            // Localize label for display only (internal label stays English).
            let label = crate::i18n::hint_label(hint.label.as_ref());
            let action_span = Span::styled(label, action_style);
            let action_width = label.width() as u16;
