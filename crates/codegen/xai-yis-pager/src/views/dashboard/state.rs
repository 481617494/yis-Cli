...
/// `~/.yis/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<xai_yis_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_yis_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.yis/config.toml`.
///
...
/// `~/.yis/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no yis home"))?;
...
fn config_path() -> Option<PathBuf> {
    let home = xai_yis_shell::util::yis_home::yis_home();
...
mod tests {
    ...
    /// per-key serial lock. The `YIS_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(YIS_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `YIS_AGENT_DASHBOARD` key reads it concurrently.
