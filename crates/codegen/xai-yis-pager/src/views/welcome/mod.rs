...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a xai_yis_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Yis Cli** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Yis Cli Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Yis Cli Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    if show_api_key && is_api_key_auth {
        spans.push(Span::styled(
            crate::i18n::logged_in_api_key(),
            Style::default().fg(theme.gray),
        ));
        spans.push(sep);
    }

    let channel = xai_yis_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            spans.push(Span::styled(
                crate::i18n::product_badge(),
                Style::default()
                    .fg(theme.text_primary)
                    .add_modifier(Modifier::BOLD),
            ));
            spans.push(Span::styled(
                format!("{}{}", xai_yis_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
        }
        VersionBadgeMode::HeroFooter => {
            let channel_display = if channel.is_empty() {
                crate::i18n::PRODUCT_SHORT
            } else {
                channel.trim()
            };
            spans.push(Span::styled(
...
fn render_prompt_and_version(
    ...
    if let Some(pending) = &pending_hint {
        ...
        let label = crate::i18n::press_again_hint(pending.label);
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a xai_yis_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a xai_yis_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [xai_yis_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a xai_yis_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            ...
            let login_text = if crate::i18n::is_zh() {
                format!("使用 {} 登录", label)
            } else {
                format!("Login with {}", label)
            };
            let quit = crate::i18n::quit_label();
        ...
        AuthState::Done if params.is_zdr_blocked => {
            let menu = [
                ("l", crate::i18n::switch_account_label()),
                ("q", crate::i18n::quit_label()),
            ];
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                content_area,
                buf,
                Some((crate::i18n::zdr_blocked_msg(), theme.gray_bright)),
                &menu,
                params.selected,
                None,
                h_margin,
...
/// multi-line block showing the workspace path and the warning that Yis Cli
...
fn render_welcome_trust(
    ...
    let menu_items = [
        ("y", crate::i18n::trust_yes()),
        ("n", crate::i18n::trust_no()),
    ];
    let lines = vec![
        Line::from(Span::styled(
            crate::i18n::trust_question(),
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &xai_yis_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let menu_items: &[(&str, &str)] = if !p.has_access {
        gate_menu = [
            (key_g, cta),
            (key_l, crate::i18n::logout_label()),
            (key_q, crate::i18n::quit_label()),
        ];
        &gate_menu
    } else {
        ...
        if p.has_claude_import {
            ...
            items.push((
                key_i_with_x,
                crate::i18n::t("导入 Claude 配置", "Import Claude settings"),
            ));
        }
        items.push((
            key_w,
            crate::i18n::t("新建 Worktree", "New worktree"),
        ));
        items.push((
            key_s,
            crate::i18n::t("恢复会话", "Resume session"),
        ));
        // "Changelog" above Quit; no shortcut — opened by click (row or block).
        if show_changelog_action {
            items.push(("", crate::i18n::t("更新日志", "Changelog")));
        }
        items.push((key_q, crate::i18n::quit_label()));
        owned_menu = items;
        owned_menu.as_slice()
    };
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use xai_yis_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = xai_yis_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn long_ann() -> xai_yis_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = xai_yis_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
