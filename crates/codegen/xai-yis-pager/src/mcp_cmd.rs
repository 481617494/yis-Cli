...
use crate::util::display_user_yis_path;
...
Examples:
  ...
  # Add to the project config (./.yis/config.toml) instead of ~/.yis/config.toml
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.yis/config.toml`, available in all your projects
    User,
    /// `./.yis/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.yis/config.toml) or project (./.yis/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = xai_yis_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `yis mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    xai_yis_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => xai_yis_shell::util::config::user_config_path(),
        McpScope::Project => {
            xai_yis_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_yis_path("config.toml"),
...
fn select_remove_site(
    ...
    use xai_yis_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    xai_yis_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        xai_yis_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("  user: {}", display_user_yis_path("config.toml"));
    ...
    // ancestor .yis/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = xai_yis_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        xai_yis_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["yis", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["yis", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["yis", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "yis", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["yis", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["yis", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["yis", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["yis", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["yis", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["yis", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = xai_yis_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.yis/config.toml");
