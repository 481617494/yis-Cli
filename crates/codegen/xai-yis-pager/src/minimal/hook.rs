...
//! render mode.
//!
//! `xai-yis-pager` cannot depend on `xai-yis-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-yis-pager-bin`) wires it up once at
//! startup by calling `xai_yis_pager_minimal::install()`. When nothing has
