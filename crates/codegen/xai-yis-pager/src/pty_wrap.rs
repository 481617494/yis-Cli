...
//! system clipboard. This is the engine behind `yis wrap` (see
...
///
/// This is the engine behind `yis wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // reads this (see `xai_yis_pager_render::clipboard::osc52_sink_active`) to
    ...
    // these two without clearing it. The canonical `YIS_OSC52_SINK` is
    ...
    cmd.env("YIS_OSC52_SINK", "1");
    cmd.env("LC_YIS_OSC52_SINK", "1");
    ...
    // detached — `yis wrap` is short-lived and exits with the child.
...
///
/// Delegates to [`xai_yis_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = xai_yis_shell::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = xai_yis_pager_render::clipboard::system_clipboard_get_image();
