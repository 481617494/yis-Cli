...
use xai_yis_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a yis-shell instance.
    pub is_yis_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<xai_yis_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<xai_yis_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_yis_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_yis_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    xai_yis_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_yis_shell::util::yis_home::yis_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a yis-shell agent
    let is_yis_shell = resp
        .meta
        .as_ref()
...
mod tests {
    ...
    #[test]
    fn startup_auth_yis_com_no_provider_needs_login_pending() {
    ...
    #[test]
    fn startup_auth_yis_com_with_external_provider_command() {
    ...
    #[test]
    fn startup_auth_non_yis_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.yis/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_yis_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    /// `startup_auth_yis_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_yis_shell::agent::auth_method::{YIS_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(YIS_COM_METHOD_ID, "Yis Cli", None),
