...
use xai_yis_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn args_required(&self) -> bool {
        // 无参数时打开模型选择器（与 Ctrl+M / 命令面板「切换模型」一致）。
        false
    }
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        if trimmed.is_empty() {
            // 打开交互式模型列表（与 Ctrl+M、命令面板一致）。
            return CommandResult::Action(Action::OpenModelPicker);
        }

        // 从选择器点了「+ 新增模型」→ 打开完整管理弹窗
        if trimmed == "model-add-hint"
            || trimmed.eq_ignore_ascii_case("model-add")
            || trimmed == "+ 新增模型（/model-add）"
        {
            return CommandResult::Action(Action::OpenModelManager);
        }
...
fn build_model_items(models: &ModelState) -> Vec<ArgItem> {
    ...
    // 打开 qoder-switch 式管理弹窗
    items.push(ArgItem {
        display: if crate::i18n::is_zh() {
            "+ 新增/管理模型".into()
        } else {
            "+ Add / manage models".into()
        },
        match_text: "model-add".into(),
        insert_text: "model-add-hint".into(),
        description: if crate::i18n::is_zh() {
            "选厂商 · 密钥 · 自动拉列表 · 编辑删除".into()
        } else {
            "vendor · key · fetch list · edit/delete".into()
        },
    });
    items
}
...
mod tests {
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        ...
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("yis", "Grok");
    ...
    #[test]
    fn run_empty_args_opens_model_picker() {
        let state = ModelState::default();
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "");
        assert!(
            matches!(result, CommandResult::Action(Action::OpenModelPicker)),
            "expected OpenModelPicker, got {result:?}"
        );
        let result_ws = ModelCommand.run(&mut ctx, "   ");
        assert!(
            matches!(result_ws, CommandResult::Action(Action::OpenModelPicker)),
            "expected OpenModelPicker for whitespace, got {result_ws:?}"
        );
    }
