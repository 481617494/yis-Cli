...
use xai_yis_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $YIS_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
    ...
    if !agent_config.is_trace_upload_enabled() {
        ...
        if !args.json {
            eprintln!(
                ...
                crate::util::display_user_yis_path("config.toml")
            );
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            yis_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    xai_yis_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_yis_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    yis_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `yis login` or set a deployment key. \
                 See {} for upload overrides.",
                crate::util::display_user_yis_path("docs/user-guide")
            );
        }
    };
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            xai_yis_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
            "No trace upload bucket configured. Set `YIS_TELEMETRY_GCS_BUCKET`, \
             `YIS_TRACE_UPLOAD_BUCKET`, or `endpoints.trace_upload_bucket` in \
    ...
    let upload_config = xai_yis_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &xai_yis_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = xai_yis_shell::auth::ensure_authenticated_or_noninteractive(
