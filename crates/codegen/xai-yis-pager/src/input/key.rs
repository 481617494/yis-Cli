...
//!
//! ```
//! use xai_yis_pager::input::key::key;
//!
//! // Simple key
//! let q = key!('q');
//!
...
impl KeyShortcut {
    ...
    ///
    /// Uses platform-aware modifier labels (`Ctrl+Q` on Windows/Linux,
    /// `⌃+Q` / `⌘+,` on macOS). Shift is always shown explicitly
    /// (e.g. `Shift+G`, `Ctrl+Shift+P`, `Shift+Tab`).
    pub fn display_pretty(&self) -> String {
        let mut parts: Vec<String> = Vec::new();
        // SUPER first, spelled per-platform (Cmd/⌘ on macOS).
        if self.modifiers.contains(KeyModifiers::SUPER) {
            parts.push(mod_super_label().into());
        }
        if self.modifiers.contains(KeyModifiers::CONTROL) {
            parts.push(mod_control_label().into());
        }
        if self.modifiers.contains(KeyModifiers::ALT) {
            parts.push(mod_alt_label().into());
        }
        let has_shift = self.modifiers.contains(KeyModifiers::SHIFT);
        if has_shift {
            parts.push(mod_shift_label().into());
        }
...
/// True when shortcut chrome should use macOS modifier labels/symbols.
#[inline]
pub fn is_macos_key_ui() -> bool {
    cfg!(target_os = "macos")
}

/// Control / ⌃ — physical Control key (not Command). Terminal bindings use
/// CONTROL; advertising ⌘ would be wrong (and Cmd+Q is often swallowed by
/// the host terminal).
#[inline]
pub fn mod_control_label() -> &'static str {
    if is_macos_key_ui() {
        "⌃"
    } else {
        "Ctrl"
...
/// Alt / ⌥ Option.
#[inline]
pub fn mod_alt_label() -> &'static str {
    if is_macos_key_ui() {
        "⌥"
    } else {
        "Alt"
...
/// Super / ⌘ Command (Win key on Windows, Super on Linux).
#[inline]
pub fn mod_super_label() -> &'static str {
    if is_macos_key_ui() {
        "⌘"
    } else {
        "Super"
...
/// Shift / ⇧.
#[inline]
pub fn mod_shift_label() -> &'static str {
    if is_macos_key_ui() {
        "⇧"
    } else {
        "Shift"
...
/// Rewrite a Windows-style chord string (`Ctrl+x`, `Alt+Tab`, `Shift+Tab`)
/// for the current OS. Used for hardcoded `hint_key_display` / cheatsheet
/// copy that bypasses [`KeyShortcut::display`].
///
/// Windows/Linux: returned unchanged. macOS: `Ctrl`→`⌃`, `Alt`→`⌥`,
/// `Cmd`/`Super`/`Win`→`⌘`, `Shift`→`⇧`.
pub fn platformize_chord_display(s: &str) -> String {
    if !is_macos_key_ui() {
        return s.to_string();
    }
    let mut out = s.to_string();
    // Longer / more specific tokens first so we don't partial-replace.
    const REPLACEMENTS: &[(&str, &str)] = &[
        ...
        ("SHIFT+", "⇧+"),
        ("shift+", "⇧+"),
        // Slash-separated dual labels, e.g. "Ctrl+./X"
        ("Control", "⌃"),
        ("Ctrl", "⌃"),
        ("CTRL", "⌃"),
        ("Option", "⌥"),
...
impl fmt::Display for KeyShortcut {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let has_shift = self.modifiers.contains(KeyModifiers::SHIFT);
        // SUPER first, spelled per-platform (⌘ on macOS).
        if self.modifiers.contains(KeyModifiers::SUPER) {
            write!(f, "{}+", mod_super_label())?;
        }
        if self.modifiers.contains(KeyModifiers::CONTROL) {
            write!(f, "{}+", mod_control_label())?;
        }
        if self.modifiers.contains(KeyModifiers::ALT) {
            write!(f, "{}+", mod_alt_label())?;
        }
        if has_shift {
            write!(f, "{}+", mod_shift_label())?;
        }
        match self.code {
            ...
            KeyCode::BackTab => write!(f, "{}+Tab", mod_shift_label()),
...
mod tests {
    ...
    fn display_formatting() {
        ...
        if cfg!(target_os = "macos") {
            // macOS: Apple modifier symbols (⌃ Control, ⌥ Option, ⌘ Command, ⇧ Shift).
            assert_eq!(key!('c', CONTROL).to_string(), "⌃+c");
            assert_eq!(key!('x', ALT).to_string(), "⌥+x");
            assert_eq!(key!('n', CONTROL | SHIFT).to_string(), "⌃+⇧+n");
            assert_eq!(key!('h', CONTROL | SHIFT).to_string(), "⌃+⇧+h");
            assert_eq!(key!('g', SHIFT).to_string(), "⇧+g");
            assert_eq!(key!(',', SUPER).to_string(), "⌘+,");
            assert_eq!(key!(',', SUPER).display_pretty(), "⌘+,");
            assert_eq!(key!('n', CONTROL | SHIFT).display_pretty(), "⌃+⇧+n");
            assert_eq!(key!('h', CONTROL | SHIFT).display_pretty(), "⌃+⇧+h");
            assert_eq!(platformize_chord_display("Ctrl+x"), "⌃+x");
            assert_eq!(platformize_chord_display("Shift+Tab"), "⇧+Tab");
            assert_eq!(platformize_chord_display("Ctrl+Space / F8"), "⌃+Space / F8");
        } else {
            // Windows / Linux: keep Windows-style labels.
            ...
            assert_eq!(platformize_chord_display("Ctrl+x"), "Ctrl+x");
            assert_eq!(platformize_chord_display("Shift+Tab"), "Shift+Tab");
        }
    }
