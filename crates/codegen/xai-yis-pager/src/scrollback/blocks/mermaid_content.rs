...
//! the generic [`CodeBlockSpan`](xai_yis_markdown::CodeBlockSpan) API) and
...
use xai_yis_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_yis_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_yis_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_yis_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `YisDay` is the only light theme; every other concrete theme (and the
/// `YisNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `xai_yis_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::YisDay)
}
...
mod tests {
    ...
    use xai_yis_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_yisday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::YisDay),
            "YisDay is the light theme"
        );
        for dark in [
            ThemeKind::YisNight,
