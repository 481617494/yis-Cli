    ...
    pub fn message(&self) -> String {
        match self {
            ...
            SessionEvent::ReAuthRequired => {
                // Local / privacy mode: no xAI browser login — fix BYOK credentials.
                if crate::i18n::local_mode() {
                    if crate::i18n::is_zh() {
                        "认证失败：模型 API Key 或 base_url 可能无效。请运行 /model-add \
                         或 yis models setup 检查密钥后重试。"
                            .to_string()
                    } else {
                        "Authentication failed: check the model API key / base_url. \
                         Run /model-add or `yis models setup`, then resend."
                            .to_string()
                    }
                } else {
...
mod tests {
    ...
    fn reauth_required_message_points_at_login() {
        ...
        // Local mode (Yis default) points at model setup, not /login.
        if crate::i18n::local_mode() {
            assert!(
                msg.contains("/model-add") || msg.contains("models setup") || msg.contains("API"),
                "local mode must guide BYOK setup: {msg}"
            );
        } else {
