...
use xai_yis_shell::session::{ContextInfo, count_detail};
...
impl ContextInfoBlock {
    ...
    fn build_lines(&self, theme: &Theme, bar: BarLayout) -> Vec<Line<'static>> {
        ...
        // (routed from xai-yis-shell's model config resolution). This makes
...
mod tests {
    use super::*;
    use xai_yis_shell::session::TokenUsageCategory;
    ...
    fn bar_and_legend_reconcile_used_with_overhead_excluding_tools() {
        ...
        let block = ContextInfoBlock::new(snap, "yis-build");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
