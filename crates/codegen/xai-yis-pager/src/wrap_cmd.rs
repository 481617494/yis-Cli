//! `yis wrap` — run any command in a local PTY that forwards its clipboard.
//!
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = xai_yis_config::shell::is_command_available(program);
        (
...
/// The program and argv `yis wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
/// deliberately not `xai_yis_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `grok ssh`, `yis wrap` does not gate on the terminal brand: the user
