#!/bin/bash
#
# Yis Cli installer — https://x.ai/cli/install.sh
#
# Auth: YIS_DEPLOYMENT_KEY (takes precedence) or ~/.yis/auth.json from `yis login`.
# Env: YIS_CHANNEL (stable|alpha|enterprise, default: stable), YIS_BIN_DIR, YIS_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   YIS_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.yis/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_yis_token() {
    local auth_file="$HOME/.yis/auth.json"
    local scope="$1"
...
# Resolve auth: YIS_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$YIS_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_yis_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_yis_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.yis/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.yis/auth.json." >&2
    fi
fi
...
DOWNLOAD_DIR="$HOME/.yis/downloads"
BIN_DIR="${YIS_BIN_DIR:-$HOME/.yis/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${YIS_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Yis Cli $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Yis Cli $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/yis-$platform"
artifact_base="${BASE_URL}/yis-${version}-${platform}"
...
echo "  Downloading yis ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Yis Cli is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in yis.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/yis.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded yis failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.yis/bin and ~/.yis/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/yis"
    ln -sf "$link_target" "$BIN_DIR/yis"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/yis and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.yis/completions/bash" "$HOME/.yis/completions/zsh"
"$BIN_DIR/yis" completions bash > "$HOME/.yis/completions/bash/yis.bash" 2>/dev/null || true
"$BIN_DIR/yis" completions zsh  > "$HOME/.yis/completions/zsh/_yis"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/yis" completions fish > "$HOME/.config/fish/completions/yis.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.yis/config.toml"
...
if [ -n "$YIS_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${YIS_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$YIS_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.yis/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.yis/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.yis/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.yis/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Yis Cli $version installed to $BIN_DIR/yis.exe" >&2
else
    echo "Yis Cli $version installed to $BIN_DIR/yis" >&2
fi

# --- Ensure yis is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so yis works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/yis" "$candidate/yis"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/yis -> $BIN_DIR/yis" >&2
...
# Also update shell config so ~/.yis/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> yis installer >>>
fish_add_path $HOME/.yis/bin
# <<< yis installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> yis installer >>>
export PATH="$HOME/.yis/bin:$PATH"
fpath=(~/.yis/completions/zsh $fpath)
...
# <<< yis installer <<<'
    ...
    if grep -qs "yis installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> yis installer >>>/ { skip=1; next }
            /# <<< yis installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'yis' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'yis' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'yis' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.yis/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use yis from cmd.exe or PowerShell, add %USERPROFILE%\\.yis\\bin to your PATH." >&2
fi
