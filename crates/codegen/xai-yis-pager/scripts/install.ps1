#
# Yis Cli installer for PowerShell — https://x.ai/cli/install.ps1
#
# Auth: YIS_DEPLOYMENT_KEY env var (takes precedence) or ~/.yis/auth.json from `grok login`.
...
#   $env:YIS_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:YIS_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
$YisDir = Join-Path $env:USERPROFILE '.yis'
...
function Read-GrokToken([string]$Scope) {
    $authFile = Join-Path $YisDir 'auth.json'
...
if ($env:YIS_DEPLOYMENT_KEY) {
    ...
    if ($oidcToken) {
        ...
        Write-Host 'Auth: using OIDC token from ~/.yis/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.yis/auth.json.' -ForegroundColor DarkGray
    }
}
...
$DownloadDir = Join-Path $YisDir 'downloads'
$BinDir = if ($env:YIS_BIN_DIR) { $env:YIS_BIN_DIR } else { Join-Path $YisDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:YIS_CHANNEL) { $env:YIS_CHANNEL } else { 'stable' }
...
$binaryPath = Join-Path $DownloadDir "yis-$platform.exe"
...
foreach ($binName in @('yis.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\yis.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $YisDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'yis.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'grok.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $YisDir 'config.toml'
...
if ($env:YIS_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:YIS_PROXY_URL) { $env:YIS_PROXY_URL } else { 'https://cli-chat-proxy.grok.com/v1' }
    Write-Host '  Fetching deployment config...' -ForegroundColor DarkGray
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:YIS_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $YisDir 'managed_config.toml'
        $requirementsPath = Join-Path $YisDir 'requirements.toml'
...
Write-Host "Grok $resolvedVersion installed to $BinDir\yis.exe" -ForegroundColor Green
