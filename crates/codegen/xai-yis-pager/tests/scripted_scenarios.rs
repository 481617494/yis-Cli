...
//!
//! These tests exercise the real `xai-yis-pager` binary through a PTY using
...
/// enterprise deploy report: with `YIS_GOAL=1`, `/goal` must
...
/// explicitly off (`YIS_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `YIS_FOLDER_TRUST=1` and a git repo that
...
/// also pins `YIS_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
