...
async fn reasoning_efforts_from_config_toml_menu() {
    ...
    // Seed `~/.yis/config.toml` with a per-model reasoning-effort menu.
    let yis_home = content.home().join(".yis");
    std::fs::create_dir_all(&yis_home).expect("create .yis");
    // Quote the dotted model id: bare `[model.yis-4.5]` is TOML key-path syntax (model.yis-4.5), not the id "grok-4.5".
    std::fs::write(
