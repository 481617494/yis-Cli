...
use xai_yis_pager_pty_harness::PtyHarness;
pub(crate) use xai_yis_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`xai-yis-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `xai-yis-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `YIS_SCROLL_SPEED`) appended to the controller's env.
