...
/// Like [`seed_fake_oauth`], but under the `YIS_LOCAL_AUTH` dev issuer
...
/// `YIS_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let yis_home = content.home().join(".yis");
    std::fs::create_dir_all(&yis_home).expect("create temp .yis");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("YIS_LOCAL_AUTH", "1"));
...
#[ignore = "PTY e2e; run with cargo test -p xai-yis-pager --test pty_e2e -- --ignored"]
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("YIS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("YIS_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
