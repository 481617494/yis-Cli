//! End-to-end tests for the built `xai-yis-pager` binary, driven through a PTY.
//!
//! Mirrors `xai-yis-shell/tests/test_built_binary_e2e.rs`: every test is
...
//! 2. `YIS_BINARY` env var (shared convention with the shell crate)
//! 3. `CARGO_BIN_EXE_xai-yis-pager` (set by `cargo test` for in-tree runs)
...
//! lives in the [`xai_yis_pager_pty_harness`] crate, not here, so its deps
//! never land in the production pager binary.
//!
//! Run locally:
//! ```bash
//! cargo test -p xai-yis-pager --test pty_e2e -- --ignored --nocapture
//! ```
//!
