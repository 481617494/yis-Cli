...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`yis-build-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "YIS_PROMPT_SUGGESTIONS");
    env.push(("YIS_PROMPT_SUGGESTIONS".into(), "true".into()));
