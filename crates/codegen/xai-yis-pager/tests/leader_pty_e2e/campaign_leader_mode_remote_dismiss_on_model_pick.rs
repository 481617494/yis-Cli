...
#[ignore = "PTY e2e; run with cargo test -p xai-yis-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared YIS_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let yis_home = content.home().join(".yis");
    std::fs::create_dir_all(&yis_home).expect("create YIS_HOME");
    std::fs::write(
        yis_home.join("config.toml"),
        format!("[models]\ndefault = \"{CONFIG_MODEL}\"\n"),
    )
    .expect("write config.toml");
    let socket = yis_home.join("leader-e2e.sock");
    ...
    let state_path = yis_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(yis_home.join("config.toml")).expect("read config.toml");
    assert!(
