#!/usr/bin/env node
// Runs once after npm install/update. Reads the yis binary from the
// matching per-platform optional dependency (@yis/cli-<platform>)
// and installs it to ~/.yis/bin/ using versioned filenames:
//
//   Unix:    yis-<version>  +  yis  (symlink)
//   Windows: yis-<version>.exe  +  yis.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.yis', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@yis/cli: unsupported platform ${key}`);
    process.exit(0);
}
...
if (!version) {
    console.error('@yis/cli: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@yis/cli: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@yis/cli: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running yis processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid yis-* matching yis-pager-*.
...
if (!platformDir) {
    console.error(`@yis/cli: platform package @yis/cli-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @yis/cli');
    process.exit(0);
}

installBinary('yis', platformDir, `yis${EXE}`);
cleanupOldVersions('yis');
cleanupOldVersions('yis-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.yis');
...
// Persist the npm registry so `yis update` and the trampoline use the same one.
const npmRegistry = process.env.YIS_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @yis:registry',
...
// Set YIS_INSTALL_COMPLETIONS=1 to auto-generate to ~/.yis/completions.
const YIS_PATH = path.join(CANONICAL_DIR, `yis${EXE}`);
if (process.env.YIS_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.yis', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'yis.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_yis');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(YIS_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(YIS_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.yis/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: yis completions bash > ~/.local/share/bash-completion/completions/yis');
    console.log('     yis completions zsh  > ~/.zsh/completions/_yis');
}
