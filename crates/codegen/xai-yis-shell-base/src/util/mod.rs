...
pub mod tips;
pub mod uname;
pub use xai_yis_shared::clipboard;
pub use xai_yis_shared::stderr::{stderr_lock, with_locked_stderr};
...
pub fn is_yis_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("yis"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    fn is_grok_process_self_true_impossible_pid_false() {
        assert!(is_yis_process(std::process::id()));
        assert!(!is_yis_process(u32::MAX));
    }
}
