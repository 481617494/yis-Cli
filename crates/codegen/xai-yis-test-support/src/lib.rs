...
//! - [`YisStdioClient`] — ACP client that drives `grok agent stdio` as a subprocess
...
//! - [`run_headless`] — Run `yis -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`yis_binary`] — Resolve the yis binary path (YIS_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{YisStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, yis_binary};
pub use headless::{
