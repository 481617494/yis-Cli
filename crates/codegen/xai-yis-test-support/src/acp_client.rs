...
//! [`YisStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
use crate::env::{yis_binary, test_env_cmd_tokio};
...
/// hermeticity setup exists exactly once for the typed ([`YisStdioClient`])
...
/// kill-list so a test can still set e.g. `YIS_DEBUG_LOG=1` explicitly.
...
///
/// Exists for wire shapes the typed [`YisStdioClient`] (`ClientSideConnection`,
