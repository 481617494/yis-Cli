...
/// (version-skew) test. Falls back to [`yis_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "YIS_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`yis_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "YIS_BINARY_CLIENT";
...
/// (`YIS_BINARY_LEADER`, else the shared [`yis_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`YIS_BINARY_CLIENT`,
/// else the shared [`yis_binary`] resolution).
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&yis_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export YIS_* vars
            // (e.g. YIS_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("YIS_LEADER_SOCKET", home.join(".yis").join("leader.sock"))
            .env("YIS_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("YIS_XAI_API_BASE_URL", server.url())
            ...
            // ~/.yis/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "xai_yis_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".yis").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".yis").join("leader.log")).unwrap_or_default()
}
