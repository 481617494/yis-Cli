...
fn local_yis_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xai-yis-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_yis_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "xai-yis-pager", "--bin", "xai-yis-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xai-yis-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xai-yis-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xai-yis-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve yis binary: `YIS_BINARY` env (CI) or a locally built `xai-yis-pager` binary.
pub fn yis_binary() -> PathBuf {
    if let Ok(path) = std::env::var("YIS_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "YIS_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_xai-yis-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_yis_binary_path();
    ensure_local_yis_binary(&binary);
    binary
}
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        ...
        // without an explicit YIS_HOME every spawned child shares the real
        // `%USERPROFILE%\.yis` — test 1's models_cache.json (which embeds
        ...
        .env("YIS_HOME", home.join(".yis"))
        .env("YIS_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("YIS_XAI_API_BASE_URL", mock_url)
        .env("XAI_API_KEY", "test-key-for-ci")
        .env("YIS_TELEMETRY_ENABLED", "false")
        .env("YIS_FEEDBACK_ENABLED", "false")
        .env("YIS_TRACE_UPLOAD", "false")
        .env("YIS_INSTRUMENTATION", "disabled")
