...
//! agent-session config types in `xai-yis-config` and friends.
...
/// `xai-yis-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_yis_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
