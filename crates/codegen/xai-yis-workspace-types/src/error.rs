...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] xai_yis_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `xai-yis-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `xai_yis_tools::ToolError`) happens
