//! Wire types for the `xai-yis-workspace` API.
//!
...
//! `xai-yis-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `xai-yis-workspace-types` (instead of `xai-yis-workspace` or
/// `xai-yis-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `xai_yis_workspace::permission`
