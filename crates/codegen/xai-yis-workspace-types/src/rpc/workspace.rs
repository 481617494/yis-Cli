...
/// over `xai_yis_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// One TODO list item (slim DTO over `xai_yis_tools`'s `TodoState`). `status`
...
/// in `xai-yis-workspace/src/hub_server.rs`.
