...
//!
//! `xai_yis_hooks` pulls in `git2`/`reqwest`/`xai-yis-tools`, too heavy for
...
/// Wire mirror of `xai_yis_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `xai_yis_hooks::config::HookSpec`.
///
...
/// `xai-yis-hooks`, so a server-side test (`xai-yis-workspace`'s
...
/// Wire mirror of `xai_yis_hooks::event::HookEventName`.
///
