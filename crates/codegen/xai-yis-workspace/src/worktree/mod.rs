...
//!
//! Moved from `xai-yis-shell/src/session/worktree.rs` into the workspace
...
// Canonical in xai-yis-workspace-types; re-exported for existing paths.
pub use xai_yis_workspace_types::rpc::worktree::{
...
/// Resolve the yis home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`xai_fast_worktree::resolve_yis_home`), so checkout dirs and
/// the metadata DB always live under the same `.yis` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_yis_config::yis_home()`,
/// so worktree paths also agree with trust/hooks and other yis-home paths.
fn yis_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_yis_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.yis/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    yis_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.yis/worktrees/<repo_name>`)
/// for a given source path, correctly handling yis-managed worktrees.
///
/// When `source_path` is already under `~/.yis/worktrees/<repo>/...`, the
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = yis_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.yis/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the yis-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.yis/worktrees/`.
...
///
/// When the source path is already inside `~/.yis/worktrees/<repo>/`, the
...
use xai_fast_worktree::{
    ...
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_yis_home,
};
...
/// Scan `~/.yis/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/yis/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/yis/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/yis/subagents/standalone-1";
    ...
    /// Point `YIS_HOME` at an isolated tempdir (`resolve_yis_home` re-reads
    ...
    /// and restores `YIS_HOME` on drop (before releasing the lock), so the
