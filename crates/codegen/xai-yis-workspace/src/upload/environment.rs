...
impl WorkspaceIdentity {
    ...
    /// `YisAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `xai-yis-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$YIS_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("YIS_SANDBOX_PROFILE").ok(),
            xai_yis_sandbox::is_inside_bwrap(),
