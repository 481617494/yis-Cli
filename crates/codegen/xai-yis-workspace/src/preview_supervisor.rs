...
/// `yis_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "yis_workspace_preview_proxy_restart_total",
...
/// Build the unspawned proxy command. Secrets (`YIS_SERVER_KEY` /
/// `YIS_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "YIS_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
