...
//!
//! Persists per-folder trust decisions to `~/.yis/trusted_folders.toml`.
...
//!
//! The store is rooted at [`xai_yis_config::user_yis_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.yis`) when
//! neither `$YIS_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.yis/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.yis/`.
...
impl TrustStore {
    /// Load the trust store from `<user_yis_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_yis_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`xai_yis_config::user_yis_home`], never
    /// [`xai_yis_config::yis_home`], so it never falls back to a cwd-relative
    /// `./.yis` — that fallback would let an untrusted cloned repo's `.yis`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(xai_yis_config::user_yis_home())
    }

    /// Map a resolved user-yis-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_yis_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_yis_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user yis home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A yis-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.yis/worktrees.db` registry), so every `grok -w` worktree
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A yis-managed worktree (any creation mode, incl. standalone clones git
...
/// sharing `~/.yis/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.yis/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = xai_yis_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global yis-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match xai_yis_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.yis");
        assert_eq!(
        ...
        // cwd-relative `./.yis` that yis_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.yis/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_yis_home() {
        // Thin source-pin: the production accessor reads user_yis_home()
        // (Option, no cwd fallback), not yis_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            xai_yis_config::user_yis_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    // ── workspace_key registry collapse (yis-managed worktrees) ─────────
    ...
    /// Point `YIS_HOME` at an isolated tempdir and register one yis-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `YIS_HOME` on
    ...
    fn register_grok_worktree(
        ...
        let env = LockedTestEnv::lock().set("YIS_HOME", &home);
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<yis_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        let outside = root.join("yis-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <yis_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
