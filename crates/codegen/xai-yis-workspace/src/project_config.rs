...
//! `.yis/config.toml` files by walking from `cwd` up to the git root.
//!
...
use xai_yis_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$YIS_HOME/config.toml` (user tier, not project).
fn is_user_grok_config_file(config_path: &Path) -> bool {
    let Some(user_home) = xai_yis_config::user_yis_home() else {
        return false;
    };
...
/// Find all `.yis/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.yis/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.yis/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.yis/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".yis").join("config.toml"))
...
mod tests {
    ...
    fn find_project_configs_excludes_user_grok_config_file() {
        ...
        std::fs::create_dir_all(project.join(".yis")).unwrap();
        std::fs::write(project.join(".yis/config.toml"), "# project\n").unwrap();
