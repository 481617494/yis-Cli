...
/// Directory (under `<cwd>/.yis`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.yis/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
