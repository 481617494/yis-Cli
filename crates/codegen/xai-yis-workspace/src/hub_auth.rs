//! Hub [`AuthProvider`] from `~/.yis/auth.json` for the standalone
...
//!
//! The in-leader `yis workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grok = xai_yis_config::user_yis_home()
        .ok_or_else(|| anyhow::anyhow!("no user yis home (set $YIS_HOME or $HOME)"))?;
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `yis login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `yis login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.yis/auth.json`).
pub fn provider(
    hub_url: &Url,
