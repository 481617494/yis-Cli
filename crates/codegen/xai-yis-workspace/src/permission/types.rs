...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "yis-shell", alias = "yis_shell")]
    Generic,
    /// Yis TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "yis-tui", alias = "grok_tui")]
    YisTUI,
    /// Yis Web client - identified by clientIdentifier "yis-web"
    #[serde(rename = "yis_web", alias = "grok_web")]
    YisWeb,
    ...
    Extension,
    /// Yis Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to YisTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"yis-pager"` (what the pager actually
    ...
    #[serde(rename = "yis-pager", alias = "grok_pager")]
    YisPager,
    /// Yis Desktop (Electron) client - identified by clientIdentifier "grok-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "yis_desktop", alias = "grok_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `yis-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "yis-shell",
            Self::YisTUI => "yis-tui",
            Self::YisWeb => "grok-web",
            Self::Nebula => "nebula",
            Self::Extension => "grok-code-extension",
            Self::YisPager => "yis-pager",
...
impl From<&xai_yis_tools::types::ToolInput> for AccessKind {
    fn from(input: &xai_yis_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.yis/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use xai_yis_tools::implementations::yis_build_hashline::edit::types::HashlineEditInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use xai_yis_tools::implementations::yis_build::bash::BashToolInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use xai_yis_tools::implementations::use_tool::UseToolInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use xai_yis_tools::implementations::yis_build::monitor::types::MonitorInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use xai_yis_tools::implementations::yis_build::web_fetch::WebFetchInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use xai_yis_tools::implementations::yis_build::web_search::WebSearchInput;
        use xai_yis_tools::types::ToolInput;
    ...
    fn client_type_deserializes_grok_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("yis-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("yis_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
