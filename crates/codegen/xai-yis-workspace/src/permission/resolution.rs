//! Permission resolution engine: merges native `.yis/config.toml`,
...
/// (fail-safe) with a warn + skip record for `yis inspect`.
...
/// user `~/.yis` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    xai_yis_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.yis/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.yis/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `xai-yis-shell::config::find_project_configs`.
fn find_project_grok_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".yis").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".yis").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
///
///   * `~/.yis/config.toml` (lowest priority)
///   * Each `.yis/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.yis/config.toml` first (lowest priority within this layer).
    // Gated on user_yis_home() so a project's .yis/config.toml is never read as
    // global permissions when neither YIS_HOME nor a home dir resolves.
    if let Some(global_path) = xai_yis_config::user_yis_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match xai_yis_config::load_config_file(&global_path) {
    ...
    for path in find_project_grok_configs(cwd) {
        match xai_yis_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[xai_yis_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `yis inspect`.
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &xai_yis_config::managed_config_layers(),
            ),
        }
    }
}
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `yis inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in find_claude_settings_paths(cwd) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `yis inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `yis inspect` via the outer resolver.
...
/// [`xai_yis_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = xai_yis_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = xai_yis_config::requirements_layers();
...
/// without `~/.yis`); `path` only names the layer in a non-bool warning.
...
pub const MANAGED_MCP_PREFIX: &str = "yis_com_";

/// Max `char` length of a managed runtime name (`yis_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `yis_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `YIS_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // YIS_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("YIS_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_YIS_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate YIS_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate YIS_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "yis_com_foo"));
        assert!(mcp_name_matches("yis_com_foo", "foo"));
        assert!(mcp_name_matches("yis_com_foo", "yis_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "yis_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "yis_com_slack"));
        assert!(mcp_name_matches("My Server", "yis_com_my_server"));
        assert!(mcp_name_matches("yis_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("yis_com_", "yis_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        for unrelated in ["foobar", "yis_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("yis_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("yis_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("yis_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "yis_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("yis_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("yis_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/yis/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/yis/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.yis/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/yis/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.yis/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/yis/managed_config.toml".into(),
        }));
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            xai_yis_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            xai_yis_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
