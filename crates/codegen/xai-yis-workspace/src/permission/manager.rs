...
use xai_yis_tools::implementations::yis_build::web_fetch::{
...
fn permission_mode_artifact_str(mode: xai_yis_telemetry::enums::PermissionMode) -> &'static str {
    use xai_yis_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.yis/sessions/<cwd>/permission.toml. This caused
            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && xai_yis_sandbox::should_auto_allow_bash()
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / YisWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `GrokTUI | YisPager | Desktop`).
    ...
    async fn auto_classifier_block_prompt_still_offers_always_approve() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // YisPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::YisPager);
                mgr.set_auto_mode(true);
