...
//! `.yis`, `.cursor`, and `.claude` vendor dirs, spawns a real `grok agent
//! stdio` process against the mock inference server (toggling the
//! `YIS_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
...
//! ```bash
//! cargo test -p xai-yis-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_YIS_SKILL: &str = "ZZ_YIS_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".yis", "grok-skill", MARKER_YIS_SKILL);
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_YIS_SKILL),
...
/// `YIS_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_YIS_SKILL),
...
/// `YIS_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_YIS_SKILL),
            "grok-skill always present"
        );
...
/// `YIS_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `YIS_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `YIS_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `YIS_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("YIS_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("YIS_CURSOR_SKILLS_ENABLED", "false"),
            ("YIS_CURSOR_RULES_ENABLED", "false"),
            ("YIS_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_YIS_SKILL), "grok always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("YIS_CLAUDE_SKILLS_ENABLED", "false"),
            ("YIS_CLAUDE_RULES_ENABLED", "false"),
            ("YIS_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
