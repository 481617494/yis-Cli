...
use xai_yis_shell::auth::{AuthManager, AuthMode, YisAuth, YisComConfig};
use xai_yis_shell::heap_profile::{
...
use xai_yis_test_support::{EnvGuard, MockInferenceServer};
use xai_yis_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = YisComConfig::default().auth_scope();
    let auth = YisAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[xai_yis_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), YisComConfig::default()));
