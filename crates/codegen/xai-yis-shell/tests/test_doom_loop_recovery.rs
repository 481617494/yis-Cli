...
use xai_yis_sampler::RetryPolicy;
use xai_yis_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
use xai_yis_shell::sampling::{
...
use xai_yis_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// ```bash
/// cargo test -p xai-yis-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = xai_yis_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let yis_home = home.path().join(".yis");
    std::fs::create_dir_all(&yis_home).expect("create .yis home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(xai_yis_test_support::yis_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_yis_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("YIS_HOME", yis_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("YIS_LEADER_SOCKET");

    let result = xai_yis_test_support::run_headless_with_cmd(cmd).await;
    xai_yis_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
