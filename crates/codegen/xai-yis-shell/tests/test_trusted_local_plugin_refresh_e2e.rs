...
//! 4. Smoke-validate session JSON under `$YIS_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`YIS_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-yis-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-yis-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_yis_agent::plugins::SharedPluginRegistryHandle;
use xai_yis_agent::plugins::discovery::DiscoveryConfig;
use xai_yis_agent::plugins::git_install::{InstallSource, install_from_source};
use xai_yis_agent::plugins::install_registry::{
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("YIS_HOME", &yis_home);
    ...
    let mut registry = InstallRegistry::empty(yis_home.join("installed-plugins"));
    ...
    // yis_home() caches the first YIS_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_yis_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/YIS_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via yis_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _grok_guard = EnvVarGuard::set("YIS_HOME", &yis_home);
    ...
    // Smoke: session storage under YIS_HOME/sessions has JSON artifacts.
