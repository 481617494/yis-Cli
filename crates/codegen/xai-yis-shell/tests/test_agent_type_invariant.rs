...
//! Run locally:
//! ```bash
//! cargo test -p xai-yis-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "yis-build")
/// - `model-b`: no agent_type (→ "yis-build")
...
#[tokio::test]
#[ignore]
async fn test_default_model_uses_yis_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Grok") || sys_prompt.contains("yis"),
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_grok =
            original_sys_prompt.contains("Grok") || original_sys_prompt.contains("yis");
        let resumed_has_grok =
            resumed_sys_prompt.contains("Grok") || resumed_sys_prompt.contains("yis");
        assert_eq!(
            original_has_grok,
            resumed_has_grok,
...
/// default to `"yis-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_yis_build() {
    with_local_set(|| async {
...
/// The `YIS_AGENT` escape hatch should override the model's agent_type.
/// Setting `YIS_AGENT=grok-build` with an alternate-agent model should use
/// grok-build harness.
#[tokio::test]
#[ignore]
async fn test_yis_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            xai_yis_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("YIS_AGENT", "yis-build");
            ...
            assert!(
                sys_prompt.contains("Grok") || sys_prompt.contains("yis"),
                "YIS_AGENT=grok-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
