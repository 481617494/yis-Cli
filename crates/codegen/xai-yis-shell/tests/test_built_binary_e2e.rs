//! Built-binary end-to-end tests for the grok (xai-yis-pager) binary.
//!
...
//! ```bash
//! cargo test -p xai-yis-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `YIS_BINARY` to point at the release artifact:
//! ```bash
//! YIS_BINARY=./artifacts/grok-0.1.159-linux-x86_64 \
//!   cargo test -p xai-yis-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use xai_yis_test_support::env::test_env_cmd_tokio;
...
async fn yis_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("grok-4.5", "yis-build")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("YIS_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "grok --version with YIS_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = yis_build_server().await;
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Yis Cli usage limit"),
...
async fn test_headless_json_reports_server_cost() {
    use xai_yis_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_yis_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        xai_yis_test_support::scripted::ScriptedResponse::sse(
            xai_yis_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
/// Isolated headless run with a custom `~/.yis/`. Clean env (no leaked
/// host credentials). Write config files into `yis_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".yis")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("YIS_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("YIS_TELEMETRY_ENABLED".into(), "false".into()),
                ("YIS_FEEDBACK_ENABLED".into(), "false".into()),
                ("YIS_TRACE_UPLOAD".into(), "false".into()),
                ("YIS_INSTRUMENTATION".into(), "disabled".into()),
                ("YIS_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn yis_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".yis")
    }
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    ...
    std::fs::write(
        h.yis_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
...
"#,
    ...
    .unwrap();
    h.env("YIS_TEST_BYOK_TOKEN", "test-byok-secret-token");
