...
/// All tests share this as YIS_HOME (the `OnceLock` in xai-yis-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = xai_yis_shell::config::load_effective_config().unwrap();
    let effective_cfg = xai_yis_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    xai_yis_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = xai_yis_shell::util::config::load_config_from_toml(&user_toml);
