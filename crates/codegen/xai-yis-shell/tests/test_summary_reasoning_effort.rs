...
//! Run locally:
//! ```bash
//! cargo test -p xai-yis-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.yis/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".yis").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let yis_dir = home.path().join(".yis");
        std::fs::create_dir_all(&yis_dir).expect("create .yis dir");
        std::fs::write(
