...
//! caller-owned `$YIS_HOME`, then inspects `~/.yis/debug/`:
...
//!   `~/.yis/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `YIS_LOG_SAMPLING`/`YIS_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `YIS_DEBUG_LOG=1`) writes a per-session
//!   `~/.yis/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.yis/debug/` files).
//! - `YIS_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p xai-yis-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$YIS_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".yis").join("debug")
}

/// List firehose `*.txt` files under `~/.yis/debug` (excluding the `latest.txt`
...
/// Build a headless `yis -p` command with a pinned `$YIS_HOME` so the firehose
/// lands under `<home>/.yis/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    let mut cmd = tokio::process::Command::new(yis_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    xai_yis_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("YIS_HOME", home.join(".yis"));
    cmd.env_remove("YIS_DEBUG_LOG");
    cmd.env_remove("YIS_LOG_FILE");
    cmd.env_remove("YIS_LOG_SAMPLING");
    cmd.env_remove("YIS_HOOKS_LOG");
    cmd
}
...
/// assert it carries first-party (`xai_yis`) content. Panics with the captured
...
/// headless `yis -p` client is near-silent, so its lazily-opened firehose may
...
/// A live `agent` session writes `~/.yis/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let yis_home = home.path().join(".yis");
        let yis_home_str = yis_home.to_string_lossy().into_owned();

        let client = GrokStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("YIS_DEBUG_LOG", "1"), ("YIS_HOME", &yis_home_str)],
        )
        .await;
        ...
        let session_file = yis_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.yis/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = yis_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `YIS_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `YIS_LOG_SAMPLING`/`YIS_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears YIS_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.yis/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // YIS_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = yis_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.yis/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `YIS_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn yis_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    cmd.env("YIS_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok YIS_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit YIS_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "YIS_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
