...
//! ```bash
//! cargo test -p xai-yis-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // GrokStdioClient sets HOME=<temp>; the binary uses <HOME>/.yis as YIS_HOME.
        let yis_home = shared_home.path().join(".yis");
        let session_dir = locate_session_dir(&yis_home, session_id.0.as_ref());
