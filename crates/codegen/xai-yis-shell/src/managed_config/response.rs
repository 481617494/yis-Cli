...
use xai_yis_config::signed_policy::now_unix;
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that YIS_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `yis login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.yis. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<xai_yis_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        &self,
    ) -> Option<xai_yis_config::signed_policy::SignatureEnvelope> {
        self.signature_sidecar_with(xai_yis_config::signed_policy::embedded_key_id_trusted)
    }
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: xai_yis_config::signed_policy::SignatureEnvelope,
    pub(super) payload: xai_yis_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use xai_yis_config::signed_policy;
...
mod tests {
    ...
    fn signature_sidecar_picks_trusted_envelope_then_falls_back() {
        use xai_yis_config::signed_policy::SignatureEnvelope;
