...
use xai_yis_agent::plugins::git_install::{self, UpdateStatus};
use xai_yis_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use xai_yis_plugin_marketplace::git::{self, SourceCacheLease};
use xai_yis_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.yis/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &xai_yis_agent::plugins::install_registry::MarketplaceProvenance,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &xai_yis_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.yis/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user grok via user_yis_home() (None when no home resolves) and
    // home separately, so removal still runs from $YIS_HOME when no home dir
    // exists, and never touches a cwd-relative .yis.
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> xai_yis_workspace::permission::resolution::MarketplaceAllowlist {
        xai_yis_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = xai_yis_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
