...
use xai_yis_telemetry::id::agent_id;
...
/// hot-reload watcher when `<cwd>/.yis/config.toml`,
...
/// Hot-reload the model catalog from `~/.yis/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .yis/plugins under it, plus
