...
//!
//! Fetches the authenticated user's Yis Cli billing configuration
...
/// Current billing configuration for Yis Cli coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `YisBuildBillingConfig` fields
...
/// `GetYisBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// absent (legacy `GetYisBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
///
/// Keeps history to a count + the most recent period so `~/.yis/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `yis login` to authenticate.",
    )?;
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            crate::auth::YisComConfig::default().token_header,
        )
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `yis login` to authenticate.",
    )?;
    ...
    fn billing_config_deserializes_credits_config_shape() {
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_YIS_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
