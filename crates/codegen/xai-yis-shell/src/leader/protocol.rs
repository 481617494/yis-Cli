...
pub struct LeaderCapabilities {
    ...
    /// (driven by `yis update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `yis update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `yis update` just installed; the leader uses
