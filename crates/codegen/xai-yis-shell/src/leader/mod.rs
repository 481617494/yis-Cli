//! Leader-follower IPC architecture for yis-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.yis/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use xai_yis_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::YisBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = xai_yis_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`yis -p`).
...
pub struct LeaderEnvUrls {
    pub yis_ws_url: String,
    pub yis_ws_origin: String,
}
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<YisBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(YisBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<YisBuildEnvironment> {
    let environments: &[YisBuildEnvironment] = &[YisBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<YisBuildEnvironment> {
    let environments: &[YisBuildEnvironment] = &[YisBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::yis_home::yis_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : xai_yis_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::YisComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::YisComConfig) -> Self {
        Self {
            yis_ws_url: c.yis_ws_url.clone(),
            yis_ws_origin: c.yis_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`yis -p` where hanging forever is unacceptable.
...
///     "yis-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.yis_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "yis-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.yis_ws_url);
...
///
/// For a **managed install** — the running binary lives under `yis_home`
/// (e.g. `~/.yis/...`) — prefer the managed `~/.yis/bin/grok` symlink. After an
/// auto-update or `yis update` atomically swaps that symlink, `current_exe()`
...
/// `xai_yis_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `yis_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.yis/bin/grok` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::yis_home::yis_home())
}
fn resolve_binary_with_home(yis_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(yis_home, std::env::current_exe().ok())
}
/// Binary file name for the managed grok install (`grok` / `grok.exe`).
fn managed_grok_bin_name() -> &'static str {
    if cfg!(windows) { "grok.exe" } else { "yis" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = yis_home.join("bin").join(managed_grok_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, yis_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--grok-ws-url").arg(&env_urls.yis_ws_url);
    cmd.arg("--grok-ws-origin").arg(&env_urls.yis_ws_origin);
    ...
    let log_path = crate::util::yis_home::yis_home().join("leader.log");
    ...
    let leader_log = std::env::var("YIS_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "xai_yis_shell=info,xai_acp_lib=warn,xai_yis_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            yis_ws_url: "wss://test.invalid".into(),
            yis_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("yis"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("yis")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
