...
use xai_yis_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "yis-tui", "grok-code-extension").
...
impl AuthProvider for LeaderAuthProvider {
    ...
    /// read. Mirrors the in-process path (`mvp_agent`): prefer `YisAuth.team_id`
...
async fn handle_workspace_start(
    ...
    let status_config = xai_yis_workspace::StatusConfig::from_env();
    ...
    let upload_queue_enabled =
        std::env::var("YIS_WORKSPACE_UPLOAD_QUEUE_ENABLED").as_deref() != Ok("false");
    ...
    let handle = xai_yis_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
...
pub async fn run_leader_server(
    ...
    loop {
        tokio::select! {
            ...
            xai_yis_telemetry::unified_log::info("leader.client.registered", None,
            ...
            xai_yis_telemetry::unified_log::info("leader.client.disconnected", None,
            ...
            xai_yis_telemetry::unified_log::warn("leader.response.orphaned", None,
            ...
            xai_yis_telemetry::unified_log::warn("leader.response.send_failed", None,
...
mod tests {
    ...
    async fn relay_demand_signals_only_on_headless_registration() {
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let _stdio =
            connect_and_register_with_mode(&sock_path, "yis-tui", ClientMode::Stdio).await;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "yis-tui",
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "yis-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "yis-tui", ClientId(42));
        assert_eq!(
    ...
    fn inject_yolo_notification_adds_client_identifier() {
        ...
        assert_eq!(json["params"]["clientIdentifier"], "yis-tui");
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "yis-tui").await;
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_x.ai/mcp/servers_updated","params":{"method":"x.ai/mcp/servers_updated","params":{"mcpServers":[{"name":"yis_com_slack","source":"managed"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a
                .as_deref()
                .is_some_and(|p| p.contains("yis_com_slack")),
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"yis-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "yis-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "yis-tui", ClientId(2));
        assert_eq!(
