...
//! [`xai_yis_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xai_yis_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xai_yis_agent::Agent`] owns an [`xai_yis_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`xai_yis_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use xai_yis_agent::config::AgentDefinition;
use xai_yis_agent::error::AgentBuildError;
use xai_yis_agent::prompt::context::PromptAudience;
use xai_yis_agent::prompt::skills::SkillsConfig;
use xai_yis_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use xai_yis_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use xai_yis_tools::implementations::yis_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<xai_yis_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<xai_yis_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use xai_yis_tools::implementations::yis_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(xai_yis_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(xai_yis_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use xai_yis_tools::implementations::yis_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            xai_yis_tools::computer::local::LocalTerminalBackend::new_local(
                xai_yis_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: xai_yis_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(xai_yis_tools::types::tool::ToolKind::Task)
            .expect("YisBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("YisBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_yis_build())
                    .await
