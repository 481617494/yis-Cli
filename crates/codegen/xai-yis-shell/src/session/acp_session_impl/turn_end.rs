...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            xai_yis_workspace::session::git::get_worktree_info(cwd),
            xai_yis_workspace::session::git::get_branch(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<xai_yis_tools::implementations::yis_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_install_error(
        err: &xai_yis_agent::plugins::install_registry::InstallError,
    ) -> String {
