...
impl SessionActor {
    ...
    /// (`YIS_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`yis-build-0.1`). Every
    ...
    /// `yis-build-0.1` for OAuth users) the request is **skipped
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        tracing::debug!(
            raw_preview = %xai_yis_tools::util::truncate_str(raw.trim(), 60),
