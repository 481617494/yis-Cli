...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: xai_yis_tools::implementations::yis_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`YIS_GOAL_PLANNER`) and a plan
...
/// Override with `YIS_GOAL_REVERIFY_AFTER` (floored at 1).
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use xai_yis_tools::implementations::yis_build::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                xai_yis_tools::implementations::yis_build::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
