...
use xai_yis_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use xai_yis_agent::prompt::skills::SkillsConfig;
use xai_yis_sampler::SamplerConfig as SamplingConfig;
use xai_yis_sampling_types::truncate_bytes;
...
use xai_yis_tools::types::compat::CompatConfig;
use xai_yis_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use xai_yis_workspace::file_system::CodebaseIndexManager;
use xai_yis_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use xai_yis_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
impl xai_yis_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        xai_yis_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `xai-yis-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<xai_yis_sampler::SharedAttributionCallback>,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<xai_yis_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<xai_yis_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<xai_yis_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<xai_yis_agent::Agent>,
    ...
    /// Kill-switch (`YIS_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<xai_yis_tools::implementations::yis_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, xai_yis_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<xai_yis_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<xai_yis_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<xai_yis_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `xai-yis-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: xai_yis_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`xai_yis_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`xai_yis_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: xai_yis_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == xai_yis_tools::implementations::yis_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<xai_yis_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &xai_yis_agent::PromptContext) {
...
) -> Option<xai_yis_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use xai_yis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_yis_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl xai_yis_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use xai_yis_tools::registry::types::ToolConfig;
    use xai_yis_tools::tool_taxonomy::TOOL_META_KEY;
    use xai_yis_workspace::permission::PermissionCommand;
    ...
    async fn prepare_tool_call_stamps_early_tool_call_and_refinement() {
        ...
        local
            .run_until(async {
                ...
                assert_eq!(t["namespace"], "yis_build");
...
mod managed_gateway_tool_tests {
    ...
    use xai_yis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_yis_tools::types::tool::{ToolKind, ToolNamespace};
    use xai_yis_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<xai_yis_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
