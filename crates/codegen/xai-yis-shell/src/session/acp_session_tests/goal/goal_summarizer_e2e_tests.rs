...
//!
//! Tests mutate `YIS_GOAL_CLASSIFIER` so they carry `serial`.
...
use xai_yis_tools::implementations::yis_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<xai_yis_tools::implementations::yis_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        xai_yis_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
