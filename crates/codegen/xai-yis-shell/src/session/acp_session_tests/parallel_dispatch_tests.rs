...
//!
//! These tests verify the parallel dispatch path (YIS_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and yis_build's
...
#[test]
fn lock_path_for_args_matches_yis_build_file_path() {
    // yis_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_yis_build_target_file() {
    // yis_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_yis_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. yis_build search_replace + StrReplace
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use xai_yis_sampling_types::{ConversationItem, SyntheticReason};
