...
/// attribution.
fn auth_error() -> xai_yis_sampler::SamplingErrorInfo {
    xai_yis_sampler::SamplingErrorInfo {
        kind: xai_yis_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> xai_yis_sampler::SamplingErrorInfo {
    xai_yis_sampler::SamplingErrorInfo {
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `yis login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> xai_yis_sampler::SamplingErrorInfo {
    xai_yis_sampler::SamplingErrorInfo {
            kind: xai_yis_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=xai-yis-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
