...
//! (`xai_yis_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](xai_yis_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](xai_yis_compaction::FullReplaceError).
...
use xai_yis_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use xai_yis_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    xai_yis_telemetry::session_ctx::log_event(CompactionRetryDegraded {
