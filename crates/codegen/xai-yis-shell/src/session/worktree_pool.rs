...
//!   under `~/.yis/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.yis/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.yis/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.yis/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.yis/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.yis/`.
...
/// `CleanAll` mode. Placed under `~/.yis/worktree_pool/<instance_id>/<uuid>/`.
