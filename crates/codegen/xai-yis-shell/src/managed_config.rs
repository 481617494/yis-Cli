...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        xai_yis_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
/// would just 401).
fn eligible_team_principal(auth: YisAuth) -> Option<YisAuth> {
...
fn read_active_team_auth() -> Option<YisAuth> {
    let home = crate::util::yis_home::yis_home();
...
/// files (TUI tick vs `yis login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `yis setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `YIS_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("YIS_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xai-yis-http` classifier.
...
/// Override with `YIS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("YIS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** YisBuildDeployment UUID (the id
...
/// Resolve deployment key from `YIS_DEPLOYMENT_KEY` env var, then config files.
...
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("YIS_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.yis/`, trying the
...
struct SyncOutcome {
    ...
    /// `yis setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
fn apply_fetched(
    ...
    let verified = if xai_yis_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(xai_yis_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        xai_yis_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
/// Post-login hook for `yis login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<YisAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `yis setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(YisAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// `yis setup`); ceasing to serve `fail_closed` rolls back.
pub fn managed_policy_gate() -> Result<(), String> {
    // Skip under the lib unit-test build only: `bootstrap` reaches this without a staged
    // `YIS_HOME` and would flake on the dev machine's real marker/auth. The pure decision
...
/// Outcome of the `yis setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.yis`.
    Installed,
...
/// exactly as `yis setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `yis setup` would install them.
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `yis setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && xai_yis_config::signed_policy::verification_active()
...
/// Run the `yis setup` sync for the current principal. The caller must check
