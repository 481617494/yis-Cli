...
use xai_yis_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use xai_yis_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `xai-yis-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use xai_yis_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use xai_yis_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use xai_yis_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.yis/config.toml` at each level.
/// Project-scoped `.yis/config.toml` entries override global `~/.yis/config.toml`
...
/// `.yis/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.yis/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.yis/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.yis/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .yis/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.yis/config.toml`.
...
/// Upsert an MCP server entry in `~/.yis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.yis/config.toml`.
...
/// Delete an MCP server entry from `~/.yis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.yis/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = xai_yis_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.yis/config.toml` files.
...
// `.mcp.json` discovery moved to `xai-yis-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use xai_yis_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.yis/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::yis_home::yis_home().join("config.toml")
}

/// Path to the user-level config file (`~/.yis/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.yis/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".yis").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.yis/skills", "~/.yis/skills/special/SKILL.md"]
ignore = ["~/.yis/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.yis/skills", "~/.yis/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.yis/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${YIS_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&yis_dir).unwrap();
        std::fs::write(
            yis_dir.join("config.toml"),
            r#"
