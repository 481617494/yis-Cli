...
pub fn resolve_zdr_access_enabled(
    ...
    BoolFlag::env("YIS_ZDR_ACCESS_ENABLED")
...
///
/// Yis / air-gap: `YIS_LOCAL_MODE` (default on) and `YIS_OFFLINE=1` disable
/// remote catalog + settings fetches so startup does not hang on xAI endpoints.
/// Set `YIS_LOCAL_MODE=0` to restore upstream cloud fetches.
///
/// Config layers still apply when local mode is off.
...
/// Check if local / privacy mode is active.
///
/// Canonical source: [`xai_yis_env::is_local_mode`] (via shell-base re-export).
/// Release builds always return true; see that function for details.
pub fn is_local_mode() -> bool {
    crate::env::is_local_mode()
}

/// Whether storage uploads (GCS via cli-chat-proxy) are allowed.
/// Always disabled in local mode.
pub fn resolve_storage_upload_enabled() -> bool {
    !is_local_mode()
}

/// Whether telemetry (Mixpanel, product events, OTLP traces) is enabled.
/// Always disabled in local mode.
pub fn resolve_telemetry_enabled() -> bool {
    !is_local_mode()
}

/// Whether relay WebSocket sync is enabled.
/// Always disabled in local mode.
pub fn resolve_relay_enabled() -> bool {
    !is_local_mode()
}

/// Whether managed config fetching is enabled.
/// Always disabled in local mode.
pub fn resolve_managed_config_enabled() -> bool {
    !is_local_mode()
}

/// Whether backend CRUD (code.grok.com session storage) is enabled.
/// Always disabled in local mode.
pub fn resolve_backend_crud_enabled() -> bool {
    !is_local_mode()
}

/// Whether Computer Hub workspace exposure is enabled.
/// Always disabled in local mode.
pub fn resolve_computer_hub_enabled() -> bool {
    !is_local_mode()
}
