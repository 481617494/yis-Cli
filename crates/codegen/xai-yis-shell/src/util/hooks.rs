...
use xai_yis_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .yis/hooks/ directories are still scanned (they hold
    ...
    // user_yis_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .yis.
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".yis").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    xai_yis_hooks::discovery::HookRegistry,
    Vec<xai_yis_hooks::error::HookError>,
) {
