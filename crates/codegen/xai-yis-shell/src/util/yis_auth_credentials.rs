...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: xai-yis-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct YisAuthCredentials {
...
impl std::fmt::Debug for YisAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("YisAuthCredentials")
            .field(
                "user_token",
...
impl YisAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your YIS_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `yis login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> YisAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> YisAuthCredentials {
...
impl xai_yis_auth::HttpAuth for YisAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        YisAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = YisAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = YisAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = YisAuthCredentials::new(Some("static-token".into()));
