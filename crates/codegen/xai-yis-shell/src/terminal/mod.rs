...
/// Resolved absolute path to bash. On Unix uses the `xai_yis_config` shell
/// resolution cascade (`$YIS_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_yis_config::shell::unix_shell_path(xai_yis_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-yis-tools`.
pub use xai_yis_tools::util::pager_env;
...
/// xai-yis-pager which renders its own UI and doesn't need raw ANSI codes).
///
