...
//!       [--api-key <key> | $XAI_API_KEY | <yis-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use xai_yis_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// `$XAI_API_KEY`, then `<yis-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$YIS_HOME` or `~/.yis`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    yis_home: Option<PathBuf>,
}
...
mod tests {
    ...
    #[test]
    fn cli_yis_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.yis_home, Some(PathBuf::from("/tmp/scratch-grok")));
    }
