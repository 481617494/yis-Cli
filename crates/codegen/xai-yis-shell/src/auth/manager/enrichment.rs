...
use crate::auth::model::{YisAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: YisAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.yis_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut YisAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: YisAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut YisAuth, user_info: UserInfo) {
