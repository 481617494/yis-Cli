...
use super::super::config::{ForceLoginTeam, YisComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, YisAuth};
use base64::Engine;
...
pub fn is_configured(config: &YisComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &YisComConfig) -> Option<ForceLoginTeam> {
...
/// xAI 官方 IdP issuer（生产默认）。
const XAI_AUTH_ISSUER: &str = "https://auth.x.ai";

/// 写死的 xAI OIDC discovery 文档（来自
/// `https://auth.x.ai/.well-known/openid-configuration`，2026-07 抓取）。
///
/// 国内网络访问 `auth.x.ai` 的 discovery 常失败/超时，导致启动报错：
/// `error sending request for url (.../openid-configuration)`。
/// 对官方 issuer **直接返回内置文档**，不再发 discovery HTTP。
///
/// 注意：token 刷新仍会访问 `token_endpoint`；纯 BYOK 本地模式请用
/// `/model-add` 配置自有 API，并尽量避免依赖 xAI OIDC 登录。
fn baked_xai_discovery() -> Discovery {
    Discovery {
        authorization_endpoint: "https://auth.x.ai/oauth2/authorize".into(),
        token_endpoint: "https://auth.x.ai/oauth2/token".into(),
        jwks_uri: Some("https://auth.x.ai/.well-known/jwks.json".into()),
        id_token_signing_alg_values_supported: Some(vec!["ES256".into()]),
    }
}

fn is_xai_auth_issuer(issuer_key: &str) -> bool {
    let k = issuer_key.trim_end_matches('/');
    k.eq_ignore_ascii_case(XAI_AUTH_ISSUER)
        || k.eq_ignore_ascii_case("http://auth.x.ai")
        || k.eq_ignore_ascii_case("https://auth.x.ai/")
}
...
pub(super) async fn discover(issuer: &str) -> anyhow::Result<Discovery> {
    ...
    // 官方 xAI IdP：内置 discovery，跳过网络（避免国内无梯子超时）
    if is_xai_auth_issuer(&issuer_key) {
        let doc = baked_xai_discovery();
        DISCOVERY_CACHE
            .write()
        ...
        tracing::debug!(
            ...
            "OIDC: using baked-in xAI discovery (no network)"
        );
        return Ok(doc);
    }
    ...
    {
        Ok(doc) => doc,
        // 非 xAI 的 issuer 网络失败时仍报错；xAI 已在上方短路
        Err(e) => return Err(e),
    };
    DISCOVERY_CACHE
        .write()
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "yis-cli:access".into()],
            audience: None,
        };
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access yis-cli:access api:access", "principal_type" : "Team",
