...
use super::super::config::{YisComConfig, OidcAuthConfig};
use super::super::{AuthManager, YisAuth};
use super::protocol::{
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to Yis Cli.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.yis_com_config().oauth2.as_ref();
    ...
    let principal_policy = login_principal_policy(auth_manager.yis_com_config());
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), YisComConfig::default())
