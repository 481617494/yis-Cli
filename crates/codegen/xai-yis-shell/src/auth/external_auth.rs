use crate::auth::{AuthMode, YisAuth};
...
pub(crate) struct ExternalAuthOutput {
    ...
    /// see [`YisAuth::is_xai_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `YisAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<YisAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<YisAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &YisAuth) -> Option<YisAuth> {
...
mod tests {
    ...
    fn sync_sets_grok_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $YIS_AUTH_EXPIRED", true).unwrap();
