...
impl std::fmt::Debug for YisAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("YisAuth")
...
impl YisAuth {
    /// Create an empty placeholder credential for local mode — no auth
    /// required, no network calls made.
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &YisAuth) {
...
#[cfg(test)]
impl YisAuth {
    /// Returns a `YisAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// YisAuth { key: "my-key".into(), ..YisAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, YisAuth>;
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<YisAuth> {
...
/// Early-invalidation buffer. Override with `YIS_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("YIS_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &YisAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &YisAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> YisAuth {
        YisAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn is_xai_auth_matrix() {
        ...
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| YisAuth {
