...
use crate::auth::{AuthManager, YisAuth, YisComConfig, parse_output};
use crate::util::yis_home;
...
/// session.
fn is_cached_credential_compatible(auth: &YisAuth, yis_com_config: &YisComConfig) -> bool {
    let expected_issuer = yis_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| yis_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(yis_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("YIS_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `YIS_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `yis_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("YIS_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.yis_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = yis_com_config.oidc.is_some(),
        has_oauth2 = yis_com_config.oauth2.is_some(),
        has_external_auth = yis_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, yis_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, yis_com_config)
        }) {
            xai_yis_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = yis_com_config.auth_provider_command {
    ...
    if !yis_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / YIS_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(yis_com_config) {
        return crate::auth::oidc::run_login_flow(yis_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = yis_com_config.oauth2 {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `yis login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(yis_com_config: &YisComConfig) -> Option<YisAuth> {
    let yis_home = yis_home::yis_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&yis_home, yis_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(yis_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(yis_com_config).await {
        return Some(auth);
    }
    let yis_home = yis_home::yis_home();
    let auth_manager = Arc::new(AuthManager::new(&yis_home, yis_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, yis_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<YisAuth> {
    auth_manager
        .current_or_expired()
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = yis_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: YisAuth) -> YisAuth {
...
fn report_signed_in(auth: &YisAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
) -> anyhow::Result<YisAuth> {
    // 本地模式：无需认证，返回一个空的 YisAuth
    if crate::util::config::is_local_mode() {
        return Ok(YisAuth::empty_local());
    }
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<YisAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(yis_com_config).await)
    } else {
        ensure_authenticated(yis_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `yis login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `YIS_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // 本地模式：无需登录
    if crate::util::config::is_local_mode() {
        eprintln!("本地模式无需登录。设置 YIS_LOCAL_API_BASE_URL 指向本地大模型（默认 http://localhost:11434/v1）即可使用。");
        return Ok(());
    }
    ...
    // supports the device flow. Without this guard, `yis login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.yis_com_config, login_override).await {
        if config.yis_com_config.oauth2.is_none() {
        ...
        let yis_home = yis_home::yis_home();
        let auth_manager = Arc::new(AuthManager::new(&yis_home, config.yis_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.yis_com_config) {
            eprintln!(
    ...
    // config was actually applied; `yis setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `YIS_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_yis_telemetry::external::set_identity(
            xai_yis_telemetry::external::IdentityAttrs::default(),
        );
        xai_yis_telemetry::external::flush();
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    ...
    let yis_home = yis_home::yis_home();
    let auth_manager = AuthManager::new(&yis_home, config.yis_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `YIS_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("YIS_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("YIS_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("YIS_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A grok.com first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> YisAuth {
        YisAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
            refresh_token: refresh.map(str::to_string),
            ..YisAuth::test_default()
        }
    }
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), YisComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only yis_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), YisComConfig::default()));
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), YisComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("YIS_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("YIS_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `yis login` must use loopback, not device —
    ...
    fn oidc_auth(issuer: &str) -> YisAuth {
        YisAuth {
    ...
    fn pinned_cfg(team: &str) -> YisComConfig {
        YisComConfig {
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = YisComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = YisAuth {
        ...
        // YIS_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    /// CLI `yis login` passes `on_stderr=None`; stderr must be inherited so
