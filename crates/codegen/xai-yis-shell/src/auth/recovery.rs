...
use crate::auth::model::YisAuth;
use crate::auth::token_type::TokenType;
use xai_yis_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&YisAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        xai_yis_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<YisAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<YisAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<YisAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.yis_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<YisAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            xai_yis_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<YisAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<YisAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &YisAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, YisAuth};
    ...
    fn rejected_cred() -> Option<YisAuth> {
        Some(YisAuth {
    ...
    impl TokenRefresher for OkRefresher {
        async fn refresh(&self, _reason: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            ...
            RefreshOutcome::Success(Box::new(YisAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), YisComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<YisAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.yis_com_config().auth_scope();
        let fresh = YisAuth {
