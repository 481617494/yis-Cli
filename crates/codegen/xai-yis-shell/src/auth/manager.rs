...
use crate::auth::config::YisComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use xai_yis_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, YisAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<YisAuth>>>,
    path: PathBuf,
    scope: String,
...
impl AuthManager {
    pub fn new(yis_home: &Path, yis_com_config: YisComConfig) -> Self {
        let scope = yis_com_config.auth_scope();
        let proxy_base_url =
        ...
        xai_yis_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "yis_home": yis_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "YIS_HOME": std::env::var("YIS_HOME").unwrap_or_else(|_| "(unset)".into()),
                "YIS_AUTH_PATH": std::env::var("YIS_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "YIS_AUTH": std::env::var("YIS_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // YIS_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("YIS_AUTH") {
            if let Ok(auth) = serde_json::from_str::<YisAuth>(&inline_json) {
                return Self::assemble(
                    Some(auth),
                    yis_home.join("auth.json"),
                    scope,
                    yis_com_config,
                    proxy_base_url,
                    None,
            ...
            tracing::warn!("YIS_AUTH set but failed to parse as JSON, falling back to file");
        }

        // YIS_AUTH_PATH: custom file path (overrides default $YIS_HOME/auth.json).
        let path = std::env::var("YIS_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| yis_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &YisAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.yis_com_config)?;
    ...
    fn vet_cached(&self, auth: YisAuth) -> Option<YisAuth> {
    ...
    pub(crate) fn current(&self) -> Option<YisAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<YisAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&YisAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<YisAuth> {
    ...
    pub(crate) fn expired_auth(&self) -> Option<YisAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &YisAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &YisAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &YisAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.yis_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `YisAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: YisAuth) -> std::io::Result<YisAuth> {
        ...
        match &write_result {
            Ok(()) => xai_yis_telemetry::unified_log::info(
            ...
            Err(e) => xai_yis_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: YisAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut YisAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn yis_com_config(&self) -> &YisComConfig {
        &self.yis_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<YisAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: YisAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<YisAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> YisAuth {
        self.inner.read().clone().unwrap_or_else(|| YisAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: YisAuth) -> Option<YisAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<YisAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<YisAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<YisAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in YIS_AUTH mode).
            DiskAuthState::Ok => {
                xai_yis_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<YisAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<YisAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<YisAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.yis_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`yis login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.yis_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<YisAuth, AuthError> {
        if self.yis_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        xai_yis_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) fn is_different_token(&self, candidate: &YisAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<xai_yis_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[yis_com] auth_token_ttl` to drive a real schedule.
...
impl xai_yis_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](xai_yis_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](xai_yis_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> xai_yis_tools::types::SharedApiKeyProvider {
