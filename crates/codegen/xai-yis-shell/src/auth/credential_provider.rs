...
use crate::util::yis_auth_credentials::YisAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::YisAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_yis_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: YisAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = YisAuthCredentials::new(None);
...
///
/// - `"yis-shell"`   — classic Yis Cli / TUI (xai-yis-shell)
/// - `"yis-pager"`   — new Grok Pager / TUI (xai-yis-pager)
/// - `"grok-desktop"` — Yis Desktop app
/// - `"grok-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-yis-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_yis_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = YisAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = YisAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_yis_telemetry::external::set_identity(
            xai_yis_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-yis-pager` and `xai-yis-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> xai_yis_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let yis_com_config = crate::auth::YisComConfig::default();
    let exporter = xai_yis_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = yis_com_config.token_header.clone();
    let yis_home = crate::util::yis_home::yis_home();
    let bootstrap = Arc::new(AuthManager::new(&yis_home, yis_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    xai_yis_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::YisComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use xai_yis_auth::AuthCredentialProvider;
    /// Serializes tests that pin `YIS_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `YIS_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("YIS_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("YIS_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("YIS_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("YIS_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> YisAuth {
        YisAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<YisAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::YisComConfig::default(),
        ));
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(YisAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_yis_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    /// user token, matching the precedence in `YisAuthCredentials::apply`.
