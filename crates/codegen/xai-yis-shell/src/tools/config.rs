...
use xai_yis_sampler::SamplerConfig;
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `YIS_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> xai_yis_tools::implementations::yis_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("YIS_WEB_FETCH_PROXY"))
...
/// from `xai_yis_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `YisBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `YisBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "YisBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "YisBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"YisBuild:read_file"));
        assert!(ids.contains(&"YisBuild:search_replace"));
        assert!(ids.contains(&"YisBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"YisBuildHashline:hashline_read"));
        assert!(ids.contains(&"YisBuildHashline:hashline_edit"));
        assert!(ids.contains(&"YisBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            xai_yis_agent::config::AgentDefinition::plan(),
            xai_yis_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "YisBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"YisBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"YisBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"YisBuildHashline:hashline_edit"),
