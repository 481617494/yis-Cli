// Re-export all types from the standalone xai-yis-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_yis_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-yis-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-yis-sampler` crate can reference them without a circular
// dep on `xai-yis-shell`.

// Tests for the types now live in xai-yis-sampling-types crate.

use xai_yis_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-yis-sampling-types` or
/// `xai-yis-tools`) so neither of those crates needs to depend on
