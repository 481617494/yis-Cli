...
use crate::auth::{AuthManager, YisAuth};

const YIS_WEB_URL: &str = "https://grok.com";
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("YIS_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("YIS_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| YIS_WEB_URL.to_string());
        Self {
    ...
    async fn require_xai_auth(&self) -> Result<YisAuth, ConvError> {
    ...
    fn apply_auth_headers(
        ...
        let mut builder = builder
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.yis_com_config().token_header.clone(),
            )
