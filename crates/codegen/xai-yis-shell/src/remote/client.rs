...
use crate::auth::{YisAuth, YisComConfig};
...
const YIS_CODE_BACKEND_URL: &str = "https://code.grok.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const YIS_CODE_WEB_URL: &str = "https://grok.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("YIS_CODE_WEB_URL").unwrap_or_else(|_| YIS_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-XAI-Token-Auth", YisComConfig::default().token_header)
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::yis_auth_credentials::YisAuthCredentials::new(
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn xai_yis_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("YIS_CODE_BACKEND_URL")
                .unwrap_or_else(|_| YIS_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&YisAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn xai_yis_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<YisAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-XAI-Token-Auth so nginx auth subrequest routes to authenticate_xai_yis_cli_token.
    /// See: crates/codegen/xai-yis-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-XAI-Token-Auth",
            required(&YisComConfig::default().token_header, "X-XAI-Token-Auth")?,
        );
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-yis-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            ...
            HeaderValue::from_static(xai_yis_version::VERSION),
        );
        Ok(headers)
    }
...
/// Fetch `yis_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(xai_yis_telemetry::id::agent_id)
        .await
        .ok()?;
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| xai_yis_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(xai_yis_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            client_identifier: header_str(&headers, "x-yis-client-identifier"),
                            client_version: header_str(&headers, "x-yis-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-yis-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-yis-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::YisComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("xai-yis-cli"));
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "yis-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "yis-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "yis-build", "context_window" : 256_000, "_meta" : {
    ...
    /// REGRESSION: `yis setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("YIS_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("YIS_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
