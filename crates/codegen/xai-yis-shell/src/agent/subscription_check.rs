...
/// Subscription tiers that qualify for Yis Cli access.
...
async fn fetch_user_info(
    ...
    let request = http_client
        ...
        .header(
            "X-XAI-Token-Auth",
            auth_manager.yis_com_config().token_header.as_str(),
        )
...
pub(crate) async fn single_check(
    ...
    // Yis 本地模式：跳过 paywall HTTP（国内无梯子时常超时 30s+）
    if !crate::util::config::resolve_remote_fetch_enabled() {
        tracing::info!("paywall_check skipped: remote_fetch disabled");
        return None;
    }
