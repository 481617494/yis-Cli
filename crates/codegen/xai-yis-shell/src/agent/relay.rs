...
impl RelayConfig {
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.yis_ws_url.clone(),
            ws_origin: ctx.yis_ws_origin.clone(),
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `yis login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(xai_yis_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                xai_yis_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test YisAuth with the given key.
    fn test_auth(key: &str) -> YisAuth {
        YisAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &YisAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = YisAuth {
    ...
    /// Helper: write a YisAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &YisAuth) {
