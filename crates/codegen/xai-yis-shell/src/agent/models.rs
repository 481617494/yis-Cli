...
use crate::auth::{AuthManager, YisAuth, YisComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use xai_yis_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&yis_home, YisComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<xai_yis_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<xai_yis_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// Hot-reload the catalog from `~/.yis/models_cache.json` after an
    ...
    /// When another grok process sharing `~/.yis` (a `--no-leader` run, a
    ...
    /// `ModelsCacheManager` path is fixed to `yis_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::yis_home::yis_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.yis_version.as_deref() != Some(xai_yis_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            yis_version: Some(xai_yis_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Yis Cli" both route to `grok-build`)
...
fn prefetch_models_blocking_gated(
    ...
    // Local / air-gap: do not use cloud catalog cache either (it still points
    // at cli-chat-proxy models and makes `/model` look like only Grok cloud).
    if !remote_fetch_enabled {
        tracing::info!("models fetch skipped: remote_fetch disabled (no network, no cloud cache)");
        return None;
    }
...
fn resolve_prefetch_env_with_auth(auth: Option<YisAuth>) -> Option<PrefetchEnv> {
...
fn resolve_prefetch_env(yis_com_config: Option<YisComConfig>) -> Option<PrefetchEnv> {
    let yis_home = crate::util::yis_home::yis_home();
    let auth_manager = AuthManager::new(&yis_home, yis_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<YisAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(yis_com_config: Option<YisComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(yis_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let auth_manager = Arc::new(AuthManager::new(&tmp, YisComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use xai_yis_sampling_types::ReasoningEffortOption;
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("YIS_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "yis-build", Some("Auto")),
            make_entry_config_with_id(Some("yis-build"), "yis-build", Some("Yis Cli")),
        ...
        assert!(map.contains_key("yis-build"));
        assert!(map.contains_key("grok-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "yis-build",
            "auto entry should still route to grok-build"
        );
        assert_eq!(map["yis-build"].info.model, "yis-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("yis-build"), "yis-build", Some("First")),
            make_entry_config_with_id(Some("yis-build"), "yis-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["yis-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            ...
            make_model_entry("yis-build"),
        );
        catalog.insert("yis-build".to_string(), make_model_entry("yis-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("yis-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "yis-build", "must match id, not first slug hit");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("yis-build".to_string(), make_model_entry("yis-build"));
        models.insert(
            "enterprise-grok-build".to_string(),
            make_model_entry("yis-build"),
        );
        ...
        let persisted = acp::ModelId::new("yis-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("yis-build".to_string(), make_model_entry("grok-4.5"));
        models.insert("other".to_string(), make_model_entry("yis-build"));

        let available = test_available_keys(&["yis-build", "other"]);

        let persisted = acp::ModelId::new("yis-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "yis-build");
    }
