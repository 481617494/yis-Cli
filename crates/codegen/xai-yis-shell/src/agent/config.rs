...
use crate::auth::{AuthManager, YisComConfig, OidcAuthConfig};
...
use std::sync::Arc;
use xai_yis_agent::prompt::skills::SkillsConfig;
use xai_yis_sampler::{AuthScheme, SamplerConfig};
use xai_yis_sampling_types::{
...
use xai_yis_tools::types::compat::{
...
pub struct EndpointsConfig {
    ...
    /// Env: `YIS_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `YIS_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `YIS_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `YIS_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `YIS_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `YIS_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `YIS_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `YIS_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `YIS_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `YIS_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `YIS_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `YIS_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `YIS_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub yis_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `YIS_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub yis_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `YIS_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
/// format, shared with `YIS_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    pub fn proxy_url(&self) -> String {
        use crate::util::config::is_local_mode;
        if is_local_mode() {
            return String::new();
        }
    ...
    pub fn resolve_inference_base_url(&self) -> String {
        use crate::util::config::is_local_mode;
        if is_local_mode() {
            // 本地模式：回退到 xai_api_base_url（用户通过环境变量配置），不复用 proxy_url
    ...
    /// Managed deployment-config URL (`yis setup`): explicit `managed_config_url`,
    ...
    /// 1. `yis_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.yis_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 YIS_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `yis_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.yis_internal_otlp_headers) {
    ...
    /// `yis_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.yis_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.yis_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `YIS_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        use crate::util::config::is_local_mode;
        // 本地模式：无默认推理端点，需用户通过 YIS_XAI_API_BASE_URL 或 YIS_MODELS_BASE_URL 配置
        let default_api_base = if is_local_mode() {
            String::new()
        } else {
            XAI_API_BASE_URL_DEFAULT.to_owned()
        };
        Self {
            cli_chat_proxy_base_url: std::env::var("YIS_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("YIS_XAI_API_BASE_URL")
                .unwrap_or_else(|_| default_api_base),
            alpha_test_key: None,
            models_base_url: env_string("YIS_MODELS_BASE_URL"),
            models_list_url: env_string("YIS_MODELS_LIST_URL"),
            feedback_base_url: env_string("YIS_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("YIS_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("YIS_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("YIS_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("YIS_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("YIS_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("YIS_DEPLOYMENT_KEY"),
            managed_config_url: env_string("YIS_MANAGED_CONFIG_URL"),
            ...
            yis_internal_otlp_traces_endpoint: env_string("YIS_INTERNAL_OTLP_TRACES_ENDPOINT"),
            yis_internal_otlp_headers: env_string("YIS_INTERNAL_OTLP_HEADERS"),
...
pub use xai_yis_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use xai_yis_config::env_bool;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(xai_yis_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: xai_yis_tools::types::compat::CompatCell,
...
pub(crate) use xai_yis_telemetry::config::env_telemetry_mode;
pub use xai_yis_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.yis/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                xai_yis_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> xai_yis_agent::plugins::discovery::DiscoveryConfig {
        xai_yis_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `yis-build-0.1`
...
/// (`yis workspace` / leader `with_default_hub_url`). Does **not** enable
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "YIS_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("YIS_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("YIS_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "YIS_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "yis-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.yis/skills/`,
/// `.agents/skills/`, `~/.yis/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Typed as `YisComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<YisComConfig>,
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<xai_yis_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `xai_yis_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from YIS_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<xai_yis_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `YIS_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.yis/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, xai_yis_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, xai_yis_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<xai_yis_tools::implementations::yis_build::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<xai_yis_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<xai_yis_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut xai_yis_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut xai_yis_agent::config::AgentDefinition) {
...
pub use xai_yis_agent::config::AgentDefinition;
pub use xai_yis_agent::config::Effort;
pub use xai_yis_agent::config::PermissionMode;
pub use xai_yis_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .yis/agents/, ~/.yis/agents/, built-ins)
...
pub struct AgentSelectionConfig {
    ...
    /// Looked up via `xai_yis_agent::discovery::by_name_in_cwd()`.
    /// Examples: "yis-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.yis/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            yis_com_config: YisComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(xai_yis_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::yis_home::yis_home(),
            self.yis_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.yis_com_config.oidc.is_none() {
            config.yis_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.yis_com_config.oidc.is_none() && config.yis_com_config.oauth2.is_none() {
            config.yis_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("YIS_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[yis_com_config]`.
    /// `[yis_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[yis_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("yis_com_config") {
                ...
                table.insert("yis_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("YIS_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `YIS_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_telemetry_mode(&self) -> Resolved<TelemetryMode> {
        // Privacy: never enable cloud telemetry in local mode.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("YIS_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("YIS_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("YIS_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        // Privacy: no feedback upload to xAI in local mode.
        if crate::util::config::is_local_mode() {
            return Resolved::new(false, ConfigSource::Default);
        }
        let ff = self
        ...
        BoolFlag::env("YIS_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("YIS_TWO_PASS_COMPACTION")
    ...
    /// `YIS_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<xai_yis_sampling_types::DoomLoopRecoveryPolicy> {
        use xai_yis_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("YIS_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `YIS_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    /// Always off in local / privacy mode (no outbound git to official xAI source).
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("YIS_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("YIS_ASK_USER_QUESTION")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use xai_yis_tools::implementations::yis_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("YIS_IMAGE_EDIT").default(true).resolve()
    }
    ...
    /// Precedence: env `YIS_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("YIS_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("YIS_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("YIS_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `YIS_COMPACTION_MODE` > config > remote settings >
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> xai_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("YIS_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `YIS_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("YIS_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `YIS_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> xai_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("YIS_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("YIS_CANCEL_REWIND")
    ...
    ///
    /// Priority: `--oauth` > YIS_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_yis_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("YIS_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.yis/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (YIS_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("YIS_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (YIS_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `YIS_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("YIS_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (YIS_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("YIS_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (YIS_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `YIS_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("YIS_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `YIS_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(yis_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("YIS_ERROR_REPORTING"))
...
/// `YIS_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn yis_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("YIS_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.yis/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::yis_home::yis_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `YIS_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        xai_yis_config::load_merged_requirements().as_ref(),
        env_bool("YIS_EXTERNAL_OTEL"),
...
/// by `yis setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: xai_yis_telemetry::external::config::ExternalClientInfo,
) -> Option<xai_yis_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<xai_yis_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| xai_yis_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            xai_yis_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = xai_yis_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = xai_yis_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        xai_yis_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = xai_yis_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    xai_yis_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub fn resolve_model_list(
    ...
    } else if crate::util::config::is_local_mode() {
        // Privacy local mode: no empty-base cloud defaults (yis-build without URL).
        // Only user [model.*] entries and optional custom endpoints are used.
        tracing::info!("local mode: skipping built-in cloud default models");
    } else {
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `YIS_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<xai_yis_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// flag in `yis_build_settings` is absent. When the remote flag is present it takes
    ...
    /// env (`YIS_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`YIS_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `YIS_MCP_AUTO_RESTART` to opt out).
    ///
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_first_party_xai_url(&creds.base_url)
    {
        ...
        xai_yis_telemetry::unified_log::debug(
...
pub use xai_yis_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.yis_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            ...
            .or_insert_with(|| "xai-yis-cli".to_string());
        headers
...
mod tests {
    ...
    use xai_yis_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_yis_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use xai_yis_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["YIS_TEST_ENV_KEY_PRIMARY", "YIS_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "YIS_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "YIS_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "YIS_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "YIS_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "YIS_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["YIS_TEST_WS_PRIMARY", "YIS_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "YIS_TEST_WS_PRIMARY" => Some("   ".into()),
                "YIS_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("YIS_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("YIS_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use xai_yis_test_support::EnvGuard;
        let var = "YIS_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        ...
        let primary = "YIS_TEST_MULTI_ENV_PRIMARY";
        let alias = "YIS_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use xai_chat_state::AuthType;
        use xai_yis_test_support::EnvGuard;
        let primary = "YIS_TEST_EMPTY_ENV_PRIMARY";
        let alias = "YIS_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "YIS_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "YIS_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = xai_yis_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use xai_yis_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use xai_yis_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    /// xai-yis-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_yis_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    #[test]
    fn auth_alias_maps_to_yis_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.yis_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn yis_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [yis_com_config.oidc]
            issuer = "https://example.okta.com"
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.yis_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.yis_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.yis_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.yis_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, YisComConfig};
        let base = YisComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..YisComConfig::default()
        };
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "YIS_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "YIS_CLI_CHAT_PROXY_BASE_URL",
            "YIS_XAI_API_BASE_URL",
            "YIS_FEEDBACK_BASE_URL",
            "YIS_TRACE_UPLOAD_URL",
            "YIS_MANAGED_CONFIG_URL",
            "YIS_MODELS_BASE_URL",
            "YIS_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "YIS_INTERNAL_OTLP_TRACES_ENDPOINT",
            "YIS_INTERNAL_OTLP_HEADERS",
            "YIS_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        // Cloud-mode default (debug-only escape hatch); local mode always disables.
        unsafe { std::env::set_var("YIS_LOCAL_MODE", "0") };
        unsafe { std::env::remove_var("YIS_OFFLINE") };
        unsafe { std::env::remove_var("YIS_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("YIS_TELEMETRY_ENABLED") };
        let cfg = Config::default();
        let r = cfg.resolve_feedback();
        assert!(r.value, "feedback should be true by default outside local mode");
        assert_eq!(r.source, ConfigSource::Default);
        unsafe { std::env::remove_var("YIS_LOCAL_MODE") };
    }
    #[test]
    #[serial]
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("YIS_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("YIS_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("YIS_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("YIS_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("YIS_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("YIS_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("YIS_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("YIS_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("YIS_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("YIS_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("YIS_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("YIS_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("YIS_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("YIS_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("YIS_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("YIS_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("YIS_GOAL");
            std::env::remove_var("YIS_GOAL_CLASSIFIER");
            std::env::remove_var("YIS_GOAL_PLANNER");
            std::env::remove_var("YIS_GOAL_SUMMARY");
            std::env::remove_var("YIS_GOAL_VERIFIER_N");
            std::env::remove_var("YIS_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("YIS_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("YIS_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("YIS_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("YIS_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("YIS_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "YIS_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "yis-build", agent_type = "grok-build-plan" }
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "yis-build");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "yis-build");
        assert_eq!(
...
"#,
        ...
        let yis_build = crate::util::config::GoalRoleModel {
            model: "yis-build".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(yis_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(yis_build),
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            yis_internal_otlp_traces_endpoint: None,
            yis_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `yis_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_grok_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                yis_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: YIS_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                yis_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                yis_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `yis_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                yis_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> xai_yis_telemetry::external::config::ExternalClientInfo {
        xai_yis_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("YIS_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `YIS_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("YIS_SUBAGENTS");
            std::env::remove_var("YIS_RESPECT_GITIGNORE");
            std::env::remove_var("YIS_WEB_SEARCH_MODEL");
            std::env::remove_var("YIS_SESSION_SUMMARY_MODEL");
            std::env::remove_var("YIS_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("YIS_CURSOR_RULES_ENABLED");
            std::env::remove_var("YIS_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("YIS_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("YIS_CLAUDE_RULES_ENABLED");
            std::env::remove_var("YIS_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("YIS_MANAGED_MCPS_ENABLED");
            std::env::remove_var("YIS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("YIS_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "YIS_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "YIS_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "YIS_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("YIS_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("YIS_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("YIS_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("YIS_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("YIS_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("YIS_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("YIS_TELEMETRY_ENABLED") };
...
"#,
        ...
        xai_yis_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.yis-build] sets
    ...
    fn resolve_model_list_inherits_context_window_from_default_when_prefetched_has_fallback() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("yis-build", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("yis-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("yis-build").expect("model must exist");
        assert_ne!(
    ...
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("yis-build", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("yis-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("yis-build").expect("model must exist");
    ...
    fn resolve_model_list_inherits_agent_type_and_api_backend() {
        ...
        if let Some(default) = defaults.get("yis-build") {
    ...
    fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
        ...
        if let Some(e) = defs.shift_remove("yis-build") {
            p.insert("yis-build".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("yis-build"));
        let no_p = resolve_model_list(&cfg, None);
        assert!(no_p.contains_key("yis-build"));
    }
    #[test]
    ...
    fn byok_config_overlay_visible_to_api_key_users() {
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        let resolved = resolve_model_list(&cfg, None);
        let entry = resolved.get("yis-build").expect("grok-build must exist");
        assert!(
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("YIS_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("YIS_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("YIS_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("YIS_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("YIS_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("YIS_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("YIS_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("YIS_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("YIS_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("YIS_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("YIS_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("YIS_MCP_RECURSIVE_CONFIG_WATCH", "false") };
