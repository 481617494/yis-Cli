...
use crate::auth::{AuthManager, AuthMode, YisAuth, run_auth_flow};
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = xai_yis_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    xai_yis_telemetry::unified_log::set_version(xai_yis_version::VERSION);
    ...
    if let Ok(version) = std::env::var("YIS_CLIENT_VERSION") {
...
async fn run_headless_inner(
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `yis login` to sign in, or use `grok agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.yis_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None => anyhow::bail!("No cached credentials found. Run `yis login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&yis_home::yis_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    let yis_code_url = format!("{}/build", ctx.yis_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Yis Cli: {} (press Enter to open in browser)",
                yis_code_url
            );
            eprintln!();
            let url_for_open = yis_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&YisAuth>, session: &YisAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
pub async fn run_leader(
    ...
    let auth: Option<YisAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<YisAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<YisAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            let auth_scope = agent_config.yis_com_config.auth_scope();
            // Gated on user_yis_home() so a cwd-relative .yis/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = xai_yis_config::user_yis_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.yis/models_cache.json
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> YisAuth {
        YisAuth {
            key: key.into(),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::YisComConfig {
            yis_ws_url: format!("ws://{addr}"),
            yis_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
