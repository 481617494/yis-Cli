...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("yis-build", "Yis Cli"),
            model("grok-4.5", "Grok 4.5"),
        ];
        let current = acp::ModelId::from("yis-build");
        ...
        assert_eq!(selected_models[0].id, "yis-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("yis-build", "Yis Cli")];
        let current = acp::ModelId::from("yis-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("yis-build", "")];
        let current = acp::ModelId::from("yis-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "yis-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "yis-build".to_string(),
            category: "model".to_string(),
            label: "Yis Cli".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "yis-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Yis Cli");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn yis_session_detail_serializes_camel_case() {
        let detail = YisSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "yis-build");
