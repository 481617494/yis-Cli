...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "YIS_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `YIS_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `YIS_CODE_XAI_API_KEY` is set.
...
/// `YIS_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// True if `yis_com_config.auth_provider_command` is configured (sets
...
pub fn build_auth_methods(inputs: AuthMethodsBuildInputs<'_>) -> BuiltAuthMethods {
    ...
    // Local / privacy mode: never advertise grok.com / OIDC / session login.
    // Only BYOK / API key credentials are valid — no browser auth to xAI.
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.yis_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.yis_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(yis_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `yis login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `yis login`, set XAI_API_KEY, or add api_key to ~/.yis/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `yis login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.yis/auth.json".to_string())),
    )
}

pub const YIS_COM_METHOD_ID: &str = "grok.com";

/// xAI OAuth2/OIDC auth. Method id `"grok.com"` kept for ACP wire-compat.
pub fn yis_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Yis Cli");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(YIS_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    #[test]
    fn after_cached_token_unavailable_falls_to_yis_com_without_api_key() {
        assert_eq!(
    ...
    #[test]
    fn fresh_user_only_advertises_yis_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::YisCom));
    ...
    #[test]
    fn enterprise_oidc_replaces_yis_com_but_xai_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::YisCom),
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let grok = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::YisCom)
    ...
    /// `~/.yis/config.toml` skeleton from the bug report, walks it through
    ...
    /// Legacy `YIS_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `YIS_CODE_XAI_API_KEY` are set,
    ...
    //
    // `grok login --legacy` produces a YisAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `YIS_AUTH` env var (or via legacy
    ...
    // to-end. A regression in YIS_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `YIS_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn yis_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, YisAuth, YisComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("YIS_AUTH_PATH");
        ...
        // Provide it via YIS_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("YIS_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in YIS_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("YIS_AUTH");
        let _g2 = EnvGuard::unset("YIS_AUTH_PATH");
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, YIS_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![YIS_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
