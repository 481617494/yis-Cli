...
mod tests {
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("x.ai/queue/remove", &p, Some("yis-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("yis-tui"));
            }
        ...
        match parse_queue_edit_command("x.ai/queue/interject", &p, Some("yis-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
