...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = yis_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.yis/config.toml".to_string(),
...
/// Discover managed `yis_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    let yis_home = xai_yis_tools::util::yis_home::yis_home();
    let yis_com_config = YisComConfig::default();
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&yis_home, yis_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `yis login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use xai_yis_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `yis mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `yis mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
