...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "YIS_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$YIS_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${YIS_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$YIS_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$YIS_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the YIS_MEMORY env var.
...
/// Run `f` with YIS_MEMORY explicitly unset.
fn without_yis_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("YIS_MEMORY", None, f)
}
/// Run `f` with YIS_MEMORY set to a specific value.
fn with_yis_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("YIS_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_yis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "YIS_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_yis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "YIS_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_yis_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "YIS_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_yis_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "YIS_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_yis_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override YIS_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_yis_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, YIS_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_yis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override YIS_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$YIS_TEST_USER $YIS_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the YIS_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with YIS_SUBAGENTS explicitly unset.
fn without_grok_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("YIS_SUBAGENTS", None, f)
}
/// Run `f` with YIS_SUBAGENTS set to a specific value.
fn with_grok_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("YIS_SUBAGENTS", Some(value), f)
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "YIS_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_grok_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "YIS_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_grok_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override YIS_SUBAGENTS=0");
        },
    );
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "YIS_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("YIS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "YIS_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "YIS_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "YIS_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("YIS_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_yis_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_yis_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "YIS_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("YIS_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
    ...
    with_grok_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "YIS_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".yis/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".yis/prompts/impl.md"));
}
#[test]
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".yis").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".yis/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".yis").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".yis").join("bundled");
    std::fs::create_dir_all(workspace.join(".yis").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".yis").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".yis").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".yis").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".yis/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".yis/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".yis/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".yis/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".yis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".yis/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
    ...
    std::fs::remove_file(home.join(".yis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".yis/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.yis/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
#[test]
fn validate_hooks_path_rejects_outside_yis_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("must be under ~/.yis/"),
        "should mention ~/.yis/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let yis_home = crate::util::yis_home::yis_home();
    let traversal = format!("{}/../evil", yis_home.display());
...
fn validate_hooks_path_accepts_grok_hooks_subdir() {
    let yis_home = crate::util::yis_home::yis_home();
    let valid_path = yis_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.yis/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use xai_yis_workspace::permission::resolution::ManagedSettingsFeatures;
...
fn simulate_release_build() -> xai_yis_test_support::EnvGuard {
    xai_yis_test_support::EnvGuard::set(xai_yis_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). YIS_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores YIS_HOME even on panic). No user-global
/// `$YIS_HOME/config.toml` is seeded: `yis_home()` is `OnceLock`-cached,
...
/// in `xai-yis-agent`. An ABSOLUTE plugin path is used so the merged
...
/// YIS_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_yis_agent::plugins::{TrustStore, discover_plugins};
    use xai_yis_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("YIS_HOME", home.path());
    let _flag = EnvGuard::unset("YIS_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. YIS_HOME-isolated (empty
/// store); YIS_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
