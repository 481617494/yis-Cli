...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.yis/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.yis` (the writer may also be this process;
...
/// Watches `~/.yis/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.yis/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.yis/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let yis_home_buf = yis_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(yis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(yis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(yis_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(yis_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %yis_home.display(),
                    error = %e,
                    "failed to watch yis home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.yis/config.toml` so the loop above
        // watches `<cwd>/.yis/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.yis/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.yis/config.toml` trigger a
    ...
    /// `<cwd>/.yis/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.yis/` does not yet
/// exist at session-open time, the `.yis/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.yis/` followed by a
/// write to `<cwd>/.yis/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let yis_dir = cwd.join(".yis");
    if let Err(e) = debouncer
        .watcher()
        .watch(&yis_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .yis directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let yis_dir = cwd.join(".yis");
    if let Err(e) = debouncer.watcher().unwatch(&yis_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .yis directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.yis/` hasn't
...
/// Watches skill directories (`~/.yis/skills/`, `<repo>/.yis/skills/`, etc.)
...
/// recursively: `yis_home` (`~/.yis`, or `$YIS_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.yis` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, yis_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, yis_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".yis", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, yis_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(yis_home) {
        return true;
    }
...
/// `~/.yis` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](xai_yis_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_yis_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &yis_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &yis_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only yis_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_yis_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let yis_home = home.join(".yis");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &yis_home, Some(home));

        // yis_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&yis_home));
        ...
        assert!(!g(&home.join("repo").join(".yis")));
    }

    /// Regression for the ~/.yis inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.yis/worktrees/`) must not
    ...
    /// A write to `<yis_home>/models_cache.json` must surface as
    ...
    /// A write to `<cwd>/.yis/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let yis_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_grok = cwd.path().join(".yis");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.yis/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.yis/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".yis");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
