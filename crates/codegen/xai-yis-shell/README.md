...
- **Reference**
  - [Introspection (`yis inspect`)](#introspection)
...
- [Building with Grok](#building-with-yis) — headless API, ACP SDK integration
...
Credentials are stored in `~/.yis/auth.json` and persist across sessions. Tokens expire after 7 days; Grok will prompt you to re-authenticate when needed.
...
export YIS_OIDC_ISSUER="https://acme.okta.com"
export YIS_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export YIS_CLI_CHAT_PROXY_BASE_URL="https://grok-proxy.acme.com/v1"
```

**3. Run `yis`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.yis/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
4. **stdout** → captured by Grok and saved to `~/.yis/auth.json` as the access token
...
```bash
# Or via environment variables
export YIS_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export YIS_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export YIS_AUTH_TOKEN_TTL=3600               # optional
```
...
When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`yis -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
When Grok needs to refresh an expired token, it re-runs your binary with `YIS_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$YIS_AUTH_EXPIRED" = "1" ]; then
...
`YIS_AUTH_EXPIRED` is optional — if your binary ignores it, Grok still works. It just runs the same flow for both login and refresh.
...
export YIS_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `yis login` before starting — Grok runs your binary automatically on first launch. You _can_ run `yis login` to explicitly hydrate `auth.json` ahead of time if you prefer.
...
```bash
yis --debug-file /tmp/grok-auth.log -p "hello"
...
If you've authenticated with `yis login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the yis CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.yis/auth.json)" \
  -H "X-XAI-Token-Auth: xai-yis-cli" \
  -H "x-grok-model-override: yis-build" \
  -d '{
    "model": "yis-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.yis/auth.json` (set by `yis login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-yis-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-grok-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `yis-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `yis-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `yis login` to refresh.
...
yis --cwd ~/projects/my-app

# Start with an initial task
yis --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
yis --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
yis --always-approve --prompt "Format all files"
```
...
| `-m, --model <MODEL>`   | Model to use (e.g., `yis-build`)               |
...
yis -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
yis -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
yis -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
yis -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
yis -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
yis -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
yis -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
yis -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
yis -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
yis --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
yis -p "What does this project do?"

# Use a specific model
yis -p "Optimize this function" -m yis-build

# Get JSON output for parsing
yis -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
yis -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
yis -p "Remember: the secret number is 42" --output-format json
yis -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
yis -p "Continue where we left off" -c

# Run in a different directory
yis -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
yis -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
yis -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
yis -p "Generate a README" > README.md

# Parse JSON output with jq
yis -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
yis -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | yis -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  yis -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
yis -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `yis-build`)                           |
...
```bash
yis agent headless --grok-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`yis ssh`)

Use `yis ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
yis ssh -t user@host
yis ssh -L 8080:localhost:8080 user@host

# With remote command
yis ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `yis ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Use headless mode for simple integrations. Spawns `yis -p` and parses JSON output.
...
class GrokChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["yis", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="yis-build", stream=False):
...
class GrokChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("yis", this.buildArgs(prompt, model, true));
...
class GrokACPChat {
  ...
  async init() {
    this.proc = spawn("yis", ["agent", "stdio"]);
...
Grok reads configuration from `~/.yis/config.toml`. If the file doesn't exist, Grok uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "yis-build"           # model used for new sessions
...
telemetry = false                      # anonymous usage telemetry (env: YIS_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: YIS_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: YIS_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: YIS_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: YIS_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: YIS_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: YIS_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: YIS_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `YIS_TELEMETRY_BUILD_EVENTS_URL`, `YIS_TELEMETRY_BUILD_EVENTS_API_KEY`, and `YIS_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.
...
- project config: `<repo>/.yis/lsp.json`
- user config: `~/.yis/lsp.json`
...
- LSP tools are enabled (`YIS_LSP_TOOLS=1` or `[features] lsp_tools = true`)
...
- Passive diagnostics do **not** require `YIS_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `yis` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.
...
| --------------------------- | ----- | -------- |
| `./.yis/skills/`           | Local | Highest  |
| `<repo_root>/.yis/skills/` | Repo  | Medium   |
| `~/.yis/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.yis/skills/`) are outside the repo and never filtered.
...
Grok discovers agent definitions from `.yis/agents/` (project), `~/.yis/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `YIS_AGENT` env var
4. Default `yis-build` agent
...
```bash
yis --agent-profile ./my-agent.md
# or
export YIS_AGENT="my-custom-agent"
```
...
```bash
export YIS_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "yis-build"              # route explore to a lighter model
```
...
model = "yis-build"
prompt_file = ".yis/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".yis/personas/concise.md"  # or load from file
```

Both are also discovered from `.yis/roles/*.toml` and `.yis/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).
...
Grok discovers hooks from `.yis/hooks/` in the project directory. Manage them with:
...
**Web search model:** Set `[models] web_search`, `YIS_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `yis models` output). You also need a `[model.*]` entry so Grok knows
...
|----------|----------|-------------|
| `YIS_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `YIS_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export YIS_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
yis
```

Grok fetches the model list from `{YIS_MODELS_BASE_URL}/models` on startup and sends inference requests to `YIS_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `YIS_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.yis/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Grok uses API key auth (`Authorization: Bearer`) instead of session auth. `yis login` is not required — only the API key.
...
MCP servers are configured in `~/.yis/config.toml`:
...
MCP servers can also be configured per-project in `.yis/config.toml`. Grok walks from the current directory up to the git repo root, loading `.yis/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.yis/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.yis/config.toml` | This repository   | ↑        |
| `<cwd>/.yis/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.yis/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.yis/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.yis/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.yis/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `YIS_MEMORY=1` / `[memory] enabled = true` in config).
...
Memory is stored as Markdown files under `~/.yis/memory/`:
- **Global** (`~/.yis/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.yis/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.yis/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.yis/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.yis/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.yis/memory/` for external edits and reindex on search |
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.yis/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.yis/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.yis/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.yis/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.yis/sandbox.toml` (global) or `.yis/sandbox.toml` (per-project):
...
The sandbox is applied to the **entire yis process** at startup using kernel
...
Sandbox events (profile applied, violations) are logged to `~/.yis/sandbox-events.jsonl`
...
Use `yis inspect` to see everything Grok discovers in the current directory:

```bash
yis inspect          # human-readable output
yis inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.yis/skills/`, `~/.yis/skills/`, plugins, and config paths
...
Grok automatically discovers configuration from Claude Code directories alongside native `.yis/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Grok uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.yis/skills/`) |
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `YIS_WEB_FETCH=1`. 
...
Sessions are stored under `~/.yis/sessions/`, organized by URL-encoded working directory:

```
~/.yis/sessions/<encoded-cwd>/<session-id>/
...
yis -p "Remember: X=42" -s my-session
yis -p "What is X?" -s my-session

# Resume existing session (errors if not found)
yis -p "Continue" -r my-session

# Continue most recent session in current directory
yis -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
yis -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.yis/config.toml` | Configuration file                                  |
| `~/.yis/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.yis/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.yis/memory/`     | Cross-session memory files and index                |
| `~/.yis/skills/`     | User-scoped skill definitions                       |
| `~/.yis/plugins/`    | User-scoped plugins                                 |
| `~/.yis/agents/`     | User-scoped agent definitions                       |
| `.yis/config.toml`   | Project-scoped config (MCP servers)                 |
| `.yis/skills/`       | Project-scoped skill definitions                    |
| `.yis/plugins/`      | Project-scoped plugins                              |
| `.yis/agents/`       | Project-scoped agent definitions                    |
| `.yis/hooks/`        | Project-scoped hooks                                |
| `.yis/lsp.json`      | LSP server configuration                            |
...
| `YIS_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.grok.com/v1`)                          |
| `YIS_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `YIS_MODELS_LIST_URL`          | Override the model list URL if it differs from `{YIS_MODELS_BASE_URL}/models`                                              |
| `YIS_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `YIS_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `YIS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `YIS_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `YIS_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `YIS_HOME`                     | Override config directory (default: `~/.yis`)                                                           |
| `YIS_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `YIS_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `YIS_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `YIS_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `YIS_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `YIS_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `YIS_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `YIS_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `YIS_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `YIS_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.yis/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `YIS_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `yis` commands and flags.
...
yis completions bash > ~/.local/share/bash-completion/completions/yis
```
...
```bash
mkdir -p ~/.yis/completions/bash
yis completions bash > ~/.yis/completions/bash/yis.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.yis/completions/bash/yis.bash" ]] && source "$HOME/.yis/completions/bash/yis.bash"
```
...
yis completions zsh > ~/.zsh/completions/_grok
```
...
```bash
mkdir -p ~/.yis/completions/zsh
yis completions zsh > ~/.yis/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.yis/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `yis` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `yis --debug` or `YIS_LOG_FILE` instead:

```bash
# Per-session debug log (~/.yis/debug/<sessionId>.txt)
yis --debug

# Log to a custom path
YIS_LOG_FILE=/tmp/grok-debug.log yis

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.yis/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `YIS_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
YIS_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,xai_grok_shell::auth=debug" yis
```
...
cat ~/.yis/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.yis/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.yis/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
