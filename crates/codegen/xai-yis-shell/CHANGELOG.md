...
- **yis inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct yis --minimal --resume command.
...
- **yis setup --json** prints your team's managed configuration without installing it.
...
- **`yis worktree ls`** now works as a short alias for `yis worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.yis/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.yis/worktrees/.
...
- **yis inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain yis launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive yis agent stdio without silent parse drops on session/* calls.
...
- **yis --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **yis --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **yis update** on Windows no longer fails when a previous.old executable is still running.
...
- **yis models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `yis plugin marketplace add`.
...
- **Plan files** now default to.yis/plan.md to match Grok conventions.
...
- **--minimal** flag now shows in `yis --help`.
...
- **Tool result previews** no longer paint opaque panels in `yis --minimal`.
- **yis wrap** now correctly handles quoted strings and shell aliases.
...
- **`yis update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `yis wrap`.
...
- **yis agent stdio** now checks for updates in the background like other modes.
...
- **yis wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `yis agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `yis wrap`** to run any command with local clipboard support.
...
- **Fixed `yis agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or grok-desktop.
...
- You can now pass --json-schema to yis -p and receive a validated JSON object instead of free text.
...
- **yis -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in yis -p and yis agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `yis` after running `yis update`.
- Headless (`yis -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`yis plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **yis sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **yis quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, yis sessions list) are now sorted by the same activity time shown in the UI.
...
- **`yis dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`yis mcp add`** now accepts positional arguments (e.g. `yis mcp add filesystem -- npx...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`yis update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **MCP servers menu** and slash commands now work when starting yis outside a project directory.
...
- **yis --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `yis --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `yis update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`yis --debug`** now produces per-session log files under ~/.yis/debug/ even with a leader process.
...
- **`yis update`** now restarts any older running leader so all clients get the new binary.
...
- **`yis login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `yis agent --leader` without forced interactive login or timeouts.
...
- **yis inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
...
- **Version output** now shows [alpha] or [stable] in `--version`, banners, `/session-info`, and `yis inspect`.
...
- **New `/export` slash command** and `yis export` CLI command let you save or copy conversation transcripts as Markdown.
...
- **New `yis plugin` commands** let you manage plugins and marketplaces directly from the terminal.
...
- **New `yis plugin` commands** let you manage plugins and marketplaces directly from the terminal.
- New `/export` command and `yis export` let you save or copy conversation transcripts as Markdown.
...
- **Added `yis logout` subcommand** to sign out from the terminal without launching the TUI.
...
- **Improved error messages** when hitting a subscription wall with an API key set, suggesting `yis logout`.
...
- **Yis Cli** now runs natively on Intel Macs via x86_64 builds.
...
- **Paywall** now lifts automatically after subscribing via yis login or external purchase without needing to re-login.
...
- **New `yis logout` command** clears your cached login session directly from the terminal.
...
- **Fixed error messages** when hitting a subscription wall with an API key set, now suggesting `yis logout`.
- **Fixed a crash** on exit when using MCP servers that require token refresh.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on Yis Cli.
...
- **`yis sessions search`** now finds local and remote sessions correctly, and **`yis -r <id>`** restores remote sessions reliably.
...
- **Fixed git and bash commands** failing in Yis Desktop on Windows.
...
- **`yis -w my-worktree`** creates a custom-named worktree folder.
...
- **`/usage manage`** now opens billing page; **`yis login`** enforces tiers correctly after refresh.
...
- **Legacy web login** no longer treated as first-party xAI account for sharing and billing; prompts **`yis login`** upgrade.
...
- **Legacy authentication** removed; run **`yis login`** to upgrade to OAuth.
...
- **Legacy web login** no longer treated as premium xAI account; run **`yis login`** instead.
...
- **Fixed `yis sessions list`** auth failures.
...
- **`yis sessions list`** now works reliably after login.
...
- **Added yis memory clear** CLI command with scope selection and confirmation.
...
- **Computer Hub integration** into xai-yis-shell
...
- **yis memory clear command** with scope and confirmation
...
- **Shell commands now work** in Yis Desktop on Windows.
...
- **New `yis import`** resumes Claude Code sessions with `yis import <id/path>`.
...
- **Fixed `yis share`** failing with backend ZodError due to duplicate request headers.
...
- **New `yis login --devbox`** option for signing in from headless devbox environments without browser access.
...
- **New `yis mcp doctor`** diagnoses MCP servers and suggests fixes.
...
- **New `yis mcp doctor`** diagnoses MCP server configuration and connectivity.
...
- **`yis models`** and TUI picker now show team-specific models when **`GROK_CODE_XAI_API_KEY`** is set.
...
- **Clearer login error messages** now suggest `yis login`, API key env var, or config.toml.
...
- **Native Windows support** (opt-in) lets you run Yis Cli directly without WSL, with MCP server compatibility.
...
- **Authentication errors** now clearly suggest `yis login`, env var, or config.toml api_key.
- **Fixed false-positive loop warnings** for different shell commands during edit-test cycles.
...
- **Import Claude settings** from `.claude/` into `.yis/config.toml` via welcome screen ('i') or `/import-claude`.
- **Skills reload automatically** when editing/saving SKILL.md files without restarting `yis`.
...
- **Improved `yis login`** with automatic code delivery from browser consent page.
...
- **New `yis trace`** command uploads or exports session data for debugging.
...
- **New `yis trace`** command exports or uploads session data for debugging.
...
- **`yis models`** now lists available models and the default.
...
- **`yis models`** command now lists available models and authentication status.
...
- **X Premium and Premium+** tiers now grant access to Yis Cli.
...
- **`yis sessions search`** now finds sessions by content with scores and snippets.
...
- **`yis sessions search`** now supports **full-text queries** across chat content and tools.
- **Session picker** now combines fuzzy matching with **deep content search** results.
- **Unified logs** now saved to `~/.yis/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Auth.json hot-reload** fixed for yis login in other terminals.
...
- **Auto-update restart** now launches the correct new version.
- **Auth.json hot-reload** now detects `yis login` from other terminals.
...
- **External `yis login`** now hot-reloads auth without app restart.
...
- **External `yis login`** now hot-reloads auth without restarting.
...
- **New `yis ssh`** command enables clipboard copy from remote sessions in Apple Terminal.
...
- **`yis login --device-auth`** enables login from SSH, Docker, and other headless environments without port forwarding.
...
- **Added** `yis login --device-auth` **flag** for headless environments like SSH and Docker.
...
- **Bundled agents** discovered from ~/.yis/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.yis/managed_config.toml/requirements.toml with `yis inspect` and `yis setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **yis login defaults to OAuth** instead of legacy relay; use --legacy or GROK_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old yis/grok-pager binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old yis/grok-pager binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.yis/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **Legacy relay auth flow** via `yis login --legacy` restores pre-OIDC accounts.x.ai token exchange.
...
- **OIDC OAuth replaces relay auth**; removed `--auth-signin-url`/`exchange-code-url`/`redirect-target` flags, run `yis login` to migrate.

## Features

- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.yis/lsp.json`.
...
- **Dynamic nested skill discovery** in `.yis/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.yis/config.toml before existing checks.
...
- **Skills discovery expands** to.agents/skills directories alongside.yis and.claude.
...
- **OIDC OAuth2 replaces legacy relay login** for `yis login`; add `--legacy` flag for old flow and remove `--auth-signin-url` etc. CLI flags (migrate by deleting auth.json).
- **Adds `yis login --legacy`** fallback; removes `--auth-signin-url` etc flags (use env/config defaults, no migration needed for most users).
...
- **Dynamic discovery of nested skills** from.yis/skills/ subdirs at runtime via reminders.
...
- **Discover skills from**.claude/skills directories alongside.yis/skills.
...
- **Codex grep_files uses** shared rg_path() matching yis_build pattern.
...
- **ACP x.ai/auth/logout** removes scopes from ~/.yis/auth.json.
...
- **Help skill reads ~/.yis/config.toml** to answer MCP server and model configuration queries.
...
- **Bundled /help skill** extracts README.md to ~/.yis/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.yis/personas/*.toml with fail-closed resolution.
...
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.yis/auth.json.
- **Discovers skills from.claude/skills** alongside.yis/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching yis_build and opencode patterns.
...
- **Auto-installs grok-pager** alongside yis during internal and GitHub release updates.
...
- **`yis completions` subcommand** generates bash/zsh completion scripts, with fast-path exit before any network or auth warmup.
...
- **`yis memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boost in hybrid search.
...
- **User-defined subagents** via .yis/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **Session token routing fixed** for default models — `yis login` users no longer get 400 errors on proxy-routed models.
...
- **Restored remote sessions create distinct local children** with parent tracking, preventing identity reuse and duplicate restores on repeated `yis -r`.
...
- **Shell completions** via `yis completions <shell>`, leader CLI, and stdio reconnect replay.
- **User-defined subagents** via.yis/agents/*.md with config toggles.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.yis/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via GROK_FEEDBACK_ENABLED.
- **Leader CLI** adds `yis leader list/info/profile/kill/dev` for discovery and CPU profiling.
...
- **ACP extension resumes sessions in worktrees** via single call matching `yis -w -r` CLI flow.
...
- **`yis memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boosting for retrieved memory chunks.
...
- **Cross-devbox session restore** via `yis sessions list/search` CLI subcommands and hardened `yis -r` with upload-ordering safety, cwd-scoped lookups, and staged-vs-unstaged correctness.
...
- **Headless mode (`yis -p`) exit panic** fixed by flushing telemetry before runtime teardown.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.yis/bin symlink at spawn time.
...
- **`yis worktree` subcommand** for listing, inspecting, removing, and garbage-collecting session worktrees.
...
- **Prompt introspection from the CLI** is available via `yis prompt`, with JSON or section output and persisted session prompt_context snapshots.
...
- **Installation diagnostics** help troubleshoot conflicting yis binaries by listing canonical and invoked paths, versions, update status, and optional JSON output.
...
- **npm installs now use a canonical Grok binary path** via postinstall copy to `~/.yis/bin/yis`, preventing installer conflicts and version confusion.
