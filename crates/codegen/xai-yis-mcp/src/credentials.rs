...
//!
//! Credentials are stored in `$YIS_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$YIS_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$YIS_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$YIS_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user yis home (set $YIS_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$YIS_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(xai_yis_config::user_yis_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$YIS_HOME/mcp_credentials.json`. Existing credential files must keep
