...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"yis_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.yis/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
/// MCP initialization strategy. Defined in `xai-yis-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use xai_yis_telemetry::enums::McpInitStrategy;
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        xai_yis_telemetry::session_ctx::log_event(xai_yis_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
/// External-config overrides for an MCP server, surfaced to xai-yis-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: xai_yis_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("yis-shell-{server_name}"),
                xai_yis_version::VERSION.to_string(),
            ),
        )
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            xai_yis_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = xai_yis_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`yis_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "yis_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("yis-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = xai_yis_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = xai_yis_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        xai_yis_telemetry::session_ctx::log_event(
                            xai_yis_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: xai_yis_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    xai_yis_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = xai_yis_telemetry::instrumentation::timer("mcp_start_servers");
...
mod tests {
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("yis_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("YIS_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("yis_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("yis_com_linear", "https://example.com/mcp"));
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'yis_com_notion' handshake failed: Auth required, when send initialize request"
        ));
