...
//!    the lookup order — `.yis-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = xai_yis_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(xai_yis_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    #[test]
    fn yis_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        ...
        std::fs::write(
            ...
            r#"{
                ...
                "plugins": [{
                    "name": "grok-plugin",
                    "description": "From the .yis-plugin index",
                    "category": "design",
        ...
        assert_eq!(plugins[0].tags, vec!["yis"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_grok_file(dir: &Path, file: &str, content: &str) {
        let yis_dir = dir.join(".yis-plugin");
        std::fs::create_dir_all(&yis_dir).unwrap();
        std::fs::write(yis_dir.join(file), content).unwrap();
    }
