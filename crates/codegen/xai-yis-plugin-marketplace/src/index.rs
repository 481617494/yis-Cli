...
//!
//! Catalog file lookup, in order: `.yis-plugin/marketplace.json` (preferred),
//! `.yis-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.yis-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.yis-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let yis_dir = marketplace_root.join(".yis-plugin");
    let claude_dir = marketplace_root.join(".claude-plugin");
    let candidates = [
        yis_dir.join("marketplace.json"),
        yis_dir.join("plugin.json"),
...
mod tests {
    ...
    #[test]
    fn load_index_valid_yis_dir() {
        let dir = tempfile::tempdir().unwrap();
        let yis_dir = dir.path().join(".yis-plugin");
        ...
        assert_eq!(result.unwrap().name, "yis");
    }

    #[test]
    fn load_index_yis_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".yis-plugin", "yis"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "yis");
    }
