...
//! `.yis-plugin/plugin-index.json` (preferred), then
...
mod tests {
    ...
    fn load_catalog_parses_grok_plugin_dir() {
        ...
        write_catalog(dir.path(), ".yis-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_yis_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".yis-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".yis-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
