...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".yis", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn yis_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".yis")));
                assert!(!is_project_dir(&home.join(".yis/bin")));
            }
        }

        #[test]
        fn yis_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".yis-proxy-work")));
            }
        }
