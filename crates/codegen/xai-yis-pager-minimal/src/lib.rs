//! Minimal (scrollback-native) render mode — `yis --minimal`.
//!
...
//! # Wiring
//!
//! `xai-yis-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`xai_yis_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-yis-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-yis-pager`.
///
...
/// subsequent calls are ignored (see [`xai_yis_pager::minimal_hook`]).
pub fn install() {
    xai_yis_pager::minimal_hook::install(xai_yis_pager::minimal_hook::MinimalHooks { draw });
}
