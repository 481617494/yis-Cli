//! Integration tests for xai-yis-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use xai_yis_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use xai_yis_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!xai_yis_sandbox::should_restrict_child_network());
}
