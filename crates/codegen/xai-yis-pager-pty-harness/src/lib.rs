//! Unified PTY harness for xai-yis-pager.
//!
...
//!    `xai-yis-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `YIS_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use xai_yis_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
