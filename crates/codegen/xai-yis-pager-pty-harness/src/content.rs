...
//! [`MockInferenceServer`] from `xai-yis-test-support` and provides the
...
pub use xai_yis_test_support::mock_server::LogEntry;
pub use xai_yis_test_support::mock_server::MockModelEntry as MockModel;
pub use xai_yis_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use xai_yis_test_support::sse;
pub use xai_yis_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.yis
    ...
    ///
    /// Mirrors `xai_yis_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit YIS_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // YIS_HOME is set in the test runner's env).
            ("YIS_HOME".into(), yis_home),
            ("YIS_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("YIS_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("YIS_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("YIS_HOME").as_deref(),
            content.home().join(".yis").to_str()
        );
        assert_eq!(get("YIS_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("YIS_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("YIS_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("YIS_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("YIS_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("YIS_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("YIS_MAX_RETRIES").as_deref(), Some("0"));
