//! Scripted TUI scenario runner for xai-yis-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$YIS_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&yis_home)
                .with_context(|| format!("create scenario YIS_HOME {}", yis_home.display()))?;
            fs::write(yis_home.join("config.toml"), config_toml)
