...
//! elected leader) plus a fixed leader socket beneath its `YIS_HOME`; clients
...
impl LeaderCluster {
    ...
    /// The cluster's sessions root: `YIS_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".yis").join("sessions")
    }
