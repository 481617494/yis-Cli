...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build xai-yis-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build xai-yis-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_xai-yis-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-yis-pager-bin` (the composition-
///    root package that owns the `xai-yis-pager` binary)
