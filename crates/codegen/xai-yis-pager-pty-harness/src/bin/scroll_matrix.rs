//! `scroll-matrix` — scroll validation matrix sweep for xai-yis-pager.
//!
...
use xai_yis_pager_pty_harness::pager_binary;
use xai_yis_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against xai-yis-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_xai-yis-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves YIS_SCROLL_LOG against ITS cwd (the
