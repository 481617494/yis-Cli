...
//! `~/.yis/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use xai_yis_tools::util::yis_home::yis_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `xai_yis_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.yis/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.yis/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.yis/memory/`
    global_dir: PathBuf,
    /// `~/.yis/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.yis/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| yis_home().join("memory"));
    ...
    ///
    /// File path: `~/.yis/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
