...
//! `xai_yis_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `YIS_POOL_*` /
//! `YIS_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `YIS_SAMPLER_SHARED_CLIENT=0` falls back to
...
use xai_yis_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `xai_yis_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `xai_yis_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use xai_yis_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `xai-yis-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `xai-yis-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `xai-yis-telemetry` consumes
// the same type via `xai_yis_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use xai_yis_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `YIS_CLIENT_NAME` /
/// `YIS_CLIENT_VERSION` env vars. Returns `None` when
/// `YIS_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("YIS_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("YIS_CLIENT_VERSION").ok(),
        })
}
...
/// Process-level client identifier (`YIS_CLIENT_NAME` env var, default `"yis-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("YIS_CLIENT_NAME").unwrap_or_else(|_| "yis-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`yis -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-yis-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `yis -p`, `run_headless_inner` for
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn xai_yis_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(xai_yis_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("grok-desktop/1.2.3 yis-shell/"));
        ...
        assert!(without_version.starts_with("grok-web yis-shell/"));
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "yis-shell/0.1.171 (macos; aarch64)");
    }
}
