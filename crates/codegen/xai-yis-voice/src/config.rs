...
/// `~/.yis/config.toml` (STT URL pieces, language, sample rate, endpointing)
...
/// `YIS_VOICE_MODE` override) — there is deliberately no local enable/disable
...
pub struct VoiceConfig {
    ...
    /// `x-yis-client-identifier` / `User-Agent` headers the sampler and imagine
    ...
    ///
    /// `x-yis-client-identifier` value (e.g. `"yis-shell"`).
    #[serde(skip)]
    pub client_identifier: String,
    /// `User-Agent` value (e.g. `"yis-shell/1.2.3 (macos; aarch64)"`).
    #[serde(skip)]
...
mod tests {
    ...
    /// voice gate — default on, remote kill switch / `YIS_VOICE_MODE`).
    #[test]
