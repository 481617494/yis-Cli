...
use xai_yis_pager::app::{
    AgentCmd, Command, HeadlessArgs, LeaderTargetArgs, PagerArgs, join_early_prefetch,
    resolve_use_leader,
};
use xai_yis_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use xai_yis_pager::client_identity::PAGER_CLIENT_VERSION;
use xai_yis_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use xai_yis_shell::agent::config::Config as AgentConfig;
use xai_yis_shell::leader::{
...
use xai_yis_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};

/// 本地模式跳过访问 x.ai 的最低版本强制检查。
async fn maybe_enforce_minimum_version(update_config: &UpdateConfig) {
    if xai_yis_pager::i18n::local_mode() {
        return;
    }
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.yis_ws_origin {
        config.yis_com_config.yis_ws_origin = v.clone();
    }
    if let Some(v) = &args.yis_ws_url {
        config.yis_com_config.yis_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &xai_yis_pager::app::AgentArgs,
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    let local = xai_yis_pager::i18n::local_mode();

    let registry = tracing_subscriber::registry()
        .with(fmt_layer.with_filter(env_filter))
        .with(xai_yis_telemetry::sampling_log::layer())
        .with(xai_yis_telemetry::instrumentation::layer())
        .with(xai_yis_telemetry::hooks_log::layer());

    if local {
        // 本地模式：仅保留本地日志层，不构建 OTLP 导出层
        xai_yis_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    } else {
        let registry = registry.with(xai_yis_telemetry::otel_layer::build_otel_layer(
            xai_yis_telemetry::otel_layer::OtelClientInfo {
                client_name: "yis-pager",
                client_version: xai_yis_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            xai_yis_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
        xai_yis_telemetry::debug_log::install_firehose(registry, app_entrypoint);
        xai_yis_telemetry::external::init(
            xai_yis_shell::agent::config::resolve_external_otel_config(
                xai_yis_telemetry::external::config::ExternalClientInfo {
                    service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                    client_version: xai_yis_version::VERSION.to_owned(),
...
/// `yis setup`: rendering + exit codes only; fetch logic lives in `xai_yis_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
/// 本地模式下不可用。
async fn run_setup_command(json: bool) {
    use xai_yis_shell::managed_config::{self, SetupOutcome};

    if xai_yis_pager::i18n::local_mode() {
        eprintln!("setup 命令在本地模式下不可用");
        std::process::exit(1);
    }
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `yis login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export YIS_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:YIS_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  grok setup");
        eprintln!();
        eprintln!("Or add the key to ~/.yis/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(xai_yis_shell::env::YisBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, xai_yis_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = xai_yis_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = xai_yis_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "yis-pager-leader-cli",
        ClientMode::Stdio,
...
/// Env override for the `yis workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "YIS_WORKSPACE_COMMAND";
/// Resolution of the `yis workspace` gate. `Unknown` is kept separate from
...
/// The `YIS_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&xai_yis_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<xai_yis_shell::util::config::RemoteSettings> {
    join_early_prefetch(xai_yis_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`yis workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `yis workspace`. Check your \
             network connection (run `yis login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.yis_com_config.yis_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a grok session, or run `yis workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = xai_yis_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use xai_yis_shell::auth::ensure_authenticated;

    // 本地模式：workspace 命令依赖 leader relay 连接，禁用
    if xai_yis_pager::i18n::local_mode() {
        anyhow::bail!("workspace 命令在本地模式下不可用");
    }

    xai_yis_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = xai_yis_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`yis workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.yis/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.yis_com_config,
        false,
        Some("No cached credentials found. Run `yis login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.yis_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    if !xai_yis_pager::i18n::local_mode() {
        xai_yis_telemetry::sentry::flush_on_shutdown();
        xai_yis_telemetry::otel_layer::shutdown_otel();
    }
    xai_yis_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<xai_yis_pager::app::AgentArgs>,
    ...
    let _otel_guard = xai_yis_telemetry::otel_layer::otel_guard();
    xai_yis_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => xai_yis_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = xai_yis_shell::agent::models::start_early_prefetch(None);
    xai_yis_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Yis Cli (pager) - v{}",
            xai_yis_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                xai_yis_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(xai_yis_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = xai_yis_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = xai_yis_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&xai_yis_shell::agent::config::RuntimeResolutionContext {
    ...
    // 本地模式：禁用 leader 模式（leader 依赖 relay WebSocket 连接外部服务）
    let use_leader = use_leader && !xai_yis_pager::i18n::local_mode();
    ...
    if use_leader {
        ...
        let env_urls = xai_yis_shell::leader::LeaderEnvUrls::from(&agent_config.yis_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = xai_yis_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            xai_yis_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(xai_yis_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = xai_yis_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = xai_yis_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `YIS_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
...
/// (`[dashboard].enabled = false` / `YIS_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !xai_yis_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.yis/config.toml and \
             unsetting YIS_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("YIS_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<xai_yis_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xai_yis_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<xai_yis_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(xai_yis_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    xai_yis_shell::heap_profile::install(xai_yis_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    // Yis Cli：默认简体中文 + 本地模式（减少强制访问 xAI 云）
    xai_yis_pager::i18n::init_from_env();
    xai_yis_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    xai_yis_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xai_yis_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xai_yis_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = xai_yis_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    xai_yis_pager::memory_trace::start(
        xai_yis_shell::util::yis_home::yis_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = xai_yis_config::validate_requirements() {
        eprintln!("无法启动 Yis Cli: {e}");
        eprintln!();
        eprintln!("请升级到策略允许的版本，或联系管理员修正托管要求。");
        std::process::exit(2);
    }
    let _sentry_guard = xai_yis_telemetry::sentry::init(xai_yis_telemetry::sentry::Config {
        client: "yis-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: xai_yis_pager::i18n::local_mode()
            || xai_yis_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    xai_yis_pager::docs::extract_user_guide_docs(&xai_yis_shell::util::yis_home::yis_home());
    xai_crash_handler::install_terminal_restore_only();
    if xai_yis_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = xai_yis_shell::util::yis_home::yis_home().join("crash");
    ...
    let crashed = xai_yis_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("YIS_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("YIS_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(xai_yis_shell::agent::chat_modes::YIS_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(xai_yis_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("YIS_DEBUG_LOG", path);
            std::env::remove_var("YIS_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("YIS_DEBUG_LOG", "1");
        set_if_unset("YIS_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        xai_yis_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return xai_yis_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        xai_yis_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        xai_yis_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    xai_yis_shell::config::apply_sandbox(
        None,
    ...
    xai_yis_shell::http::set_client_name(if is_interactive {
        xai_yis_workspace::permission::ClientType::YisPager
    } else {
        xai_yis_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        xai_yis_update::channel_name().unwrap_or("unknown"), }
                    );
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `yis-pager agent {flag}` instead."
                    );
                }
                maybe_enforce_minimum_version(&update_config).await;
            ...
            Command::Inspect { json } => {
                ...
                xai_yis_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
                if xai_yis_pager::i18n::local_mode() {
                    eprintln!("setup 命令在本地模式下不可用（需访问 grok.com 托管配置服务）");
                    std::process::exit(1);
                }
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xai_yis_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = xai_yis_telemetry::otel_layer::otel_guard();
                return xai_yis_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models { action } => {
                init_tracing_simple("cli");
                let _otel_guard = xai_yis_telemetry::otel_layer::otel_guard();
                use xai_yis_pager::app::cli::ModelsAction;
                match action.unwrap_or(ModelsAction::List) {
                    ModelsAction::List => {
                        let config = xai_yis_shell::config::load_effective_config_disk_only()
                            .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                        let agent_config = AgentConfig::new_from_toml_cfg(&config)
                            .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                        return xai_yis_pager::models::list_available_models(&agent_config).await;
                    }
                    ModelsAction::Setup => {
                        return xai_yis_pager::model_setup_cmd::run_interactive().await;
                    }
                    ModelsAction::Add {
                        preset,
                        api_key,
                        ...
                        return xai_yis_pager::model_setup_cmd::run_add(
                            &preset,
                            &api_key,
                            model.as_deref(),
                            base_url.as_deref(),
                    ...
                    ModelsAction::Presets => {
                        xai_yis_pager::model_setup_cmd::print_presets();
                        return Ok(());
                    }
                }
            }
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_yis_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_yis_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                if xai_yis_pager::i18n::local_mode() {
                    eprintln!("share 命令在本地模式下不可用（需上传会话到 grok.com）");
                    std::process::exit(1);
                }
                init_tracing_simple("cli");
                let _otel_guard = xai_yis_telemetry::otel_layer::otel_guard();
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_yis_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xai_yis_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_yis_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return xai_yis_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                if xai_yis_pager::i18n::local_mode() {
                    eprintln!("login 命令在本地模式下不可用（本地模式无需认证）");
                    std::process::exit(0);
                }
                init_tracing_simple("cli");
                let _otel_guard = xai_yis_telemetry::otel_layer::otel_guard();
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_yis_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                xai_yis_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                if xai_yis_pager::i18n::local_mode() {
                    eprintln!("logout 命令在本地模式下不可用（本地模式无需登出）");
                    std::process::exit(0);
                }
                init_tracing_simple("cli");
                let config = xai_yis_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_yis_shell::auth::run_cli_logout(&config)?;
                xai_yis_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = xai_yis_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(xai_yis_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == xai_yis_pager::headless::OutputFormat::Plain {
                args.output_format = xai_yis_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return xai_yis_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            xai_yis_pager::headless::HeadlessOptions {
    ...
    let result = xai_yis_pager::app::run(args, bg_update_rx).await;
    xai_yis_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `yis` to start.");
            } else {
                eprintln!("Update did not complete. Run `yis update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `yis update` child
...
/// fall back to a fresh blocking `yis update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = xai_yis_shell::env::YisBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                xai_yis_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("YIS_NPM_REGISTRY"))
        .ok()
        .or_else(xai_yis_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = xai_yis_shell::config::load_effective_config_disk_only()
        && let Some(ch) = xai_yis_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    if std::env::var_os("YIS_DISABLE_AUTOUPDATER").is_some() {
        return false;
    }
    // Yis 本地模式：不访问 x.ai/cli 做自动更新（国内无需梯子）
    if xai_yis_pager::i18n::local_mode() {
        return false;
    }
    true
...
/// `YIS_DISABLE_AUTOUPDATER`, debug builds) is layered on separately via
...
/// Handle `yis-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `yis update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in xai_yis_shell::leader::discover_leaders().await {
        if d.classification != xai_yis_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match xai_yis_shell::leader::LeaderClient::connect(
            socket_path,
            "yis-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(xai_yis_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(xai_yis_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            xai_yis_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: xai_yis_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            xai_yis_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!xai_yis_shell::heap_profile::prof_available());
            return;
        }
        assert!(xai_yis_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(xai_yis_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(xai_yis_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        xai_yis_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    ...
    #[serial_test::serial(YIS_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["yis", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("YIS_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("YIS_OPEN_DASHBOARD_AT_STARTUP") };
    }
    ...
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["yis", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(YIS_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("YIS_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("YIS_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("YIS_WORKSPACE_COMMAND", "off") };
