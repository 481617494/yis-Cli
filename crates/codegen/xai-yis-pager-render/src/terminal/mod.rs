...
pub enum TerminalName {
    ...
    JetBrains,
    /// Yis Desktop (Electron app).
    #[strum(to_string = "Yis Desktop")]
    GrokDesktop,
...
impl TerminalContext {
    ...
    pub fn telemetry_snapshot(&self) -> xai_yis_telemetry::events::TerminalTelemetry {
        ...
        xai_yis_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: xai_yis_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> xai_yis_shared::session::FeedbackTerminalInfo {
        use xai_yis_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(xai_yis_shared::clipboard::native_tool_name().to_owned()),
...
/// `xai-yis-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.yis/pager.toml` and
