...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("YIS_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "YIS_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    xai_yis_tools::util::detach_std_command(&mut command);
...
mod tests {
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://grok.com/supergrok", "referrer", "yis-build");
    ...
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "yis-build");
