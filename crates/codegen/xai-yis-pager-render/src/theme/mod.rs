...
//! The default theme is YisNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! YisNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::YisNight, ThemeKind::YisDay];
    ...
    /// when quantized to 256 or 16 colors. YisNight uses neutral grays
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "yisnight" | "yis-night" | "dark" => Some(Self::YisNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "yisday" | "yis-day" | "light" | "day" => Some(Self::YisDay),
...
/// `"yisnight"` → `"Yis Night"`). Falls back to `value` verbatim.
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.yis/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::YisNight => Self::yisnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::YisDay => Self::yisday(),
            ...
            // if reached, fall back to YisNight.
            ThemeKind::Auto => Self::yisnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `YIS_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on yisnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., YisNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // YisNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    xai_yis_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::YisNight.is_auto());
        assert!(!ThemeKind::YisDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::yisday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::yisnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::yisday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. YisNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::YisNight => Theme::yisnight(),
                ThemeKind::YisDay => Theme::yisday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_yisnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark YisNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::yisnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("yisnight"),
            Some(ThemeKind::YisNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::YisNight));
        assert_eq!(ThemeKind::from_name("yisday"), Some(ThemeKind::YisDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::YisDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("yisnight", ThemeKind::YisNight),
            ("yis-night", ThemeKind::YisNight),
            ("dark", ThemeKind::YisNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("yisday", ThemeKind::YisDay),
            ("yis-day", ThemeKind::YisDay),
            ("light", ThemeKind::YisDay),
            ("day", ThemeKind::YisDay),
