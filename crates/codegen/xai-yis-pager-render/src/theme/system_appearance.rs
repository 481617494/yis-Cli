...
/// in `config.toml`. When `None`, defaults to `YisNight` / `YisDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::YisDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::YisNight),
    }
}
