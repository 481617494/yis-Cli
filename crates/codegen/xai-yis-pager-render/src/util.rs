...
pub use xai_yis_config::yis_home;

/// Path to `$YIS_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    yis_home().join("pager.toml")
}

/// User-facing label for the user grok directory (``~/.yis`` or ``$YIS_HOME``).
///
/// Derived from resolved [`yis_home()`] vs `xai_yis_config::default_yis_home()`,
/// not from whether `YIS_HOME` is set in the environment.
pub fn display_yis_home_prefix() -> String {
    if yis_home() == xai_yis_config::default_yis_home() {
        "~/.yis".to_string()
    } else {
        "$YIS_HOME".to_string()
    }
...
/// User-facing path under [`yis_home()`], e.g. ``~/.yis/config.toml``.
pub fn display_user_yis_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_yis_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`yis_home()`], then `$HOME`.
...
/// True when `path` is under user [`yis_home()`] (not project `{cwd}/.yis`).
pub fn is_under_user_yis_home(path: &Path) -> bool {
    path.starts_with(yis_home())
}
...
mod tests {
    ...
    #[test]
    fn display_yis_home_prefix_default_install() {
        if std::env::var("YIS_HOME").is_ok() {
            return;
        }
        assert_eq!(display_yis_home_prefix(), "~/.yis");
    }

    #[test]
    fn display_user_yis_path_joins_relative() {
        let path = display_user_yis_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".yis") || path.contains("$YIS_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.yis/memory/MEMORY.md");
