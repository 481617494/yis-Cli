...
use xai_yis_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use xai_yis_sampling_types::ApiBackend;

/// Process-level fallback for the `x-yis-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "yis-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "yis-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-grok-*` headers. Optional fields are skipped when empty/`None`.
struct YisRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xai_yis_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xai_yis_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-yis-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-yis-client-version"),
                header_value,
            );
        }
...

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-yis-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    pub async fn chat_completion(
        ...
        let yis_headers = YisRequestHeaders {
    ...
    pub async fn create_response(
        ...
        xai_yis_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = yis_headers
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = yis_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = xai_yis_sampling_types::extra_raw_tools(&request.hosted_tools);
...
mod tests {
    ...
    use xai_yis_sampling_types::types::ChatRequestMessage;
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "xai-yis-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the yis-shell agent product.
        assert!(ua.starts_with("my-client yis-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = xai_yis_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "yis-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
