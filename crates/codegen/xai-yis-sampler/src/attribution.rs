...
//! it" buckets.
//!
//! `xai-yis-sampler` is intentionally decoupled from `xai-yis-shell`
...
/// tools live in `xai-yis-tools`
/// (`crates/codegen/xai-yis-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `xai_yis_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `xai_yis_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `xai-yis-shell/src/auth/manager.rs` to match, otherwise the
