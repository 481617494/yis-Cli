...
/// v1.18: Added `shell_version`: the yis-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like yis-desktop.
...
/// OS-level sandbox state for a trace turn (local `xai-yis-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// The agent type / harness name for this session (e.g. "yis-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the yis-shell agent binary that handled this turn
    /// (`xai_yis_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like yis-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
